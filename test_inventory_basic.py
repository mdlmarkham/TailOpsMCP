...
class TestBasicInventoryFunctionality(unittest.TestCase):
    ...
    def test_fleet_inventory_management(self):
        ...
        service1 = EnhancedService(
            name="nginx", target_id=target1.id, service_type="docker"
        )
    ...
    def test_serialization(self):
        ...
        inventory = EnhancedFleetInventory()
        target = EnhancedTarget(
            name="test-target", role=NodeRole.DEVELOPMENT, cpu_cores=2, memory_mb=4096
        )
    ...
    def test_persistence_layer(self):
        ...
        service = EnhancedService(
            name="web-service", target_id=target.id, service_type="docker", port=8080
        )
    ...
    def test_query_methods(self):
        ...
        prod_target = EnhancedTarget(name="prod-server", role=NodeRole.PRODUCTION)
        dev_target = EnhancedTarget(name="dev-server", role=NodeRole.DEVELOPMENT)
        staging_target = EnhancedTarget(name="staging-server", role=NodeRole.STAGING)
    ...
    def test_snapshot_creation(self):
        ...
        inventory = EnhancedFleetInventory()
        target = EnhancedTarget(
            name="snapshot-server", role=NodeRole.PRODUCTION, health_score=0.85
        )
        inventory.add_target(target)
    ...
    def test_snapshot_comparison(self):
        ...
        target1 = EnhancedTarget(
            name="server-01", role=NodeRole.PRODUCTION, cpu_cores=4, health_score=0.9
        )
        ...
        target2_new = EnhancedTarget(
            name="server-02", role=NodeRole.DEVELOPMENT, cpu_cores=2, health_score=0.7
        )
        ...
        snapshot1 = snapshot_manager.create_snapshot(
            inventory=inventory1, name="snapshot-before", description="Before changes"
        )

        snapshot2 = snapshot_manager.create_snapshot(
            inventory=inventory2, name="snapshot-after", description="After changes"
        )
...
def run_basic_tests():
    ...
    if result.wasSuccessful():
        ...
        print(
            "The Enhanced Fleet Inventory System core components are working correctly."
        )
    else:
