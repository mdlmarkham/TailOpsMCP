...
from src.models.proxmox_models import (
    ...
    ProxmoxContainer, ProxmoxVM, ProxmoxSnapshot, ProxmoxBackup,
    ContainerCreationResult, SnapshotResult, BackupResult
)
...
from src.services.proxmox_capabilities import ProxmoxCapabilityExecutor, ProxmoxCapabilityValidator
from src.tools.proxmox_tools import ProxmoxTools
from src.utils.proxmox_security import (
    ProxmoxSecurityLogger, ProxmoxSecurityManager, ProxmoxSecurityContext,
    ProxmoxSecurityEventType, SecuritySeverity
)
from src.utils.proxmox_monitoring import (
    ProxmoxHealthChecker, ProxmoxMetricsCollector, ProxmoxAlertManager,
    ProxmoxMonitoringService, HealthStatus, AlertSeverity
)
...
class TestProxmoxModels:
    ...
    def test_container_config_to_proxmox_config(self):
        ...
        assert proxmox_config["ostemplate"] == "local:vztmpl/debian-12-standard_12.7-1_amd64.tar.gz"
...
class TestProxmoxCLI:
    ...
    async def test_cli_availability_check(self, cli_client):
        ...
        with patch.object(cli_client, '_detect_proxmox_environment', return_value=False):
...
class TestProxmoxDiscovery:
    ...
    async def test_container_discovery(self, discovery_service):
        ...
        with patch.object(discovery_service, '_discover_containers_via_api') as mock_discover:
            mock_containers = [
                Mock(vmid=100, name="container-1", status="running", cores=2, memory=1024, disk=20),
                Mock(vmid=101, name="container-2", status="stopped", cores=1, memory=512, disk=10)
            ]
...
class TestProxmoxCapabilities:
    ...
    async def test_container_create_capability(self, capability_executor):
        ...
        with patch.object(capability_executor, '_get_api_client') as mock_client:
            ...
            mock_api.create_container = AsyncMock(return_value=ContainerCreationResult(
                vmid=200,
                status="created",
            ...
            result = await capability_executor.execute_proxmox_container_create(parameters)
    ...
    async def test_container_start_capability(self, capability_executor):
        ...
        with patch.object(capability_executor, '_get_api_client') as mock_client:
            ...
            result = await capability_executor.execute_proxmox_container_start(parameters)
    ...
    def test_capability_validation(self, capability_executor):
        ...
        errors = validator.validate_parameters("proxmox_container_create", invalid_params)
        assert len(errors) > 0
...
class TestProxmoxTools:
    ...
    async def test_discovery_tools(self, proxmox_tools):
        ...
        with patch.object(proxmox_tools.discovery_service, 'discover_proxmox_hosts') as mock_discover:
            mock_hosts = [
                Mock(hostname="pve-node-01", address="pve.test.com", node_name="pve-node-01")
            ]
    ...
    async def test_container_management_tools(self, proxmox_tools):
        ...
        with patch.object(proxmox_tools.capability_executor, 'execute_proxmox_container_start') as mock_execute:
            mock_execute.return_value = Mock(success=True, output="Container started successfully")
    ...
    async def test_snapshot_tools(self, proxmox_tools):
        ...
        with patch.object(proxmox_tools.capability_executor, 'execute_proxmox_snapshot_create') as mock_execute:
            mock_execute.return_value = Mock(success=True, output="Snapshot created successfully")
            
            result = await proxmox_tools.snapshot_ct(100, "daily-backup", "pve.test.com")
...
class TestProxmoxSecurity:
    ...
    def test_security_manager_credential_validation(self):
        ...
        with patch.object(security_manager, '_check_access_permissions', return_value=True):
            result = security_manager.validate_credentials(credentials, security_context)
            assert result is True
...
class TestProxmoxMonitoring:
    ...
    def test_metrics_prometheus_format(self, metrics_collector):
        ...
        assert "# HELP proxmox_host_up Whether the Proxmox host is reachable" in prometheus_output
        assert "proxmox_host_up{host=\"pve.test.com\"} 1" in prometheus_output
    
    def test_alert_evaluation(self):
        """Test alert rule evaluation."""
        from src.utils.proxmox_monitoring import AlertRule, AlertManager
...
class TestProxmoxIntegration:
    ...
    async def test_complete_container_lifecycle(self):
        ...
        with patch.object(tools, 'initialize', new_callable=AsyncMock):
            with patch.object(tools.capability_executor, 'execute_proxmox_container_create') as mock_create:
    ...
    async def test_monitoring_integration(self):
        ...
        # Mock health checks
        with patch.object(monitoring_service.health_checker, 'check_all_hosts') as mock_health:
