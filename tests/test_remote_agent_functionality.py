...
from src.connectors.remote_agent_connector import (
    RemoteAgentConnector, LogEntry, ServiceStatus, DockerContainer,
    FileInfo, CommandResult, OperationResult, HealthStatus
)
...
from src.services.connection_manager import RemoteConnectionManager, SSHConnectionImpl
from src.services.remote_operation_executor import ResilientRemoteOperation, OperationType
...
from src.utils.remote_security import (
    RemoteOperationSecurityManager, RemoteOperationAuditor,
    SecurityContext, SecurityLevel, AccessScope
)
...
class MockSSHConnection:
    ...
    async def _mock_journalctl_command(self, command: str) -> CommandResult:
        ...
        return CommandResult(command=command, exit_code=0, stdout="mock journalctl output", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
    ...
    async def _mock_systemctl_command(self, command: str) -> CommandResult:
        ...
        if "show" in command:
            ...
            return CommandResult(command=command, exit_code=0, stdout=output, stderr="", execution_time=0.1, timestamp=datetime.utcnow())
        return CommandResult(command=command, exit_code=0, stdout="mock systemctl output", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
    ...
    async def _mock_docker_command(self, command: str) -> CommandResult:
        ...
        if "ps" in command and "json" in command:
            ...
            return CommandResult(command=command, exit_code=0, stdout=json.dumps(mock_containers[0]), stderr="", execution_time=0.1, timestamp=datetime.utcnow())
        return CommandResult(command=command, exit_code=0, stdout="mock docker output", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
    
    async def _mock_file_command(self, command: str) -> CommandResult:
        """Mock file operations."""
        if "cat" in command:
            return CommandResult(command=command, exit_code=0, stdout="Mock file content", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
        elif "ls" in command:
            return CommandResult(command=command, exit_code=0, stdout="drwxr-xr-x 2 root root 4096 Dec 14 03:44 /tmp\n-rw-r--r-- 1 root root 123 Dec 14 03:44 test.txt", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
        elif "stat" in command:
            return CommandResult(command=command, exit_code=0, stdout="123 root root 644 1702535040 1702535040 1702535040", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
        return CommandResult(command=command, exit_code=0, stdout="mock file output", stderr="", execution_time=0.1, timestamp=datetime.utcnow())
...
class TestServiceConnector:
    ...
    def service_connector(self):
        mock_target = TargetConnection(executor=ExecutorType.SSH, host="test.example.com")
...
class TestRemoteOperationExecutor:
    ...
    async def test_execute_with_retry_success(self, executor):
        ...
        result = await executor.execute_with_retry(successful_operation, operation_name="test_op")
    ...
    async def test_execute_with_retry_failure(self, executor):
        ...
        result = await executor.execute_with_retry(failing_operation, operation_name="test_op")
    ...
    async def test_execute_with_timeout(self, executor):
        ...
        result = await executor.execute_with_timeout(slow_operation, timeout=1, operation_name="test_op")
...
class TestRemoteAgentTools:
    ...
    async def test_get_journald_logs(self, tools):
        ...
        with patch.object(tools, '_get_target_connection') as mock_target, \
             patch.object(tools, '_create_connector') as mock_connector:
            
            mock_target.return_value = TargetConnection(executor=ExecutorType.SSH, host="test.example.com")
            ...
            result = await tools.get_journald_logs("test.example.com", service="test-service", lines=10)
        ...
        with patch.object(tools, 'get_journald_logs') as mock_get_logs:
            ...
            mock_get_logs.return_value = {
                ...
                "logs": [
                    ...
                    {"level": "info", "message": "Test message 2"}  # Duplicate for testing
                ]
            }
...
class TestSecurityControls:
    ...
    def test_validate_operation_security_allowed(self, security_manager, security_context):
    ...
    def test_validate_operation_security_blocked(self, security_manager, security_context):
    ...
    def test_command_injection_detection(self, security_manager):
        ...
        has_injection = security_manager._detect_command_injection("test_operation", parameters)
    ...
    def test_rate_limiting(self, security_manager):
        ...
        is_allowed, error = security_manager._check_rate_limit("get_journald_logs", user_id)
...
class TestIntegration:
    ...
    async def test_full_workflow(self):
        ...
        result = await RemoteAgentCapabilities.get_journald_logs(target, {"service": "nginx"})
    ...
    async def test_fleet_operations(self):
        ...
        result = await RemoteAgentCapabilities.fleet_operations(targets, {
...
class TestMockSSHConnection:
    ...
    async def test_file_operations(self):
        ...
        # Test download
        download_result = await conn.download_file("/remote/test.txt", "/local/test.txt")
