...
from src.models.enhanced_fleet_inventory import (
    EnhancedFleetInventory, EnhancedTarget, EnhancedService, 
    EnhancedStack, NodeRole, HealthStatus, ServiceStatus
)
from src.models.inventory_snapshot import (
    SnapshotType, SnapshotDiff, InventorySnapshot, SnapshotMetadata
)
from src.tools.enhanced_inventory_tools import register_enhanced_inventory_tools
...
class TestInventoryOrchestration:
    ...
    @pytest.mark.asyncio
    async def test_inventory_discovery_and_synchronization(self, mock_inventory_service,
        ...
        mock_inventory_service.run_incremental_discovery.return_value = incremental_update
    ...
    @pytest.mark.asyncio
    async def test_health_monitoring_and_scoring(self, mock_inventory_service,
                                               sample_inventory_data):
        ...
        for scenario in health_scenarios:
            mock_inventory_service.get_target_health.return_value = {
                ...
                "issues": ["High memory usage"] if scenario["expected_status"] == HealthStatus.WARNING else [],
                "last_check": datetime.utcnow()
            }
            
            health_result = await mock_inventory_service.get_target_health(scenario["target"].id)
    ...
    @pytest.mark.asyncio
    async def test_change_detection_and_drift_monitoring(self, mock_inventory_service,
                                                       sample_inventory_data):
        ...
        # Add new target
        current_state["containers"].append(EnhancedTarget(
            id="container-103",
        ...
        changes = await mock_inventory_service.detect_changes(baseline_snapshot, current_state)
    ...
    @pytest.mark.asyncio
    async def test_inventory_snapshot_and_comparison(self, mock_inventory_service,
                                                   sample_inventory_data):
...
class TestInventoryEdgeCases:
    ...
    async def test_inventory_with_network_failures(self, mock_inventory_service):
        ...
        assert recovered_inventory.discovery_stats["errors"] == ["Network restoration in progress"]
    ...
    async def test_inventory_with_corrupted_data(self, mock_inventory_service):
        ...
        recovery_result = await mock_inventory_service.validate_and_recover_target(corrupted_target)
    ...
    async def test_inventory_with_large_scale_data(self, mock_inventory_service):
        ...
        large_inventory_data = {
            ...
            "containers": [
                EnhancedTarget(
                    ...
                    stacks=[],
                    last_seen=datetime.utcnow(),
                    metadata={"image": "ubuntu:20.04", "cpu_limit": "1", "memory_limit": "512MB"}
        ...
        processing_result = await mock_inventory_service.process_large_inventory(large_inventory_data)
...
class TestInventoryPerformance:
    ...
    async def test_discovery_performance(self, mock_inventory_service):
        ...
        for count in target_counts:
            ...
            mock_inventory_service.run_full_discovery.return_value = EnhancedFleetInventory(
                targets={f"target-{i}": EnhancedTarget(
                    id=f"target-{i}",
                    hostname=f"host-{i}",
                    role=NodeRole.CONTAINER,
            ...
            performance_results[count] = {
                ...
                "targets_per_second": count / processing_time if processing_time > 0 else float('inf')
            }
        ...
        for count in target_counts:
            ...
            if count > 100:  # For large inventories
                assert result["processing_time"] < count * 0.2  # Should scale sub-linearly
    ...
    async def test_inventory_query_performance(self, mock_inventory_service):
        ...
        for scenario in query_scenarios:
            ...
            mock_inventory_service.query_inventory.return_value = {
                "results": [{"id": f"result_{i}", "data": {}} for i in range(scenario["expected_results"])],
