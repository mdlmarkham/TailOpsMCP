...
from src.models.policy_models import SecurityTier
...
class TestSystemIntegration:
    ...
    async def test_end_to_end_provisioning_workflow(self, system_integration_framework):
        ...
        validation_result = await inventory_service.validate_environment(
        ...
        progress = await workflow_engine.get_workflow_status(
            provisioning_execution["execution_id"]
        )
        ...
        final_result = await workflow_engine.get_workflow_status(
            provisioning_execution["execution_id"]
        )
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_end_to_end_backup_and_recovery_workflow(
        self, system_integration_framework
    ):
        ...
        backup_snapshot = await inventory_service.create_backup_snapshot(
            test_data["test_environment"]
        )
        ...
        integrity_check = await inventory_service.validate_backup_integrity(
        ...
        recovery_execution = await workflow_engine.execute_recovery_workflow(
            recovery_request
        )
        ...
        recovery_progress = await workflow_engine.get_workflow_status(
            recovery_execution["execution_id"]
        )
        ...
        recovery_result = await workflow_engine.get_workflow_status(
            recovery_execution["execution_id"]
        )
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_end_to_end_security_compliance_workflow(
        self, system_integration_framework
    ):
        ...
        assessment_result = await inventory_service.run_security_assessment(
            security_assessment
        )
        ...
        enforcement_result = await policy_engine.enforce_security_policies(
            assessment_result,
            system_integration_framework["test_data"]["test_environment"],
        )
    ...
    async def test_multi_system_orchestration(self, system_integration_framework):
        ...
        environments = {
            ...
            "staging": {
                ...
                "proxmox_hosts": [
                    {"id": "staging-proxmox", "containers": ["staging-web"]}
                ],
            },
            "development": {
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_gateway_first_architecture_integration(
        self, system_integration_framework
    ):
        ...
        for operation in gateway_operations:
            ...
            validation_result = await policy_engine.validate_gateway_operation(
                operation
            )
        ...
        channel_result = await inventory_service.establish_secure_channel(
            secure_channel_test
        )
