...
from src.models.policy_models import (
    PolicyConfig, PolicyRule, PolicyAction, PolicyCondition,
    PolicyTarget, SecurityTier, CapabilityDefinition
)
from src.services.access_control import AccessControl, Role, Permission
from src.auth.middleware import secure_tool, PolicyGate
from src.tools.fleet_policy import FLEET_MANAGEMENT_POLICY
...
class TestPolicyOrchestration:
    ...
    @pytest.mark.asyncio
    async def test_capability_execution_with_policies(self, mock_policy_engine, 
    ...
    @pytest.mark.asyncio
    async def test_policy_evaluation_and_enforcement(self, mock_policy_engine,
                                                   sample_policy_config):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            mock_policy_engine.evaluate_policy.return_value = {
                ...
                "tier": SecurityTier.OBSERVE if test_case["operation"] == "fleet_discover" else SecurityTier.CONTROL
            }
    ...
    @pytest.mark.asyncio
    async def test_deny_by_default_security_posture(self, mock_policy_engine,
                                                  sample_policy_config):
    ...
    async def test_role_based_access_control(self, mock_access_control):
        ...
        roles = {
            "admin": [Permission.FLEET_READ, Permission.FLEET_WRITE, Permission.FLEET_EXECUTE],
    ...
    @pytest.mark.asyncio
    async def test_policy_validation_and_rollback(self, mock_policy_engine,
                                                sample_policy_config):
        ...
        mock_policy_engine.validate_policy.return_value = {
            "valid": True,
            "errors": [],
            "warnings": ["Consider adding time-based restrictions for control operations"]
        }
        
        validation_result = await mock_policy_engine.validate_policy(sample_policy_config)
...
class TestPolicySecurityCompliance:
    ...
    async def test_policy_injection_prevention(self, mock_policy_engine):
        ...
        sanitized_policy = await mock_policy_engine.sanitize_policy(malicious_policy_data)
        
        assert sanitized_policy["operations"]["exec"]["conditions"][0]["value"] != "rm -rf /"
...
class TestPolicyEdgeCases:
    ...
    async def test_policy_with_network_failure(self, mock_policy_engine):
        ...
        mock_policy_engine.validate_policy.side_effect = ConnectionError("Network unavailable")
        
        with pytest.raises(ConnectionError, match="Network unavailable"):
            await mock_policy_engine.validate_policy(PolicyConfig(
                version="1.0",
                policy_name="test",
                default_tier=SecurityTier.OBSERVE,
                operations={},
