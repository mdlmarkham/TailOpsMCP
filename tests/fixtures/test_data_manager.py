...
class TestDataManager:
    ...
    def _generate_proxmox_hosts_data(
        ...
        for i in range(host_count):
            host = {
                ...
                "containers": [
                    f"container-{j:04d}" for j in range((i * 10) + 1, (i * 10) + 11)
                ],
                "services": [
                    {
    ...
    def _generate_containers_data(self, config: TestDataConfig) -> List[Dict[str, Any]]:
        ...
        for i in range(container_count):
            ...
            container = {
                ...
                "stacks": [],
                "last_seen": (
                    datetime.utcnow() - timedelta(minutes=random.randint(1, 120))
                ).isoformat(),
                "metadata": {
                    "image": random.choice(
                        [
    ...
    def _generate_stacks_data(self, config: TestDataConfig) -> List[Dict[str, Any]]:
        ...
        for i in range(stack_count):
            stack = {
                ...
                "type": random.choice(
                    ["web", "database", "cache", "monitoring", "security"]
                ),
                ...
                "updated_at": (
                    datetime.utcnow() - timedelta(hours=random.randint(1, 168))
                ).isoformat(),
                "dependencies": random.sample(
                    [f"stack-{j + 1:03d}" for j in range(stack_count)],
                    random.randint(0, min(3, stack_count - 1)),
                )
                if stack_count > 1
                else [],
            }
            ...
            for impl in implementations:
                service = {
                    ...
                    "dependencies": random.sample(
                        [
                            f"{dep}-service"
                            for _, dep_list in service_types
                            for dep in dep_list
                            if dep != impl
                        ],
                        random.randint(0, 3),
    ...
    def _generate_workflow_blueprints(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(blueprint_count):
            blueprint = {
                ...
                "approvals_required": random.choice(
                    [[], ["admin"], ["admin", "security_team"]]
                ),
    ...
    def _generate_workflow_executions(
        ...
        for i in range(execution_count):
            ...
            execution = {
                ...
                "completed_at": (
                    started_at + timedelta(minutes=random.randint(5, 120))
                ).isoformat()
                if random.random() > 0.2
                else None,
                "current_step": f"step-{random.randint(1, 10):03d}"
                ...
                "rollback_executed": random.choice([True, False])
    ...
    def _generate_policies(self, config: TestDataConfig) -> List[Dict[str, Any]]:
        ...
        for i in range(policy_count):
            policy = {
                ...
                "scope": random.choice(
                    ["global", "environment", "resource_group", "resource"]
                ),
                ...
                "tags": random.sample(
                    ["production", "staging", "critical", "automated", "security"],
                    random.randint(2, 4),
                ),
            }
    ...
    def _generate_health_status(self, config: TestDataConfig) -> str:
        ...
        if config.edge_cases:
            ...
            return random.choices(
                ["healthy", "warning", "critical"], weights=[70, 25, 5]
            )[0]

    def _generate_stacks_for_host(
        self, host_index: int, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(stack_count):
            stack = {
                ...
                "status": random.choice(["running", "stopped", "error"]),
                "services": [
                    f"service-{host_index:03d}-{i:02d}-{j:02d}"
    ...
    def _generate_container_services(
        self, container_index: int, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_workflow_steps(self, config: TestDataConfig) -> List[Dict[str, Any]]:
        ...
        for i in range(step_count):
            step = {
                ...
                "conditions": self._generate_step_conditions()
    ...
    def _generate_policy_rules_for_policy(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_authz_violations(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_vulnerability_scans(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_security_audit_events(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_performance_events(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_event_correlations(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_workflow_schedules(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
        """Generate workflow schedule data."""
        return []

    def _generate_workflow_approvals(
        self, config: TestDataConfig
    ) -> List[Dict[str, Any]]:
...
def generate_inventory_test_data(
    count: int = 100, complexity: str = "medium"
) -> Dict[str, Any]:
    """Generate inventory test data with specified parameters."""
    config = TestDataConfig(
        category=DataCategory.INVENTORY, count=count, complexity=complexity
    )
    return test_data_manager.generate_inventory_data(config)


def generate_workflow_test_data(
    count: int = 50, complexity: str = "medium"
) -> Dict[str, Any]:
    """Generate workflow test data with specified parameters."""
    config = TestDataConfig(
        category=DataCategory.WORKFLOW, count=count, complexity=complexity
    )
    return test_data_manager.generate_workflow_data(config)


def generate_security_test_data(
    count: int = 200, security_level: SecurityLevel = SecurityLevel.MEDIUM
) -> Dict[str, Any]:
    """Generate security test data with specified parameters."""
    config = TestDataConfig(
        category=DataCategory.SECURITY, count=count, security_level=security_level
    )
    return test_data_manager.generate_security_data(config)


def generate_performance_test_data(
    count: int = 1000, complexity: str = "high"
) -> Dict[str, Any]:
    """Generate performance test data with specified parameters."""
    config = TestDataConfig(
        category=DataCategory.PERFORMANCE, count=count, complexity=complexity
    )
    return test_data_manager.generate_inventory_data(
        config
    )  # Reuse inventory for performance
