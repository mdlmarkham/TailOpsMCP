...
TEST_TARGETS_CONFIG = {
    "targets": {
        "local-host": {
            ...
            "metadata": {"hostname": "localhost", "os": "linux", "arch": "x86_64"},
        },
        "ssh-server": {
            "id": "ssh-server",
            "type": "remote",
        ...
        "docker-host": {
            ...
            "metadata": {"docker_version": "20.10", "storage_driver": "overlay2"},
        },
        "proxmox-node": {
            "id": "proxmox-node",
            "type": "proxmox",
            ...
            "metadata": {"proxmox_version": "7.4", "cluster_name": "test-cluster"},
        },
        "api-endpoint": {
            "id": "api-endpoint",
            "type": "api",
            ...
            "metadata": {"api_version": "v1", "provider": "custom"},
        },
    }
}
...
class TargetRegistryFixtures:
    ...
    def create_test_target(
        ...
        if constraints is None:
            constraints = {"timeout": 60, "concurrency": 1, "sudo_policy": "none"}
...
TEST_TARGETS = {
    ...
    "write-target": TargetRegistryFixtures.create_test_target(
        "write-target",
        capabilities=[
            Scope.CONTAINER_READ.value,
            Scope.CONTAINER_WRITE.value,
            Scope.SYSTEM_READ.value,
            Scope.FILE_WRITE.value,
        ],
    ),
    ...
    "restricted-target": TargetRegistryFixtures.create_test_target(
        ...
        constraints={"timeout": 30, "concurrency": 1, "sudo_policy": "none"},
    ),
}
...
def assert_target_metadata_equal(actual: TargetMetadata, expected: TargetMetadata):
    ...
    if expected.constraints:
        assert (
            actual.constraints.allowed_commands == expected.constraints.allowed_commands
        )
