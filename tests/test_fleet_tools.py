...
from src.tools.fleet_policy import load_fleet_management_policy, validate_operation_parameters
...
class TestFleetTools:
    ...
    @pytest.mark.asyncio
    async def test_fleet_discover_success(self, mock_mcp, mock_policy_gate, mock_discovery_manager):
        """Test successful fleet discovery."""
        with patch('src.tools.fleet_tools.is_gateway_mode', return_value=True), \
             patch('src.tools.fleet_tools.PolicyGate', return_value=mock_policy_gate), \
             patch('src.tools.fleet_tools.DiscoveryManager', return_value=mock_discovery_manager), \
            ...
            mock_discovery_manager.pipeline.run_discovery.return_value = mock_discovery_result
            ...
            # Call the function
            result = await fleet_discover(targets=["node1", "node2"], force_refresh=True, format="toon")
    ...
    async def test_fleet_discover_not_gateway_mode(self, mock_mcp):
        ...
        with patch('src.tools.fleet_tools.is_gateway_mode', return_value=False), \
    ...
    @pytest.mark.asyncio
    async def test_fleet_inventory_get_success(self, mock_mcp, mock_policy_gate, mock_inventory_persistence):
        """Test successful fleet inventory retrieval."""
        with patch('src.tools.fleet_tools.is_gateway_mode', return_value=True), \
             patch('src.tools.fleet_tools.PolicyGate', return_value=mock_policy_gate), \
             patch('src.tools.fleet_tools.FleetInventoryPersistence', return_value=mock_inventory_persistence), \
             patch('src.tools.fleet_tools.format_response') as mock_format_response:
    ...
    @pytest.mark.asyncio
    async def test_fleet_node_health_success(self, mock_mcp, mock_policy_gate, mock_inventory_persistence):
        ...
        with patch('src.tools.fleet_tools.is_gateway_mode', return_value=True), \
            ...
            mock_inventory.get_events_for_node.assert_called_once_with("test-node", limit=10)
            mock_format_response.assert_called_once()
    ...
    async def test_fleet_operation_execute_success(self, mock_mcp, mock_policy_gate):
        ...
        with patch('src.tools.fleet_tools.is_gateway_mode', return_value=True), \
             patch('src.tools.fleet_tools.PolicyGate', return_value=mock_policy_gate), \
             patch('src.tools.fleet_tools._retrieve_operation_plan') as mock_retrieve_plan, \
            ...
            mock_execute_plan.return_value = {"node1": {"success": True}, "node2": {"success": True}}
...
class TestFleetPolicy:
    ...
    def test_validate_operation_parameters_valid(self):
        ...
        errors = validate_operation_parameters(
            "restart_service",
            {"service": "nginx"}
        )
