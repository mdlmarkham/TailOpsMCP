...
from src.models.event_models import (
    ...
    EventBuilder, ResourceUsage, EventFilters
)
from src.services.event_collector import EventCollector, EventAggregator
from src.services.event_store import EventStore, get_event_store
from src.services.event_analyzer import EventAnalyzer, get_event_analyzer
from src.services.event_processor import EventStreamProcessor, EventStreamConfig
from src.services.event_alerting import (
    EventAlerting, AlertRule, Alert, NotificationConfig,
    NotificationChannel, create_default_alert_rules
)
from src.services.event_reporting import (
    EventReporting, TimeRange, HealthReport, SecurityReport,
    OperationalReport, ComplianceReport
)
from src.services.system_integration import (
    SystemIntegrationManager, FleetInventoryIntegration,
    PolicyEngineIntegration, SecurityAuditIntegration
)
from src.tools.event_management_tools import EventManagementTools
from src.utils.observability_config_manager import (
    ConfigManager, ObservabilityConfig, get_observability_config
)
...
class TestEventStore(unittest.TestCase):
    ...
    async def test_event_filtering(self):
        ...
        events = [
            SystemEvent(event_type=EventType.HEALTH_CHECK, severity=EventSeverity.INFO, source=EventSource.FLEET_INVENTORY),
            SystemEvent(event_type=EventType.ERROR, severity=EventSeverity.ERROR, source=EventSource.SYSTEM),
            SystemEvent(event_type=EventType.WARNING, severity=EventSeverity.WARNING, source=EventSource.PROXMOX_API),
        ]
    ...
    async def test_event_statistics(self):
        ...
        events = []
        for i in range(10):
            severity = EventSeverity.CRITICAL if i < 2 else EventSeverity.ERROR if i < 5 else EventSeverity.WARNING
            event = SystemEvent(
                severity=severity,
                source=EventSource.FLEET_INVENTORY,
                title=f"Event {i}"
...
class TestEventCollector(unittest.TestCase):
    ...
    async def test_event_aggregation(self):
        ...
        # Create test events
        events = [
            SystemEvent(event_type=EventType.HEALTH_CHECK, source=EventSource.FLEET_INVENTORY),
            SystemEvent(event_type=EventType.HEALTH_CHECK, source=EventSource.FLEET_INVENTORY),
...
class TestEventAnalyzer(unittest.TestCase):
    ...
    async def test_pattern_detection(self):
        ...
        for cycle in range(3):
            for i, event_type in enumerate([EventType.HEALTH_CHECK, EventType.ERROR, EventType.WARNING]):
                event = SystemEvent(
    ...
    async def test_insight_generation(self):
        ...
        if insights:
            ...
            self.assertIn(insight.insight_type, ['operational', 'security', 'performance', 'health'])
...
class TestEventAlerting(unittest.TestCase):
    ...
    async def test_alert_creation(self):
        ...
        success = await self.alerting.acknowledge_alert(alert.id, "test_user", "Acknowledged for testing")
        self.assertTrue(success)
        
        # Test alert resolution
        success = await self.alerting.resolve_alert(alert.id, "test_user", "Issue resolved")
        self.assertTrue(success)
...
class TestEventReporting(unittest.TestCase):
    ...
    async def test_report_export(self):
        ...
        exported_path = await self.reporting.export_report(health_report, "json", json_path)
...
class TestEventManagementTools(unittest.TestCase):
    ...
    async def test_get_health_summary(self):
        ...
        with patch.object(self.tools.event_reporting, 'generate_health_report') as mock_report:
    ...
    async def test_get_system_status(self):
        ...
        with patch.object(self.tools.event_store, 'get_statistics') as mock_stats:
            with patch.object(self.tools.event_alerting, 'get_alert_statistics') as mock_alerts:
...
async def event_store_fixture():
    ...
    for i in range(20):
        event = SystemEvent(
            event_type=EventType.HEALTH_CHECK if i % 3 == 0 else EventType.ERROR if i % 3 == 1 else EventType.WARNING,
            severity=EventSeverity.CRITICAL if i % 5 == 0 else EventSeverity.ERROR if i % 5 == 1 else EventSeverity.WARNING if i % 5 == 2 else EventSeverity.INFO,
