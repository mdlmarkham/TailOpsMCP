...
"""
        ...
        with patch('subprocess.run', return_value=Mock(returncode=100, stdout=yum_output)):
            result = await pm.check_updates()
...
class TestUpdateSystem:
    ...
    async def test_yum_update_system_success(self):
        ...
        with patch('subprocess.run', return_value=Mock(returncode=0, stdout='Complete!', stderr='')):
            result = await pm.update_system(auto_approve=True)
...
class TestInstallPackage:
    ...
    async def test_apt_install_success(self):
        ...
        with patch('subprocess.run', return_value=Mock(returncode=0, stdout='nginx installed', stderr='')):
    ...
    async def test_apt_install_failure(self):
        ...
        with patch('subprocess.run', return_value=Mock(returncode=1, stderr='Package not found')):
...
class TestOutputTruncation:
    ...
    async def test_install_output_truncation(self):
        ...
        with patch('subprocess.run', return_value=Mock(returncode=0, stdout=long_output, stderr='')):
            result = await pm.install_package('nginx')
