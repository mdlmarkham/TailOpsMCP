...
class TestProxmoxDiscovery(unittest.TestCase):
    ...
    def test_create_discovery_event(self):
        ...
        event = discovery.create_discovery_event(
            "test_source", "Test message", target_id="test-id", target_type="test-type"
        )
...
class TestDiscoveryPipeline(unittest.TestCase):
    ...
    def test_discovery_pipeline_initialization(self):
        ...
        config = {"discovery_interval": 300, "health_check_interval": 60}
    ...
    def test_should_run_discovery(self):
        ...
        pipeline.last_discovery = datetime.datetime.utcnow() - datetime.timedelta(
            seconds=100
        )  # 100 seconds ago
...
async def test_discovery_pipeline_integration():
    ...
    with patch(
        "src.services.proxmox_discovery.ProxmoxDiscovery.discover_proxmox_hosts"
    ) as mock_discover:
        with patch(
            "src.services.proxmox_discovery.ProxmoxDiscovery.discover_nodes"
        ) as mock_discover_nodes:
            with patch(
                "src.services.node_probing.NodeProbing.probe_node"
            ) as mock_probe:
                # Setup mocks
...
async def test_discovery_manager_operations():
    ...
    with patch(
        "src.services.discovery_pipeline.DiscoveryPipeline.run_discovery_cycle"
    ) as mock_run:
        # Setup mock
        mock_inventory = Mock()
