...
class MockPolicyGate:
    ...
    def was_operation_authorized(self, tool: str, target_id: str = "local") -> bool:
        ...
        for attempt in self.authorization_history:
            if attempt["tool"] == tool and attempt["target"] == target_id:
                # Find the corresponding result
                # This is simplified - in real implementation, we'd track results
                return (
                    tool,
                    target_id,
                ) in self.allowed_operations or self.default_allow
            ...
            for param_name, rule in rules.items():
                if param_name in parameters:
                    ...
                    if "min" in rule and value < rule["min"]:
                        errors.append(
                            f"Parameter {param_name} must be >= {rule['min']}"
                        )

                    if "max" in rule and value > rule["max"]:
                        errors.append(
                            f"Parameter {param_name} must be <= {rule['max']}"
                        )
...
def create_mock_policy_gate(
    default_allow: bool = True, validation_mode: ValidationMode = ValidationMode.STRICT
) -> MockPolicyGate:
    ...
    """
    return MockPolicyGate(default_allow=default_allow, validation_mode=validation_mode)


def create_mock_policy_gate_with_validation(
    default_allow: bool = True, validation_mode: ValidationMode = ValidationMode.STRICT
) -> MockPolicyGateWithValidation:
    ...
    return MockPolicyGateWithValidation(
        default_allow=default_allow, validation_mode=validation_mode
    )
...
class PolicyGateConfigs:
    ...
    def with_parameter_validation() -> MockPolicyGateWithValidation:
        ...
        gate.set_validation_rule(
            "start_container", "container_id", {"type": str, "min_length": 1}
        )

        gate.set_validation_rule(
            "deploy_stack",
            "stack_name",
            {"type": str, "allowed_values": ["web", "db", "cache"]},
        )

        gate.set_validation_rule(
            "update_system", "package_count", {"type": int, "min": 0, "max": 100}
        )
