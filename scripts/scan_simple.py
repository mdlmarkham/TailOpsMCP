...
class SimpleSecurityScanner:
    ...
    def generate_report(
        self, findings: Dict[str, Any], output_file: Optional[str] = None
    ) -> str:
        ...
        report = {
            ...
            "summary": {
                ...
                "total_issues": len(findings.get("secrets", []))
                + len(findings.get("bandit_issues", []))
                + len(findings.get("safety_issues", [])),
            },
            "findings": findings,
        }
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick scan (secrets + basic checks)"
    )

    parser.add_argument(
        "--secrets", action="store_true", help="Scan for exposed secrets only"
    )

    parser.add_argument(
        "--full", action="store_true", help="Full scan with bandit and safety"
    )
