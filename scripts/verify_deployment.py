...
class DeploymentVerifier:
    ...
    def check_systemd_available(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                False,
                "✗ systemd not found (use Docker deployment on non-systemd systems)",
            )
        except Exception as e:
        ...
        try:
            ...
            if len(lines) >= 2:
                ...
                if available_gb >= required_gb:
                    ...
                    return (
                        False,
                        f"✗ Only {available_gb:.1f}GB available (need {required_gb}GB)",
                    )
        except Exception as e:
        ...
        try:
            ...
            if len(lines) >= 2:
                ...
                if available_gb >= required_gb:
                    ...
                    return (
                        False,
                        f"✗ Only {available_gb}GB available (need {required_gb}GB)",
                    )
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--check-prereq", action="store_true", help="Check deployment prerequisites"
    )
    parser.add_argument(
        "--check-health", action="store_true", help="Check server health"
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument(
        "--host", default="localhost", help="Server hostname (for health checks)"
    )
    parser.add_argument(
        "--port", type=int, default=8080, help="Server port (for health checks)"
    )
