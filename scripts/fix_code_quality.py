...
class CodeQualityFixer:
    ...
    def fix_linting_issues(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "check", "--fix", "src", "tests"], check=False
        )
    ...
    def fix_formatting(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "format", "src", "tests"], check=False
        )
    ...
    def fix_import_sorting(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["isort", "--profile", "black", "src", "tests"], check=False
        )
    ...
    def fix_docstrings(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "check", "--select", "D", "--fix", "src", "tests"], check=False
        )
    ...
    def fix_type_annotations(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "check", "--select", "ANN", "--fix", "src", "tests"], check=False
        )
    ...
    def fix_security_issues(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "check", "--select", "S", "--fix", "src", "tests"], check=False
        )
    ...
    def fix_performance_issues(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "check", "--select", "PERF", "--fix", "src", "tests"], check=False
        )
    ...
    def fix_complexity_issues(self) -> Dict[str, Any]:
        ...
        success, stdout, stderr = self.run_command(
            ["ruff", "check", "--select", "C90", "--fix", "src", "tests"], check=False
        )
    ...
    def verify_fixes(self) -> Dict[str, Any]:
        ...
        lint_success, lint_output, _ = self.run_command(
            ["ruff", "check", "src", "tests"], check=False
        )
        ...
        import_success, import_output, _ = self.run_command(
            ["isort", "--check-only", "--diff", "src", "tests"], check=False
        )
...
Examples:
  ...
  %(prog)s --lint --security        # Fix linting and security issues
    ...
    parser.add_argument(
        "--all", action="store_true", help="Fix all auto-fixable issues (default)"
    )

    parser.add_argument(
        "--imports", action="store_true", help="Fix import sorting and organization"
    )

    parser.add_argument("--format", action="store_true", help="Fix code formatting")

    parser.add_argument("--lint", action="store_true", help="Fix linting issues")

    parser.add_argument(
        "--type-annotations", action="store_true", help="Fix missing type annotations"
    )

    parser.add_argument(
        "--docstrings", action="store_true", help="Fix missing docstrings"
    )

    parser.add_argument(
        "--security", action="store_true", help="Fix security-related issues"
    )

    parser.add_argument(
        "--performance", action="store_true", help="Fix performance-related issues"
    )

    parser.add_argument(
        "--complexity", action="store_true", help="Fix complexity-related issues"
    )
        ...
        if args.verbose:
            ...
            for tool, result in verification.items():
                status = (
                    "✅"
                    if result.get(
                        f"is_{tool}" if tool != "linting" else "no_issues", False
                    )
                    else "❌"
                )
