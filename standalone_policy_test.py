...
class PolicyRule:
    ...
    def __init__(self, name: str, operations: List[OperationType], 
                 target_roles: List[TargetRole], allowed: bool = True):
        self.name = name
...
class PolicyEngine:
    ...
    def evaluate_operation(self, operation: OperationType, 
                          target_role: TargetRole) -> PolicyDecision:
        """Simple policy evaluation."""
        # Check global rules
        for rule in self.config.global_policies:
            if (operation in rule.operations and 
                target_role in rule.target_roles and 
                rule.enabled):
        ...
        return PolicyDecision.DENY if self.config.deny_by_default else PolicyDecision.ALLOW
...
class CapabilityValidator:
    ...
    def validate_parameters(self, operation_type: OperationType, 
            ...
            if param_name in parameters:
                ...
                if expected_type == "string" and "max_length" in param_schema:
                    if len(param_value) > param_schema["max_length"]:
                        errors.append(f"Parameter {param_name}: exceeds max length {param_schema['max_length']}")
                
                # Range validation
                if expected_type == "int":
                    if "min" in param_schema and param_value < param_schema["min"]:
                        errors.append(f"Parameter {param_name}: below minimum {param_schema['min']}")
                    if "max" in param_schema and param_value > param_schema["max"]:
                        errors.append(f"Parameter {param_name}: above maximum {param_schema['max']}")
...
class ExecutionBackendFactory:
    ...
    def _create_default_backends(self):
        ...
        for op in [OperationType.SERVICE_RESTART, OperationType.SERVICE_START, 
        ...
        for op in [OperationType.SERVICE_RESTART, OperationType.CONTAINER_CREATE,
                   OperationType.CONTAINER_DELETE, OperationType.CONTAINER_START,
                   OperationType.CONTAINER_STOP, OperationType.BACKUP_CREATE,
...
def test_basic_functionality():
    ...
    # Add global rules
    config.add_global_rule(PolicyRule(
    ...
    config.add_role_rule(TargetRole.DEVELOPMENT, PolicyRule(
    ...
    config.add_role_rule(TargetRole.PRODUCTION, PolicyRule(
    ...
    decision = engine.evaluate_operation(OperationType.SERVICE_RESTART, TargetRole.DEVELOPMENT)
    print(f"Development service_restart: {decision.value}")
    
    decision = engine.evaluate_operation(OperationType.CONTAINER_DELETE, TargetRole.DEVELOPMENT)
    print(f"Development container_delete: {decision.value}")
    
    # Test production environment
    decision = engine.evaluate_operation(OperationType.SERVICE_RESTART, TargetRole.PRODUCTION)
    print(f"Production service_restart: {decision.value}")
    
    decision = engine.evaluate_operation(OperationType.CONTAINER_DELETE, TargetRole.PRODUCTION)
...
def test_capability_validation():
    ...
    is_valid, errors = validator.validate_parameters(OperationType.SERVICE_RESTART, missing_params)
...
def test_operation_scenarios():
    ...
    config.add_role_rule(TargetRole.DEVELOPMENT, PolicyRule(
        name="dev_permissive",
        operations=[OperationType.SERVICE_RESTART, OperationType.CONTAINER_CREATE,
                   OperationType.CONTAINER_DELETE, OperationType.STACK_DEPLOY],
        target_roles=[TargetRole.DEVELOPMENT],
        allowed=True
    ))
    ...
    scenarios = [
        ("Production service restart", OperationType.SERVICE_RESTART, TargetRole.PRODUCTION),
        ("Production service status", OperationType.SERVICE_STATUS, TargetRole.PRODUCTION),
        ("Development container delete", OperationType.CONTAINER_DELETE, TargetRole.DEVELOPMENT),
        ("Development stack deploy", OperationType.STACK_DEPLOY, TargetRole.DEVELOPMENT),
