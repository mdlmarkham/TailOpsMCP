...
def validate_system():
    ...
    with open("src/models/enhanced_fleet_inventory.py", 'r') as f:
        content = f.read()
        components = ["EnhancedTarget", "EnhancedService", "EnhancedStack", "EnhancedFleetInventory"]
    ...
    with open("src/utils/inventory_persistence.py", 'r') as f:
        content = f.read()
        features = ["EnhancedInventoryPersistence", "SQLite", "save_inventory", "load_inventory"]
    ...
    with open("src/services/inventory_service.py", 'r') as f:
        content = f.read()
        features = ["InventoryService", "run_full_discovery", "create_snapshot", "compare_snapshots"]
        for feature in features:
            if feature in content:
                print(f"  [OK] {feature}")
            else:
    ...
    with open("src/tools/enhanced_inventory_tools.py", 'r') as f:
        content = f.read()
        tools = ["run_fleet_discovery", "get_fleet_overview", "create_inventory_snapshot", "compare_snapshots"]
        for tool in tools:
            if tool in content:
                print(f"  [OK] {tool}")
            else:
