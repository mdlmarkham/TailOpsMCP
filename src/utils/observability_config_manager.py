...
class EventCollectionConfig:
    ...
    fleet_inventory_config: Dict[str, Any] = field(default_factory=lambda: {
        "health_check_interval": 300,
        "resource_check_interval": 60,
        "enabled": True
    })
    
    policy_engine_config: Dict[str, Any] = field(default_factory=lambda: {
        "violation_check_interval": 60,
        "audit_check_interval": 300,
        "enabled": True
    })
    
    security_audit_config: Dict[str, Any] = field(default_factory=lambda: {
        "audit_interval": 300,
        "log_level_filter": "INFO",
        "enabled": True
    })
    
    remote_agent_config: Dict[str, Any] = field(default_factory=lambda: {
        "service_status_interval": 60,
        "enabled": True
    })
    
    discovery_pipeline_config: Dict[str, Any] = field(default_factory=lambda: {
        "discovery_interval": 600,
        "enabled": True
    })
    
    proxmox_config: Dict[str, Any] = field(default_factory=lambda: {
...
class EventStorageConfig:
    ...
    indexes: List[str] = field(default_factory=lambda: [
        "timestamp", "event_type", "severity", "source", "target", "category"
    ])
...
class AlertingConfig:
    ...
    slack_config: Dict[str, Any] = field(default_factory=lambda: {
        "webhook_url": "",
        "channel": "#alerts",
        "username": "TailOpsMCP"
    })
    
    webhook_config: Dict[str, Any] = field(default_factory=lambda: {
        "urls": {},
        "timeout": 10,
        "retry_attempts": 3
    })
    ...
    escalation_delays: List[int] = field(default_factory=lambda: [15, 30, 60])  # minutes
...
class ObservabilityConfig:
    ...
    event_collection: EventCollectionConfig = field(default_factory=EventCollectionConfig)
    ...
    event_processing: EventProcessingConfig = field(default_factory=EventProcessingConfig)
...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = config_path or os.getenv("OBSERVABILITY_CONFIG_PATH", "./config/observability.yaml")
    ...
    def _apply_environment_overrides(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if os.getenv("EVENT_COLLECTION_INTERVAL"):
            config_data.setdefault('event_collection', {})['interval'] = int(os.getenv("EVENT_COLLECTION_INTERVAL"))
        
        if os.getenv("EVENT_COLLECTION_ENABLED"):
            config_data.setdefault('event_collection', {})['enabled'] = os.getenv("EVENT_COLLECTION_ENABLED").lower() == "true"
        
        # Event storage settings
        if os.getenv("EVENT_DB_PATH"):
            config_data.setdefault('event_storage', {})['database_path'] = os.getenv("EVENT_DB_PATH")
        
        if os.getenv("EVENT_RETENTION_DAYS"):
            config_data.setdefault('event_storage', {})['retention_days'] = int(os.getenv("EVENT_RETENTION_DAYS"))
        
        # Event processing settings
        if os.getenv("WEBSOCKET_ENABLED"):
            config_data.setdefault('event_processing', {})['websocket_enabled'] = os.getenv("WEBSOCKET_ENABLED").lower() == "true"
        
        if os.getenv("WEBSOCKET_PORT"):
            config_data.setdefault('event_processing', {})['websocket_port'] = int(os.getenv("WEBSOCKET_PORT"))
        
        # Alerting settings
        if os.getenv("ALERTING_ENABLED"):
            config_data.setdefault('alerting', {})['enabled'] = os.getenv("ALERTING_ENABLED").lower() == "true"
        
        if os.getenv("EMAIL_ENABLED"):
            config_data.setdefault('alerting', {})['email_enabled'] = os.getenv("EMAIL_ENABLED").lower() == "true"
        
        if os.getenv("SMTP_HOST"):
            config_data.setdefault('alerting', {}).setdefault('email_config', {})['smtp_host'] = os.getenv("SMTP_HOST")
        
        if os.getenv("SMTP_USERNAME"):
            config_data.setdefault('alerting', {}).setdefault('email_config', {})['username'] = os.getenv("SMTP_USERNAME")
        
        if os.getenv("SMTP_PASSWORD"):
            config_data.setdefault('alerting', {}).setdefault('email_config', {})['password'] = os.getenv("SMTP_PASSWORD")
        
        if os.getenv("SLACK_ENABLED"):
            config_data.setdefault('alerting', {})['slack_enabled'] = os.getenv("SLACK_ENABLED").lower() == "true"
        
        if os.getenv("SLACK_WEBHOOK_URL"):
            config_data.setdefault('alerting', {}).setdefault('slack_config', {})['webhook_url'] = os.getenv("SLACK_WEBHOOK_URL")
        
        # Reporting settings
        if os.getenv("REPORTING_ENABLED"):
            config_data.setdefault('reporting', {})['enabled'] = os.getenv("REPORTING_ENABLED").lower() == "true"
        
        if os.getenv("AUTO_GENERATE_REPORTS"):
            config_data.setdefault('reporting', {})['auto_generate_reports'] = os.getenv("AUTO_GENERATE_REPORTS").lower() == "true"
        
        if os.getenv("EXPORT_DIRECTORY"):
            config_data.setdefault('reporting', {})['export_directory'] = os.getenv("EXPORT_DIRECTORY")
    ...
    def _validate_config(self, config: ObservabilityConfig) -> None:
        ...
        if config.event_processing.websocket_port <= 0 or config.event_processing.websocket_port > 65535:
        ...
        # Validate reporting
        if config.reporting.dashboard_port <= 0 or config.reporting.dashboard_port > 65535:
            errors.append("Dashboard port must be between 1 and 65535")
        
        if errors:
            raise ValueError(f"Configuration validation failed: {', '.join(errors)}")
    
    def save_config(self, config: ObservabilityConfig, path: Optional[str] = None) -> None:
