...
from src.models.enhanced_fleet_inventory import (
    ...
    NodeRole, ResourceStatus, SecurityStatus
)
from src.models.inventory_snapshot import (
    InventorySnapshot, SnapshotManager, SnapshotType, SnapshotDiff
)
from src.models.fleet_inventory import ProxmoxHost, Event
...
class EnhancedInventoryPersistence:
    ...
    def __init__(self, db_path: Optional[str] = None, use_sqlite: bool = True):
        ...
        if db_path is None:
            ...
            if os.path.exists('/var/lib/systemmanager'):
                ...
                default_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "enhanced_inventory.db")
    ...
    def _init_enhanced_schema(self) -> None:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_targets_host_id ON enhanced_targets(host_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_targets_role ON enhanced_targets(role)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_targets_status ON enhanced_targets(status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_targets_last_seen ON enhanced_targets(last_seen)")
            
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_services_target_id ON enhanced_services(target_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_services_stack_name ON enhanced_services(stack_name)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_services_status ON enhanced_services(status)")
            
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_stacks_status ON enhanced_stacks(stack_status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_stacks_last_deployed ON enhanced_stacks(last_deployed)")
            
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_snapshots_type ON enhanced_snapshots(snapshot_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_snapshots_created_at ON enhanced_snapshots(created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_enhanced_snapshots_expires_at ON enhanced_snapshots(expires_at)")
    ...
    def _save_inventory_sqlite(self, inventory: EnhancedFleetInventory) -> None:
        ...
        with self._get_connection() as conn:
            ...
            for target in inventory.targets.values():
                ...
                """, (
                    target.id, target.name, target.node_type.value, target.host_id, target.vmid,
                    target.status, target.cpu_cores, target.memory_mb, target.disk_gb,
                    target.ip_address, target.mac_address, target.runtime.value,
                    target.connection_method.value, target.role.value, target.description,
                    json.dumps(target.environment), target.resource_usage.dict(),
                    target.security_posture.dict(),
                    target.container_info.dict() if target.container_info else None,
                    json.dumps(target.services), json.dumps(target.stacks),
                    target.last_seen, target.last_health_check, target.health_score,
                    target.created_at, target.last_updated, target.is_managed,
                    target.is_active, json.dumps(target.tags), json.dumps(target.custom_attributes)
                ))
            ...
            for service in inventory.services.values():
                ...
                """, (
                    service.id, service.name, service.target_id, service.service_type,
                    service.status.value, service.version, service.port,
                    service.protocol, service.config_path, service.data_path,
                    service.health_endpoint, service.stack_name,
                    json.dumps(service.depends_on), json.dumps(service.environment),
                    service.cpu_limit, service.memory_limit, service.restart_policy,
                    service.health_check_enabled, service.health_check_interval,
                    service.health_check_timeout, service.health_check_retries,
                    service.last_health_check, service.health_status,
                    service.tls_enabled, service.tls_port,
                    json.dumps(service.exposed_ports), json.dumps(service.security_context),
                    service.created_at, service.last_checked, service.last_updated,
                    service.is_monitored, service.is_managed,
                    json.dumps(service.tags), json.dumps(service.custom_attributes)
                ))
            ...
            for stack in inventory.stacks.values():
                ...
                """, (
                    stack.id, stack.name, stack.description, stack.compose_file_path,
                    stack.stack_file_content, json.dumps(stack.variables),
                    json.dumps(stack.services), json.dumps(stack.targets),
                    stack.stack_status, stack.last_deployed, stack.deployment_method,
                    stack.namespace, stack.health_score, stack.last_health_check,
                    json.dumps(stack.security_scan_results), stack.compliance_status,
                    stack.total_cpu_cores, stack.total_memory_mb, stack.total_disk_gb,
                    stack.created_at, stack.last_updated, stack.is_active,
                    json.dumps(stack.tags), json.dumps(stack.custom_attributes)
                ))
        ...
        with self._get_connection() as conn:
            ...
            for row in cursor:
                ...
                target = EnhancedTarget(
                    ...
                    resource_usage=EnhancedTarget(**{}).resource_usage.__class__(**json.loads(row['resource_usage'] or '{}')),
                    security_posture=EnhancedTarget(**{}).security_posture.__class__(**json.loads(row['security_posture'] or '{}')),
            ...
            for row in cursor:
                stack = EnhancedStack(
                    ...
                    health_score=row['health_score'],
                    last_health_check=row['last_health_check'],
                    security_scan_results=json.loads(row['security_scan_results'] or '{}'),
    ...
    def _save_snapshot_sqlite(self, snapshot: InventorySnapshot) -> None:
        ...
        with self._get_connection() as conn:
            ...
            """, (
                ...
                snapshot.snapshot_type.value, snapshot.created_at, snapshot.created_by,
                json.dumps(snapshot.tags), json.dumps(snapshot.inventory_data),
                snapshot.total_targets, snapshot.total_services, snapshot.total_stacks,
                snapshot.healthy_targets, snapshot.average_health_score,
                snapshot.size_bytes, snapshot.compression_ratio, snapshot.expires_at,
    ...
    def _load_snapshot_sqlite(self, snapshot_id: str) -> Optional[InventorySnapshot]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM enhanced_snapshots WHERE id = ?",
                (snapshot_id,)
            )
            row = cursor.fetchone()
    ...
    def _delete_snapshot_sqlite(self, snapshot_id: str) -> bool:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                "DELETE FROM enhanced_snapshots WHERE id = ?",
                (snapshot_id,)
            )
    ...
    def get_targets_by_role(self, role: NodeRole) -> List[EnhancedTarget]:
        ...
        if self.use_sqlite:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM enhanced_targets WHERE role = ?",
                    (role.value,)
                )
                return [self._row_to_target(row) for row in cursor]
        else:
            inventory = self.load_inventory()
            return [target for target in inventory.targets.values() if target.role == role]
    ...
    def get_targets_by_status(self, status: str) -> List[EnhancedTarget]:
        ...
        if self.use_sqlite:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM enhanced_targets WHERE status = ?",
                    (status,)
                )
                return [self._row_to_target(row) for row in cursor]
        else:
            inventory = self.load_inventory()
            return [target for target in inventory.targets.values() if target.status == status]
    ...
    def get_unhealthy_targets(self, threshold: float = 0.7) -> List[EnhancedTarget]:
        ...
        if self.use_sqlite:
            ...
            inventory = self.load_inventory()
            return [target for target in inventory.targets.values() if target.health_score < threshold]
    ...
    def get_services_by_stack(self, stack_name: str) -> List[EnhancedService]:
        ...
        if self.use_sqlite:
            ...
            inventory = self.load_inventory()
            return [service for service in inventory.services.values() if service.stack_name == stack_name]
    ...
    def search_targets(self, query: str) -> List[EnhancedTarget]:
        ...
        if self.use_sqlite:
            ...
            results = []
            for target in inventory.targets.values():
                if (query.lower() in target.name.lower() or 
                    (target.description and query.lower() in target.description.lower()) or
                    any(query.lower() in tag.lower() for tag in target.tags)):
    ...
    def _row_to_target(self, row) -> EnhancedTarget:
        ...
        if row['security_posture']:
            ...
            target.security_posture = SecurityPosture(**json.loads(row['security_posture']))
        ...
        with self._get_connection() as conn:
            ...
            for row in cursor:
                ...
                if snapshot:
                    ...
                    archive_file = os.path.join(self.archive_dir, f"{snapshot_id}.json.gz")
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        if self.use_sqlite:
            with self._get_connection() as conn:
                # Total snapshots
                cursor = conn.execute("SELECT COUNT(*) as count FROM enhanced_snapshots")
                stats["snapshot_count"] = cursor.fetchone()['count']
                
                # Archived snapshots
                cursor = conn.execute("SELECT COUNT(*) as count FROM enhanced_snapshots WHERE is_archived = TRUE")
