...
from src.models.proxmox_models import (
    ProxmoxAPICredentials, ProxmoxContainer, ProxmoxVM, ProxmoxNode
)
...
class ProxmoxHealthChecker:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_check": max(self.last_check.values()).isoformat() + "Z" if self.last_check else None
        }
        ...
        try:
            ...
            if nodes:
                ...
                for node in nodes:
                    ...
                    if node.mem is not None and node.maxmem is not None and node.maxmem > 0:
    ...
    async def collect_all_metrics(self) -> List[ProxmoxMetric]:
        ...
        for credentials, metrics in zip(self.api_credentials, host_metrics):
            if isinstance(metrics, Exception):
                logger.error(f"Metrics collection failed for {credentials.host}: {metrics}")
                # Add error metric
                all_metrics.append(ProxmoxMetric(
        ...
        for metric_name, metrics in metric_groups.items():
            if metrics:
                ...
                if first_metric.help_text:
                    metric_lines.append(f"# HELP {metric_name} {first_metric.help_text}")
...
class ProxmoxAlertManager:
    ...
    def __init__(self, alert_rules: List[AlertRule] = None):
        ...
        logger.info(f"Proxmox alert manager initialized with {len(self.alert_rules)} rules")
    ...
    def evaluate_alerts(self, health_results: Dict[str, HealthCheckResult], 
                       metrics: List[ProxmoxMetric]) -> List[Dict[str, Any]]:
        ...
        for rule in self.alert_rules:
            ...
            try:
                ...
                if alert_triggered:
                    ...
                    alert_key = f"{rule.name}:{alert.get('labels', {}).get('host', 'unknown')}"
    ...
    def _create_alert(self, rule: AlertRule, context: Dict[str, Any],
                     health_results: Dict[str, HealthCheckResult],
                     metrics: List[ProxmoxMetric]) -> Dict[str, Any]:
    ...
    def _cleanup_resolved_alerts(self, triggered_alerts: List[Dict[str, Any]]):
        ...
        """
        triggered_keys = {f"{alert['name']}:{alert.get('labels', {}).get('host', 'unknown')}" 
...
class ProxmoxMonitoringService:
    ...
    async def _send_to_monitoring_systems(self, 
                                        health_results: Dict[str, HealthCheckResult],
    ...
    async def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_check": max(self.health_checker.last_check.values()).isoformat() + "Z" 
                         if self.health_checker.last_check else None
        }
    ...
    async def manual_health_check(self) -> Dict[str, Any]:
        ...
        return {
            "timestamp": datetime.utcnow().isoformat() + "Z",
            "results": {host: asdict(result) for host, result in health_results.items()},
...
def create_proxmox_monitoring_service(config: Dict[str, Any]) -> ProxmoxMonitoringService:
