    ...
    def export_metrics_for_dashboard(metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "numeric_metrics": sum(
                    1 for v in metrics.values() if isinstance(v, (int, float))
                ),
                "string_metrics": sum(
                    1 for v in metrics.values() if isinstance(v, str)
                ),
            },
        }

    @staticmethod
    def export_logs_for_dashboard(
        logs: List[Dict[str, Any]], limit: int = 100
    ) -> Dict[str, Any]:
