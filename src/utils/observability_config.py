...
class AuditLogConfig:
    ...
    DEFAULT_CONFIG = {
        "sinks": [
            ...
            {
                "type": "console",
                "enabled": os.getenv("SYSTEMMANAGER_LOG_CONSOLE", "true").lower()
                == "true",
                "format": "human",
            },
        ],
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        config["retention"]["max_age_days"] = int(
            os.getenv(
                "SYSTEMMANAGER_LOG_RETENTION_DAYS", config["retention"]["max_age_days"]
            )
        )

        config["retention"]["max_size_mb"] = int(
            os.getenv(
                "SYSTEMMANAGER_LOG_MAX_SIZE_MB", config["retention"]["max_size_mb"]
            )
        )
...
class ObservabilityConfig:
    ...
    def __init__(self):
        self.metrics_enabled = (
            os.getenv("SYSTEMMANAGER_METRICS_ENABLED", "true").lower() == "true"
        )
        self.health_checks_enabled = (
            os.getenv("SYSTEMMANAGER_HEALTH_CHECKS_ENABLED", "true").lower() == "true"
        )
        self.monitoring_enabled = (
            os.getenv("SYSTEMMANAGER_MONITORING_ENABLED", "false").lower() == "true"
        )
