...
def quick_serialize(data: dict, token_budget: int = DEFAULT_TOKEN_BUDGET) -> SerializationResult:
    """Quick serialization with default settings."""
    serializer = TOONSerializer()
    return serializer.serialize(data, token_budget=token_budget)

def quick_format(result: SerializationResult, format_type: LLMFormat = DEFAULT_FORMAT) -> str:
    """Quick formatting with default settings."""
    formatter = TOONLLMFormatter()
    return formatter.format(result, format_type=format_type)

def quick_toon(data: dict, token_budget: int = DEFAULT_TOKEN_BUDGET, format_type: LLMFormat = DEFAULT_FORMAT) -> str:
...
def _initialize_package():
    ...
    logger.info(f"TOON package initialized with {len(config.enabled_features)} enabled features")
