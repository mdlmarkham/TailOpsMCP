...
class TOONTemplates:
    ...
    def apply_template(
        self, document: TOONDocument, template_type: TemplateType
    ) -> TOONDocument:
...
class TOONLLMFormatter:
    ...
    def format_for_conversation(
        self, document: TOONDocument, context: FormattingContext
    ) -> FormattedResponse:
    ...
    def _format_section(
        self, section_name: str, content: Any, context: FormattingContext
    ) -> str:
    ...
    def _format_actionable(self, content: Any) -> str:
        ...
        if isinstance(content, dict):
            ...
            for key, value in content.items():
                if (
                    "error" in key.lower()
                    or "issue" in key.lower()
                    or "problem" in key.lower()
                ):
    ...
    def _group_sections_by_priority(
        self, document: TOONDocument
    ) -> Dict[ContentPriority, List[Tuple[str, Any]]]:
            ...
            if priority in [ContentPriority.CRITICAL, ContentPriority.IMPORTANT]:
                if isinstance(content, dict):
                    for key, value in content.items():
                        ...
                        elif "status" in key.lower() and value not in [
                            "healthy",
                            "running",
                            "ok",
                        ]:
                    ...
                    insights.append(
                        f"Review {section_name}: {len(content)} items need attention"
                    )
...
def create_optimized_document(
    data: Dict[str, Any], template_type: TemplateType
) -> TOONDocument:
    """Create optimized document using template."""
    from .serializer import TOONDocument, ContentPriority

    doc = TOONDocument(document_type=template_type.value, created_at=datetime.now())
