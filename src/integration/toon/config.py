...
from .serializer import TOONVersion, QualityLevel
...
class TOONSystemConfig:
    ...
    enabled_features: List[str] = field(default_factory=lambda: [
        "serialization", "formatting", "caching", "quality_checks"
    ])
...
class TOONConfigManager:
    ...
    def save_config(self, config: TOONSystemConfig, config_path: Optional[str] = None) -> None:
    ...
    def _load_from_environment(self) -> TOONSystemConfig:
        ...
        if os.getenv('TOON_TOKEN_BUDGET'):
            config.serialization.default_token_budget = int(os.getenv('TOON_TOKEN_BUDGET'))
    ...
    def _has_env_overrides(self) -> bool:
        ...
        env_vars = [
            'TOON_VERSION', 'TOON_FORMAT', 'TOON_TOKEN_BUDGET',
            'TOON_CACHE_SIZE', 'TOON_ENVIRONMENT'
        ]
