...
class FileConnector(RemoteAgentConnector):
    ...
    async def read_file(self, path: str, max_size: Optional[int] = None) -> str:
        ...
        if size_result.size > max_size:
            raise SystemManagerError(
                f"File too large: {size_result.size} bytes (limit: {max_size})"
            )
        ...
        try:
            ...
            if result.exit_code != 0:
                if "Permission denied" in result.stderr:
                    raise SecurityError(
                        f"Permission denied reading file: {validated_path}"
                    )
    ...
    async def write_file(
        ...
        try:
            ...
            if result.exit_code != 0:
                raise SystemManagerError(
                    f"Failed to write temporary file: {result.stderr}"
                )
            ...
            if result.exit_code != 0:
                raise SystemManagerError(
                    f"Failed to move file to final location: {result.stderr}"
                )
            ...
            self._log_file_operation(
                "write", validated_path, True, backup_path=backup_path
            )
    ...
    async def list_directory(
        ...
        try:
            ...
            if result.exit_code != 0:
                if "Permission denied" in result.stderr:
                    raise SecurityError(
                        f"Permission denied listing directory: {validated_path}"
                    )
                elif "No such file" in result.stderr:
            ...
            # Log operation
            self._log_file_operation(
                "list", validated_path, True, file_count=len(files)
            )
    ...
    async def get_file_stats(self, path: str) -> FileStats:
        ...
        try:
            ...
            if result.exit_code != 0:
                if "Permission denied" in result.stderr:
                    raise SecurityError(
                        f"Permission denied getting stats for: {validated_path}"
                    )
                elif "No such file" in result.stderr:
                    raise SystemManagerError(f"File not found: {validated_path}")
                else:
                    raise SystemManagerError(
                        f"Failed to get file stats: {result.stderr}"
                    )
    ...
    async def search_files(
        ...
        try:
            ...
            search_results = await self._parse_search_output(
                result.stdout, validated_dir, pattern
            )
    ...
    async def get_directory_tree(
        self, directory: str, max_depth: int = 3, include_files: bool = True
    ) -> DirectoryTreeNode:
    ...
    async def compare_files(self, path1: str, path2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"File comparison failed: {validated_path1} vs {validated_path2}: {str(e)}"
            )
            raise
    ...
    async def _check_file_permission(self, operation: str) -> bool:
        ...
        try:
            ...
            if operation == "read":
                ...
                cmd = "test -w /tmp && echo 'yes' || echo 'no'"
            elif operation == "list":
                cmd = "test -x /tmp && echo 'yes' || echo 'no'"
            elif operation == "stat":
    ...
    async def _parse_search_output(
        self, output: str, directory: str, pattern: str
    ) -> List[FileSearchResult]:
