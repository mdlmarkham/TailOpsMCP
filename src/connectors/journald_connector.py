...
class JournaldConnector(RemoteAgentConnector):
    ...
    @resilient_operation(
        operation_type=OperationType.LOG_RETRIEVAL, operation_name="get_journald_logs"
    )
    async def get_logs(
        self,
    ...
    async def get_journal_entries(
        self, filters: Dict[str, Any], limit: int = 1000, cursor: Optional[str] = None
    ) -> List[JournalEntry]:
    ...
    async def get_log_statistics(
        self, service: Optional[str] = None, time_range: str = "1 hour"
    ) -> Dict[str, Any]:
        ...
        cmd_parts.extend(["--since", time_range, "-o", "json"])
    ...
    async def _parse_json_log_line(self, line: str) -> Optional[LogEntry]:
        ...
        try:
            ...
            # Extract timestamp
            timestamp_str = data.get("SYSLOG_TIMESTAMP") or data.get(
