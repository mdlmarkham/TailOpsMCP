...
class ServiceConnector(RemoteAgentConnector):
        ...
        try:
            ...
            if active_since_str:
                try:
                    ...
                    boot_time_result = await self.execute_command(
                        "date -d @$(($(date +%s%6N) - $(cat /proc/stat/btime))) +%s%N",
                        timeout=5,
                    )
    ...
    async def list_services(
        ...
        try:
            ...
            try:
                ...
                services = await self._parse_service_list_plain(
                    result.stdout, filter_state
                )
    ...
    async def get_service_dependencies(self, service: str) -> ServiceDependency:
        ...
        try:
            ...
            if result.exit_code != 0:
                raise SystemManagerError(
                    f"Failed to get service dependencies: {result.stderr}"
                )
    ...
    async def _parse_service_list_plain(
        self, output: str, filter_state: Optional[str]
    ) -> List[ServiceInfo]:
