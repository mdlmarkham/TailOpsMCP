...
from src.services.connection_manager import SSHConnection
...
class RemoteAgentConnector(ABC):
    ...
    async def health_check(self) -> HealthStatus:
        ...
        try:
            ...
            result = await self.connection.execute_command("echo 'health_check'", timeout=10)
    ...
    def _sanitize_file_path(self, path: str) -> str:
        ...
        for prefix in sensitive_prefixes:
            if sanitized.startswith(prefix) and not self._is_allowed_sensitive_path(sanitized):
                raise SecurityError(f"Access to sensitive path not allowed: {sanitized}")
