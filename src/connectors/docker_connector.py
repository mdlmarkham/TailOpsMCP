...
from src.connectors.remote_agent_connector import (
    RemoteAgentConnector, DockerContainer, ContainerStats, OperationResult
)
...
class DockerConnector(RemoteAgentConnector):
    ...
    async def get_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.execute_command("docker version --format json", timeout=10)
            if result.exit_code != 0:
                return {
                    "available": True,
                    "permissions": "limited",
    ...
    async def _ensure_port_forwarding(self) -> bool:
        ...
        try:
            ...
            # Test connection
            test_cmd = f"DOCKER_HOST=tcp://localhost:{self._local_docker_port} docker info"
    ...
    async def get_container_logs(self, 
        ...
        try:
            ...
            if result.exit_code != 0:
                raise SystemManagerError(f"Failed to get container logs: {result.stderr}")
    ...
    async def restart_container(self, container_id: str, timeout: int = 30) -> OperationResult:
    ...
    async def start_container(self, container_id: str, timeout: int = 30) -> OperationResult:
    ...
    async def stop_container(self, container_id: str, timeout: int = 30) -> OperationResult:
    ...
    async def get_container_stats(self, container_id: str) -> ContainerStats:
        ...
        try:
            ...
            if result.exit_code != 0:
                raise SystemManagerError(f"Failed to get container stats: {result.stderr}")
            ...
            block_read = int(block_io.get('io_service_bytes_recursive', [{}])[0].get('value', 0))
            block_write = int(block_io.get('io_service_bytes_recursive', [{}])[1].get('value', 0))
            
            return ContainerStats(
                container_id=container_id,
                cpu_usage=float(stats_data.get('cpu_percentage', '0').rstrip('%')),
                memory_usage=int(stats_data.get('mem_usage', '0').rstrip('MiB')) * 1024 * 1024,
                memory_limit=int(stats_data.get('mem_limit', '0').rstrip('MiB')) * 1024 * 1024,
    ...
    async def get_container_resource_stats(self, container_id: str) -> ContainerResourceStats:
        ...
        try:
            ...
            logger.error(f"Failed to get resource stats for container {container_id}: {str(e)}")
            raise
    
    async def execute_container_command(self, 
                                       container_id: str, 
                                       command: str,
                                       timeout: int = 30) -> str:
        ...
        try:
            ...
            if result.exit_code != 0:
                raise SystemManagerError(f"Failed to execute command in container: {result.stderr}")
