...
from src.models.fleet_inventory import FleetInventory, Node, Event, EventType, EventSeverity
...
def register_tools(mcp: FastMCP):
    ...
    async def fleet_discover(
        ...
        try:
            ...
            if not is_gateway_mode():
                return format_error("fleet_discover", "Operation only available in gateway mode")
    ...
    async def fleet_inventory_get(
        ...
        try:
            # Check if in gateway mode
            if not is_gateway_mode():
                return format_error("fleet_inventory_get", "Operation only available in gateway mode")
            ...
            if not inventory:
                return format_error("fleet_inventory_get", "No inventory data available")
    ...
    async def fleet_node_health(
        ...
        try:
            # Check if in gateway mode
            if not is_gateway_mode():
                return format_error("fleet_node_health", "Operation only available in gateway mode")
    ...
    async def fleet_operation_plan(
        ...
        try:
            # Check if in gateway mode
            if not is_gateway_mode():
                return format_error("fleet_operation_plan", "Operation only available in gateway mode")
            
            # Validate operation name
            valid_operations = ["update_packages", "restart_service", "docker_compose_pull_up", 
                              "snapshot_or_backup", "restore"]
            if op_name not in valid_operations:
                return format_error("fleet_operation_plan", 
            ...
            plan = {
                ...
                "status": "planned",
                "estimated_impact": _estimate_operation_impact(op_name, targets, parameters)
            }
    ...
    async def fleet_operation_execute(
        plan_id: str
    ) -> dict:
        ...
        try:
            # Check if in gateway mode
            if not is_gateway_mode():
                return format_error("fleet_operation_execute", "Operation only available in gateway mode")
            
            # Retrieve operation plan
            plan = _retrieve_operation_plan(plan_id)
            if not plan:
                return format_error("fleet_operation_execute", f"Plan {plan_id} not found")
            ...
            return format_response("fleet_operation_execute", {
                "plan_id": plan_id,
                "status": "executed",
                "results": results
            })
...
def _calculate_node_health(node: Node, recent_events: List[Event]) -> str:
    ...
    error_events = [e for e in recent_events if e.severity in [EventSeverity.ERROR, EventSeverity.CRITICAL]]
    if error_events:
        return "degraded"
...
def _estimate_operation_impact(op_name: str, targets: List[str], parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for target in plan["targets"]:
        try:
            ...
            if op_name == "update_packages":
                ...
                result = await _execute_docker_compose_pull_up(executor, plan["parameters"])
            elif op_name == "snapshot_or_backup":
...
async def _execute_update_packages(executor, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return result.to_dict() if hasattr(result, 'to_dict') else {"success": result.success, "output": result.output}


async def _execute_restart_service(executor, parameters: Dict[str, Any]) -> Dict[str, Any]:
...
async def _execute_docker_compose_pull_up(executor, parameters: Dict[str, Any]) -> Dict[str, Any]:
...
async def _execute_snapshot_or_backup(executor, parameters: Dict[str, Any]) -> Dict[str, Any]:
