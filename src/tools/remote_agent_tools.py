...
class RemoteAgentTools:
    ...
    async def _get_target_connection(self, target_id: str) -> TargetConnection:
        ...
        return TargetConnection(
            executor="ssh", host=host, port=port, username="root", timeout=30
        )
    ...
    async def follow_service_logs(
        self, target: str, service: str, timeout: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to start log following for {service} on {target}: {str(e)}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    async def restart_remote_service(
        self, target: str, service: str, timeout: int = 60
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to restart service {service} on {target}: {str(e)}"
            )
        ...
        try:
            ...
            return {
                ...
                "status": {
                    "name": status.name,
                    "state": status.state,
                    "active_since": status.active_since.isoformat()
            ...
            self.logger.error(
                f"Failed to get service status for {service} on {target}: {str(e)}"
            )
    ...
    async def list_remote_services(
        self, target: str, filter_state: Optional[str] = None
    ) -> Dict[str, Any]:
                ...
                service_list.append(
                    {
                        ...
                        "active_since": service.active_since.isoformat()
                        if service.active_since
                        else None,
    ...
    async def get_remote_docker_containers(
        self, target: str, all_containers: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get Docker containers from {target}: {str(e)}"
            )
    ...
    async def get_container_logs_remote(
        ...
        try:
            ...
            logs = await connector.get_container_logs(
                container_id=container_id, lines=lines, since=since
            )
            ...
            self.logger.error(
                f"Failed to get logs for container {container_id} on {target}: {str(e)}"
            )
    ...
    async def restart_remote_container(
        self, target: str, container_id: str, timeout: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to restart container {container_id} on {target}: {str(e)}"
            )
    ...
    async def read_remote_file(
        self, target: str, path: str, max_size: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    async def write_remote_file(
        self, target: str, path: str, content: str, create_backup: bool = True
    ) -> Dict[str, Any]:
    ...
    async def list_remote_directory(
        self, target: str, path: str, include_hidden: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for cmd, info_type in commands:
                try:
                    ...
                    system_info[info_type] = {"success": False, "error": str(e)}
    ...
    async def analyze_service_logs_across_fleet(
        self, targets: List[str], service: str, time_range: str = "1 hour"
    ) -> Dict[str, Any]:
            ...
            for target in targets:
                try:
                    result = await self.get_journald_logs(
                        target=target, service=service, since=time_range
                    )
                    ...
                    target_results[target] = {"success": False, "error": str(e)}

            # Analyze aggregated logs
            analysis = {
                "total_targets": len(targets),
                "successful_targets": sum(
                    1 for r in target_results.values() if r["success"]
                ),
    ...
    async def check_fleet_service_health(
        self, targets: List[str], service: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for target in targets:
                try:
                    ...
                    health_report.append(
                        {"target": target, "service": service, "error": str(e)}
                    )

            # Calculate summary
            healthy_count = sum(
                1 for report in health_report if report.get("healthy", False)
            )
...
async def follow_service_logs(
    target: str, service: str, timeout: int = 30
) -> Dict[str, Any]:
    """MCP tool: Follow service logs in real-time."""
    return await remote_agent_tools.follow_service_logs(
        target=target, service=service, timeout=timeout
    )


async def restart_remote_service(
    target: str, service: str, timeout: int = 60
) -> Dict[str, Any]:
    """MCP tool: Restart service on remote target."""
    return await remote_agent_tools.restart_remote_service(
        target=target, service=service, timeout=timeout
    )
...
async def list_remote_services(target: str, filter_state: str = None) -> Dict[str, Any]:
    ...
    return await remote_agent_tools.list_remote_services(
        target=target, filter_state=filter_state
    )


async def get_remote_docker_containers(
    target: str, all_containers: bool = False
) -> Dict[str, Any]:
    """MCP tool: Get Docker containers from remote target."""
    return await remote_agent_tools.get_remote_docker_containers(
        target=target, all_containers=all_containers
    )


async def get_container_logs_remote(
    target: str, container_id: str, lines: int = 100, since: str = None
) -> Dict[str, Any]:
...
async def restart_remote_container(
    target: str, container_id: str, timeout: int = 30
) -> Dict[str, Any]:
...
async def read_remote_file(
    target: str, path: str, max_size: int = None
) -> Dict[str, Any]:
    """MCP tool: Read file from remote target."""
    return await remote_agent_tools.read_remote_file(
        target=target, path=path, max_size=max_size
    )


async def write_remote_file(
    target: str, path: str, content: str, create_backup: bool = True
) -> Dict[str, Any]:
...
async def list_remote_directory(
    target: str, path: str, include_hidden: bool = False
) -> Dict[str, Any]:
...
async def analyze_service_logs_across_fleet(
    targets: List[str], service: str, time_range: str = "1 hour"
) -> Dict[str, Any]:
...
async def check_fleet_service_health(
    targets: List[str], service: str
) -> Dict[str, Any]:
    """MCP tool: Check service health across multiple targets."""
    return await remote_agent_tools.check_fleet_service_health(
        targets=targets, service=service
    )
