...
from src.services.workflow_blueprints import (
    EnvironmentProvisioningWorkflow, BackupOrchestrationWorkflow,
    SafeUpgradeWorkflow, DisasterRecoveryWorkflow, SecurityComplianceWorkflow,
    MonitoringSetupWorkflow,
    create_production_environment_workflow, create_daily_backup_workflow,
    create_rolling_upgrade_workflow, create_iso27001_compliance_workflow
)
...
class WorkflowManagementTools:
    ...
    def __init__(self, workflow_engine: WorkflowEngine, 
                 scheduler: WorkflowScheduler, approval_system: ApprovalSystem,
        ...
        try:
            ...
            for blueprint_name, blueprint in self._available_blueprints.items():
                workflows.append({
                    ...
                    "estimated_duration_minutes": blueprint.estimated_duration.total_seconds() / 60 if blueprint.estimated_duration else None,
        ...
        try:
            ...
            for approval in blueprint.approvals:
                details["approvals"].append({
                    ...
                    "timeout_hours": approval.timeout.total_seconds() / 3600 if approval.timeout else None
                })
    ...
    def execute_workflow(self, workflow_name: str, parameters: Dict[str, Any],
                        created_by: str = "", approvals: Dict[str, str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Validate parameters
            validation_result = self.workflow_engine.validate_workflow_prerequisites(blueprint, parameters)
    ...
    def list_workflow_executions(self, workflow_name: Optional[str] = None, 
    ...
    def schedule_workflow(self, workflow_name: str, schedule: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def list_scheduled_workflows(self, workflow_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def request_workflow_approval(self, execution_id: str, step_id: str, 
                                approver: str, comment: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Request approval
            approval_request = self.approval_system.request_approval(execution_instance, step)
    ...
    def approve_workflow_step(self, approval_id: str, approver: str, 
        ...
        try:
            ...
            if not success:
                return {
                    "success": False,
                    "error": f"Failed to approve step - approval may not exist or approver not authorized"
                }
    ...
    def reject_workflow_step(self, approval_id: str, approver: str, 
        ...
        try:
            ...
            if not success:
                return {
                    "success": False,
                    "error": f"Failed to reject step - approval may not exist or approver not authorized"
                }
        ...
        try:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "total_execution_time_minutes": metrics.total_execution_time.total_seconds() / 60 if metrics.total_execution_time else None,
...
def register_tools(mcp: FastMCP, workflow_tools: WorkflowManagementTools):
    ...
    async def execute_workflow(workflow_name: str, parameters: dict, 
                             created_by: str = "", approvals: dict = None) -> dict:
        ...
        """
        try:
            return workflow_tools.execute_workflow(workflow_name, parameters, created_by, approvals)
        except Exception as e:
    ...
    async def list_workflow_executions(workflow_name: str = None, limit: int = 50) -> dict:
    ...
    async def request_workflow_approval(execution_id: str, step_id: str, 
                                      approver: str, comment: str = None) -> dict:
        ...
        """
        try:
            return workflow_tools.request_workflow_approval(execution_id, step_id, approver, comment)
        except Exception as e:
    ...
    async def approve_workflow_step(approval_id: str, approver: str, 
    ...
    async def reject_workflow_step(approval_id: str, approver: str, 
                                 reason: str) -> dict:
