...
def register_tools(mcp: FastMCP):
    ...
    async def get_system_status(
        ...
        try:
            ...
            await policy_gate.authorize(
                operation="get_system_status", target=target, tier=OperationTier.OBSERVE
            )
            ...
            result = await executor.execute(
                command="system_status", parameters={}, timeout=30
            )
    ...
    async def get_top_processes(
        ...
        try:
            ...
            policy_gate = deps.policy_gate
            await policy_gate.authorize(
                operation="get_top_processes", target=target, tier=OperationTier.OBSERVE
            )
    ...
    async def restart_service(
        service: str, target: str = "local", dry_run: bool = False
    ) -> dict:
        ...
        try:
            ...
            result = await executor.execute(
                command="restart_service", parameters={"service": service}, timeout=60
            )
            ...
            audit.log_operation(
                operation="restart_service", target=target, success=False, error=str(e)
            )
    ...
    async def health_check(target: str = "local") -> dict:
        ...
        try:
            ...
            policy_gate = deps.policy_gate
            await policy_gate.authorize(
                operation="health_check", target=target, tier=OperationTier.OBSERVE
            )
            ...
            result = await executor.execute(
                command="health_check", parameters={}, timeout=10
            )
            ...
            audit.log_operation(
                operation="health_check", target=target, success=False, error=str(e)
            )
            return {
                "status": "error",
                "target": target,
    ...
    logger.info(
        "Registered 5 system monitoring tools with capability-driven operations"
    )
