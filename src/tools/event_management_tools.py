...
from src.models.event_models import (
    SystemEvent, EventType, EventSeverity, EventSource, EventCategory, EventStatus, EventFilters
)
...
from src.services.event_alerting import get_event_alerting, create_default_alert_rules
from src.services.event_reporting import (
    get_event_reporting, generate_comprehensive_report, 
    TimeRange, HealthReport, SecurityReport, OperationalReport, ComplianceReport
)
from src.services.event_processor import get_event_stream_processor, create_default_filter_rules
...
class EventManagementTools:
    ...
    async def get_recent_events(self, hours: int = 24, event_type: str = None) -> Dict[str, Any]:
    ...
    async def get_events_by_source(self, source: str, hours: int = 24) -> Dict[str, Any]:
    ...
    async def get_events_by_target(self, target: str, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            health_scores = [event.health_score for event in events if event.health_score is not None]
            health_stats = {}
            if health_scores:
                health_stats = {
    ...
    async def get_health_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(health_events) >= 5:
                ...
                health_trends = [trend.__dict__ for trend in trends if 'health' in trend.name.lower()]
            ...
            return {
                ...
                "critical_issues": [event.to_dict() for event in health_report.critical_issues[:10]],
                "recent_failures": [event.to_dict() for event in health_report.recent_failures[:10]],
    ...
    async def get_error_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            error_stats = {
                ...
                "critical_errors": len([e for e in error_events if e.severity == EventSeverity.CRITICAL]),
                "error_events": len([e for e in error_events if e.severity == EventSeverity.ERROR])
            }
            ...
            recent_errors = error_events[-20:] if len(error_events) > 20 else error_events
    ...
    async def get_security_events(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(security_events) >= 5:
                ...
                security_insights = [insight.__dict__ for insight in insights if insight.insight_type == 'security']
            ...
            return {
                ...
                "critical_issues": [event.to_dict() for event in security_report.critical_security_issues[:10]],
                "policy_violations": [event.to_dict() for event in security_report.policy_violation_events[:10]],
    ...
    async def get_operation_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(operational_events) >= 5:
                insights = await self.event_analyzer.generate_insights(operational_events)
                performance_insights = [insight.__dict__ for insight in insights if insight.insight_type == 'performance']
            ...
            return {
                ...
                "operational_summary": {
                    ...
                    "critical_issues_count": len(operational_report.critical_operational_issues)
                },
                "operational_events": operational_events_data,
                "performance_insights": performance_insights,
                "critical_issues": [event.to_dict() for event in operational_report.critical_operational_issues[:10]],
    ...
    async def get_event_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            trend_summary = {
                ...
                "increasing_trends": len([t for t in trends_data if t['direction'] == 'increasing']),
                "decreasing_trends": len([t for t in trends_data if t['direction'] == 'decreasing']),
                "high_confidence_trends": len([t for t in trends_data if t['confidence'] > 0.7]),
        ...
        try:
            ...
            for alert in alerts:
                alert_dict = {
                    ...
                    "tags": alert.tags,
                    "triggering_events": [event.to_dict() for event in alert.triggering_events[:5]]  # Limit to first 5
                }
    ...
    async def acknowledge_alert(self, alert_id: str, user_id: str, note: str = None) -> Dict[str, Any]:
        """Acknowledge an alert."""
        try:
            success = await self.event_alerting.acknowledge_alert(alert_id, user_id, note)
    ...
    async def resolve_alert(self, alert_id: str, user_id: str, note: str = None) -> Dict[str, Any]:
        """Resolve an alert."""
        try:
    ...
    async def export_report(self, report_type: str, time_range: str = "24h", format: str = "json") -> Dict[str, Any]:
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if event_stats.total_events > 0:
                error_rate = (event_stats.error_events + event_stats.critical_events) / event_stats.total_events
            ...
            return {
                ...
                "component_status": {
                    ...
                    "event_streaming": "active" if buffer_status.get('websocket_clients', 0) > 0 else "inactive"
                }
            }
...
# MCP Tool Functions
async def mcp_get_recent_events(hours: int = 24, event_type: str = None) -> Dict[str, Any]:
    """Get recent events from the last N hours."""
...
async def mcp_acknowledge_alert(alert_id: str, user_id: str, note: str = None) -> Dict[str, Any]:
    """Acknowledge an alert."""
    tools = get_event_management_tools()
    return await tools.acknowledge_alert(alert_id, user_id, note)


async def mcp_resolve_alert(alert_id: str, user_id: str, note: str = None) -> Dict[str, Any]:
    """Resolve an alert."""
    tools = get_event_management_tools()
...
async def mcp_export_report(report_type: str, time_range: str = "24h", format: str = "json") -> Dict[str, Any]:
    """Export a specific report."""
    tools = get_event_management_tools()
