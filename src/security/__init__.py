...
def security_report(
    target_path: str = ".", output_path: Optional[str] = None
) -> Dict[str, Any]:
    ...
    comprehensive_report = {
        ...
        "executive_summary": {
            ...
            "security_health_score": _calculate_security_score(
                scan_results, compliance_results, dashboard_data
            ),
        },
...
def _generate_security_recommendations(
    ...
    if critical_vulns:
        recommendations.append(
            "IMMEDIATE: Address all critical security vulnerabilities"
        )
    ...
    if high_vulns:
        recommendations.append(
            "HIGH PRIORITY: Fix high-severity vulnerabilities within 7 days"
        )
    ...
    if total_secrets > 0:
        recommendations.append(
            "URGENT: Remove all exposed secrets and rotate credentials"
        )
    ...
    if failed_compliance:
        recommendations.append(
            "Address compliance violations to meet security standards"
        )
    ...
    if active_alerts > 0:
        recommendations.append(
            f"Investigate and resolve {active_alerts} active security alerts"
        )
