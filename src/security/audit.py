...
class AuditConfig:
    ...
    exclude_fields: Set[str] = field(
        default_factory=lambda: {"password", "token", "secret", "key", "private_key"}
    )
...
class AuditLogger:
    ...
    def _start_processing_threads(self) -> None:
        ...
        for i in range(self.config.processing_threads):
            thread = threading.Thread(
                target=self._process_event_buffer, name=f"AuditProcessor-{i}"
            )
            thread.daemon = True
            thread.start()
    ...
    def log_compliance_event(
        self, requirement: str, compliance_status: str, details: Dict[str, Any] = None
    ) -> None:
        ...
        for log_file in log_files:
            try:
                ...
                if log_file.stat().st_mtime < cutoff_date.timestamp():
                    if self.config.compress_backups and not log_file.name.endswith(
                        ".gz"
                    ):
...
def log_access(
    user_id: str, action: str, resource: str, outcome: str, **kwargs
) -> None:
    """Log access event."""
...
def log_security_event(
    ...
    get_audit_logger().log_security_event(
        event_type, severity_enum, description, **kwargs
    )
