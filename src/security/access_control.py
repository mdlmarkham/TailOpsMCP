...
from ..models.security_models import (
    SecurityPermission, SecurityRole, SecurityUser, 
    SecurityPolicy, AccessControlResult
)
...
class AccessControlEngine:
    ...
    def remove_capability_from_role(self, role_name: str, capability_name: str) -> bool:
        ...
        with self._lock:
            if (role_name in self._role_capabilities and 
                capability_name in self._role_capabilities[role_name]):
                self._role_capabilities[role_name].remove(capability_name)
                logger.info(f"Removed capability {capability_name} from role {role_name}")
                return True
    ...
    def get_user_access_level(self, user_id: str, resource_type: ResourceType) -> AccessLevel:
    ...
    def can_perform_action(self, user_id: str, action: PermissionType, 
                          resource_type: ResourceType, resource_id: str = None) -> bool:
        """Check if user can perform specific action."""
        capabilities = self.get_user_capabilities(user_id)
        
        for capability in capabilities:
            if (capability.permission_type == action and 
                resource_type in capability.resource_types):
    ...
    def _evaluate_access(self, request: AccessRequest) -> AccessDecision:
        ...
        try:
            ...
            user_capabilities = self.get_user_capabilities(request.context.user_id or "")
            
            # Check if user has required capability
            required_capability = None
            for capability in user_capabilities:
                if (capability.permission_type == request.action and 
                    request.resource_type in capability.resource_types):
                    
                    # Check capability conditions
                    if self._check_capability_conditions(capability, 
            ...
            policy_violations = self._check_policy_constraints(request, user_capabilities)
            ...
            # Make final decision
            allowed = self._make_final_decision(required_capability, request, risk_score)
    ...
    def _check_capability_conditions(self, capability: Capability, 
    ...
    def _assess_contextual_risk(self, request: AccessRequest) -> Tuple[float, List[str]]:
    ...
    def _check_policy_constraints(self, request: AccessRequest, 
                                capabilities: Set[Capability]) -> List[str]:
        ...
        if request.resource_type == ResourceType.AUDIT:
            ...
            if not any(cap.access_level >= AccessLevel.ELEVATED for cap in capabilities):
    ...
    def _make_final_decision(self, capability: Capability, request: AccessRequest, 
        ...
        if capability.conditions.get("time_restrictions", {}).get("restricted_hours"):
            ...
            restricted_hours = capability.conditions["time_restrictions"]["restricted_hours"]
...
def check_access(user_id: str, action: str, resource_type: str, resource_id: str = None) -> bool:
    """Quick access check."""
    ...
    try:
        ...
        return engine.can_perform_action(user_id, permission_type, resource_enum, resource_id)
