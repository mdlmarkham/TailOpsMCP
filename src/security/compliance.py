...
from ..models.security_models import (
    ComplianceStatus, ComplianceRequirement, ComplianceFramework,
    SecurityControl, ComplianceViolation, ComplianceReport
)
...
class ComplianceChecker:
    ...
    def check_compliance(self, target_path: str, framework: Optional[str] = None,
                        categories: Optional[Set[ComplianceCategory]] = None) -> List[ComplianceResult]:
    ...
    def _select_checks(self, framework: Optional[str] = None,
                      categories: Optional[Set[ComplianceCategory]] = None) -> List[ComplianceCheck]:
    ...
    def _execute_check(self, check: ComplianceCheck, target_path: str) -> ComplianceResult:
    ...
    def _check_config_compliance(self, check: ComplianceCheck, target_path: str, result: ComplianceResult) -> None:
            ...
            try:
                ...
                for pattern in check.forbidden_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        violations.append(f"Found forbidden pattern '{pattern}' in {config_file}")
                    ...
                    if key in content:
                        if isinstance(expected_value, str) and expected_value.startswith('>'):
                            ...
                            if actual_value and actual_value <= threshold:
                                violations.append(f"Value for {key} ({actual_value}) below required threshold ({threshold})")
                        elif expected_value is True and expected_value not in content.lower():
                            violations.append(f"Required configuration {key} not found or disabled")
                        elif expected_value is False and expected_value in content.lower():
                            violations.append(f"Configuration {key} should be disabled but is enabled")
                    else:
    ...
    def _check_file_compliance(self, check: ComplianceCheck, target_path: str, result: ComplianceResult) -> None:
    ...
    def _check_code_compliance(self, check: ComplianceCheck, target_path: str, result: ComplianceResult) -> None:
            ...
            try:
                ...
                for pattern in check.forbidden_patterns:
                    ...
                    for match in matches:
                        ...
                        violations.append(f"Found forbidden pattern '{pattern}' in {code_file}:{line_number}")
    ...
    def _check_process_compliance(self, check: ComplianceCheck, target_path: str, result: ComplianceResult) -> None:
        ...
        try:
            ...
            if "monitoring" in check.name.lower():
                ...
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        if any(term in proc.info['name'].lower() for term in ['audit', 'monitor', 'security']):
                ...
                if not monitoring_processes:
                    ...
                    evidence.append(f"Found monitoring processes: {', '.join(monitoring_processes)}")
            ...
            # Check for vulnerability scanning processes
                ...
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        if any(term in proc.info['name'].lower() for term in ['nmap', 'nessus', 'openvas', 'scanner']):
                ...
                if not vuln_processes:
                    ...
                    evidence.append(f"Found vulnerability scanning processes: {', '.join(vuln_processes)}")
        ...
        if "A02-2021" in file_path or "crypto" in file_path.lower():
            ...
            for algo in weak_algos:
                if re.search(rf"\b{algo}\b", content, re.IGNORECASE):
                    violations.append(f"Weak cryptographic algorithm '{algo}' in {file_path}")
    ...
    def _find_code_files(self, target_path: str, patterns: List[str]) -> List[str]:
        ...
        code_extensions = {'.py', '.js', '.ts', '.java', '.php', '.cpp', '.c', '.go', '.rs'}
        files = []
        target_path = Path(target_path)
    ...
    def generate_compliance_report(self, results: List[ComplianceResult], 
        ...
        report_data = {
            ...
            "summary": {
                ...
                "failed": len([r for r in results if not r.passed and r.status != ComplianceStatus.ERROR]),
                "errors": len([r for r in results if r.status == ComplianceStatus.ERROR]),
                "compliance_rate": len([r for r in results if r.passed]) / len(results) if results else 0
            },
        ...
        for framework in set(r.framework.name for r in results):
            ...
            report_data["compliance_by_framework"][framework] = {
                ...
                "compliance_rate": len([r for r in framework_results if r.passed]) / len(framework_results)
            }
        ...
        for category in set(r.category for r in results):
            ...
            report_data["compliance_by_category"][category.value] = {
                ...
                "compliance_rate": len([r for r in category_results if r.passed]) / len(category_results)
            }
    ...
    def _generate_recommendations(self, results: List[ComplianceResult]) -> List[str]:
        ...
        critical_violations = [r for r in results if r.severity == ComplianceSeverity.CRITICAL and not r.passed]
        if critical_violations:
            recommendations.append("URGENT: Address all critical compliance violations immediately")
        
        # Analyze high severity violations
        high_violations = [r for r in results if r.severity == ComplianceSeverity.HIGH and not r.passed]
        if high_violations:
            recommendations.append("HIGH PRIORITY: Address high-severity compliance issues within 7 days")
        ...
        if cis_results:
            ...
            if failed_cis:
                recommendations.append("Implement CIS benchmark controls for security hardening")
        
        owasp_results = [r for r in results if r.framework.name == "OWASP_Top10"]
        if owasp_results:
            failed_owasp = [r for r in owasp_results if not r.passed]
            if failed_owasp:
                recommendations.append("Address OWASP Top 10 security risks to prevent common attacks")
...
def quick_compliance_scan(target_path: str = ".", framework: str = "CIS_Benchmarks") -> Dict[str, Any]:
