...
from .access_control import AccessControlEngine, AccessRequest, AccessDecision
from ..models.security_models import (
    SecurityEvent, SecurityAlert, SecurityMetric, SecurityDashboard,
...
class SecurityMonitor:
    ...
    def _start_monitoring_threads(self) -> None:
        ...
        realtime_thread = threading.Thread(
            target=self._process_realtime_events,
            name="SecurityRealtimeMonitor"
        )
        ...
        rules_thread = threading.Thread(
            target=self._evaluate_rules,
            name="SecurityRulesEvaluator"
        )
    ...
    def record_metric(self, metric: SecurityMetric) -> None:
        ...
        if metric.severity and metric.severity in [AlertSeverity.HIGH, AlertSeverity.CRITICAL]:
            logger.warning(f"Security metric recorded: {metric.metric_name} = {metric.value}")
    ...
    def create_alert(self, rule: MonitoringRule, context: Dict[str, Any]) -> SecurityAlert:
        ...
        # Log alert creation
        logger.warning(f"Security alert created: {alert.title} (Severity: {alert.severity.value})")
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        active_alerts = [a for a in all_alerts if a.status not in [AlertStatus.RESOLVED, AlertStatus.DISMISSED]]
        ...
        successful_logins = len([m for m in recent_metrics.get("successful_logins", [])])
        security_violations = len([m for m in recent_metrics.get("security_violations", [])])
    ...
    def _process_single_event(self, event: AuditEvent) -> None:
        ...
        if event.event_type.value == "authentication":
            if event.outcome == "failure":
                self.record_metric(SecurityMetric(
    ...
    def _collect_application_metrics(self) -> None:
        ...
        try:
            ...
            active_alerts = len([a for a in self._alerts.values() if a.status not in [AlertStatus.RESOLVED, AlertStatus.DISMISSED]])
            self.record_metric(SecurityMetric(
        ...
        while self._monitoring_active:
            try:
                for rule in self._rules.values():
                    ...
                    if rule.monitoring_type not in [MonitoringType.SCHEDULED, MonitoringType.BATCH]:
                        continue
                    
                    # Check if it's time to evaluate
                    if (rule.last_triggered is None or 
                        datetime.now() - rule.last_triggered > timedelta(seconds=rule.check_interval)):
    ...
    def _evaluate_threshold_rule(self, rule: MonitoringRule) -> bool:
        ...
        logger.debug(f"Rule {rule.rule_id}: {count} occurrences of {metric_name} in {time_window}s (threshold: {max_count})")
    ...
    def _process_alert_escalations(self) -> None:
        ...
        for alert in self._alerts.values():
            ...
            if (alert.severity == AlertSeverity.CRITICAL and 
                time_since_creation > timedelta(hours=1) and 
            ...
            # Escalate high alerts after 4 hours
                  ...
                  time_since_creation > timedelta(hours=4) and 
                  alert.status == AlertStatus.NEW):
    ...
    def _process_alert_notifications(self) -> None:
        ...
        for alert in self._alerts.values():
            if alert.status == AlertStatus.NEW and alert.severity in [AlertSeverity.CRITICAL, AlertSeverity.HIGH]:
                logger.critical(f"NEW SECURITY ALERT [{alert.severity.value.upper()}]: {alert.title}")
    ...
    def _execute_response_actions(self, alert: SecurityAlert, actions: List[str]) -> None:
...
def create_security_alert(title: str, description: str, severity: str = "medium") -> SecurityAlert:
