...
class SecurityMiddleware:
    ...
    def __init__(self):
        ...
        self.require_auth = (
            os.getenv("SYSTEMMANAGER_REQUIRE_AUTH", "true").lower() == "true"
        )
        self.enable_approval = (
            os.getenv("SYSTEMMANAGER_ENABLE_APPROVAL", "false").lower() == "true"
        )
    ...
    def get_claims_from_context(self, **kwargs) -> Optional[TokenClaims]:
        ...
        try:
            ...
            raise SystemManagerError(
                f"Token verification failed: {e}", category=ErrorCategory.UNAUTHORIZED
            )
    ...
    def check_authorization(self, tool_name: str, claims: TokenClaims) -> None:
        ...
        if not authorized:
            ...
            raise SystemManagerError(
                f"Insufficient privileges: {reason}", category=ErrorCategory.FORBIDDEN
            )

    def check_target_capability(
        self, target_capabilities: list, required_scope: str
    ) -> bool:
    ...
    def check_approval(self, tool_name: str, args: Dict[str, Any]) -> bool:
        ...
        if not approval_webhook:
            ...
            raise SystemManagerError(
                "Operation requires approval. Configure SYSTEMMANAGER_APPROVAL_WEBHOOK or "
                "set SYSTEMMANAGER_ENABLE_APPROVAL=false to proceed (NOT RECOMMENDED for production)",
        ...
        raise SystemManagerError(
            "Approval workflow not yet implemented. Operation denied.",
            category=ErrorCategory.FORBIDDEN,
        )
