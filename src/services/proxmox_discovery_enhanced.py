...
from src.models.fleet_inventory import (
    ProxmoxHost, Node, ConnectionMethod, Runtime, NodeType, Event, EventType, EventSeverity
)
from src.models.proxmox_models import (
    ProxmoxAPICredentials, ProxmoxContainer, ProxmoxVM, ProxmoxStorage,
    ProxmoxSnapshot, ProxmoxBackup, ProxmoxNode
)
...
class ProxmoxDiscoveryEnhanced:
    ...
    async def discover_proxmox_hosts(self, 
                                   force_refresh: bool = False) -> List[ProxmoxHost]:
    ...
    async def _discover_host_via_api(self, 
                                   credentials: ProxmoxAPICredentials,
                                   force_refresh: bool = False) -> Optional[ProxmoxHost]:
        ...
        if (not force_refresh and 
            host_key in self._last_discovery and
            datetime.utcnow() - self._last_discovery[host_key] < timedelta(minutes=5)):
        ...
        try:
            ...
            if not connection_result.success:
                logger.warning(f"API connection failed for {credentials.host}: {connection_result.message}")
                return None
            ...
            host = ProxmoxHost(
                ...
                version=connection_result.data.get("version") if connection_result.data else None,
                cpu_cores=int(primary_node.maxcpu) if primary_node.maxcpu else 0,
                memory_mb=int(primary_node.maxmem // 1024 // 1024) if primary_node.maxmem else 0,
                storage_gb=int(sum(s.total for s in storage_pools if s.total) // 1024 // 1024 // 1024) if storage_pools else 0,
                tags=["api-discovered", "cluster"] + ([f"storage-{s.type.value}" for s in storage_pools[:3]]),
            ...
            if validation_errors:
                logger.error(f"Host validation failed for {credentials.host}: {validation_errors}")
                return None
    ...
    async def _discover_host_via_cli(self, 
                                   cli_client: ProxmoxCLI,
                                   force_refresh: bool = False) -> Optional[ProxmoxHost]:
        ...
        try:
            ...
            host = ProxmoxHost(
                ...
                tags=["cli-discovered", "local"] + ([f"storage-{s.type.value}" for s in storage_info[:3]]),
                is_active=True
            )
    ...
    async def discover_containers(self, host: ProxmoxHost,
                                force_refresh: bool = False) -> List[Node]:
        ...
        if (not force_refresh and 
            host_key in self._last_discovery and
            datetime.utcnow() - self._last_discovery[host_key] < timedelta(minutes=2)):
        ...
        try:
            ...
            if self.inventory_service and discovered_containers:
                await self._update_inventory_with_containers(host, discovered_containers)
            
            logger.info(f"Discovered {len(discovered_containers)} containers on {host.hostname}")
    ...
    async def _discover_containers_via_api(self, host: ProxmoxHost) -> List[Node]:
        ...
        try:
            ...
            logger.error(f"Unexpected error in API container discovery for {host.address}: {e}")
            return []
        ...
        try:
            ...
            for container in proxmox_containers:
                ...
                node = Node(
                    ...
                    disk_gb=self._extract_disk_size(config.get("rootfs", "")) if config else 10,
                ...
                if config:
                    ...
                    if config.get("hostname") and config.get("hostname") != container.name:
            ...
            logger.error(f"Unexpected error in CLI container discovery for {host.address}: {e}")
            return []
    
    async def discover_vms(self, host: ProxmoxHost,
                         force_refresh: bool = False) -> List[Node]:
    ...
    async def _discover_storage_via_api(self, host: ProxmoxHost) -> List[ProxmoxStorage]:
    ...
    async def _discover_storage_via_cli(self, host: ProxmoxHost) -> List[ProxmoxStorage]:
    ...
    async def discover_snapshots(self, node: Node) -> List[ProxmoxSnapshot]:
        ...
        try:
            ...
            api_client = None
            for client in self._api_clients.values():
                if client.credentials.host in [h.address for h in self._discovered_hosts.values()]:
                    try:
    ...
    async def discover_backups(self, host: ProxmoxHost,
    ...
    async def _discover_backups_via_api(self, host: ProxmoxHost,
                                      storage: Optional[str] = None) -> List[ProxmoxBackup]:
        ...
        try:
            ...
            for host in self._discovered_hosts.values():
                try:
                    ...
                    logger.error(f"Failed to update host {host.hostname} in inventory: {e}")
            ...
            for host_key, nodes in self._discovered_nodes.items():
                for node in nodes:
                    try:
                        ...
                        logger.error(f"Failed to update node {node.name} in inventory: {e}")
            
            logger.info(f"Updated inventory: {updated_hosts} hosts, {updated_nodes} nodes")
            return True
    ...
    async def _update_inventory_with_containers(self, host: ProxmoxHost,
    ...
    def create_discovery_event(self, source: str, message: str,
    ...
    async def get_discovery_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "discovered_nodes": sum(len(nodes) for nodes in self._discovered_nodes.values()),
            "last_discovery": {
