...
from src.connectors.remote_agent_connector import (
    SSHConnection, CommandResult, ForwardResult, 
    UploadResult, DownloadResult, HealthStatus
)
        ...
        for conn_id, conn in self.connections.items():
            try:
                if await conn.is_connected():
                    result = await conn.execute_command("echo 'pool_health_check'", timeout=5)
...
class SSHConnectionImpl:
    ...
    async def port_forward(self, local_port: int, remote_host: str, remote_port: int) -> ForwardResult:
    ...
    async def upload_file(self, local_path: str, remote_path: str) -> UploadResult:
        ...
        try:
            ...
            self.logger.error(f"File upload failed: {local_path} -> {remote_path} - {str(e)}")
    ...
    async def download_file(self, remote_path: str, local_path: str) -> DownloadResult:
        ...
        try:
            ...
            self.logger.error(f"File download failed: {remote_path} -> {local_path} - {str(e)}")
...
class RemoteConnectionManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Configuration
        self.max_connections = config.get('connection_pool', {}).get('max_connections', 50)
        self.connection_timeout = config.get('connection_pool', {}).get('connection_timeout', 30)
        self.idle_timeout = config.get('connection_pool', {}).get('idle_timeout', 300)
        self.health_check_interval = config.get('connection_pool', {}).get('health_check_interval', 60)
    ...
    async def create_connection(self, target: TargetConnection) -> SSHConnection:
        ...
        if target_id not in self.connection_pools:
            self.connection_pools[target_id] = ConnectionPool(target_id, self.max_connections)
        ...
        for target_id, pool in self.connection_pools.items():
            try:
                ...
                if not health_status.healthy:
                    self.logger.warning(f"Unhealthy connection pool for {target_id}: {health_status.issues}")
                
                # Log health metrics
                self.logger.info(f"Health check for {target_id}: "
