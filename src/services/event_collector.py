...
from src.models.event_models import (
    ...
    EventFilters, ResourceUsage, EventBuilder
)
...
class EventCollector:
    ...
    async def collect_fleet_health_events(self) -> List[SystemEvent]:
        ...
        try:
            ...
            if fleet_data and 'targets' in fleet_data:
                for target_name, target_data in fleet_data['targets'].items():
                    ...
                    if health_status != 'healthy':
                        severity = EventSeverity.ERROR if health_status == 'failed' else EventSeverity.WARNING
                    ...
                    if 'resources' in target_data:
                        ...
                        if resources.get('cpu_percent', 0) > 80:
                            events.append(self._create_resource_threshold_event(
            ...
            events.append(EventBuilder.error(
    ...
    async def collect_operation_events(self, operation_results: List[ExecutionResult]) -> List[SystemEvent]:
    ...
    async def collect_resource_events(self) -> List[SystemEvent]:
        ...
        try:
            ...
            if cpu_percent > 80:
                events.append(self._create_system_resource_event(
                    'cpu', cpu_percent, 'system'
                ))
    ...
    def _create_resource_threshold_event(self, target: str, resource_type: str, usage_percent: float) -> SystemEvent:
        ...
        event.description = f"{resource_type.capitalize()} usage at {usage_percent:.1f}% on {target}"
        event.details = {
    ...
    def _create_system_resource_event(self, resource_type: str, usage_percent: float, target: str) -> SystemEvent:
...
class EventAggregator:
    ...
    async def correlate_events(self, events: List[SystemEvent]) -> List[SystemEvent]:
        ...
        self.logger.info(f"Correlated {len(events)} events into {len(correlated_events)} unique events")
    ...
    async def detect_anomalies(self, events: List[SystemEvent]) -> List[SystemEvent]:
        ...
        try:
            ...
            for (event_type, source), count in event_counts.items():
                if count > threshold:
                    anomaly_events.append(EventBuilder.error(
                        EventSource.SYSTEM,
