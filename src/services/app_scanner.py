...
class ApplicationScanner:
    ...
    def scan(self) -> List[DetectedApp]:
        ...
        for app_type, rules in self.DETECTION_RULES.items():
            detected = self._check_application(app_type, rules, services, processes, ports)
            if detected:
                self.detected_apps.append(detected)
        
        logger.info(f"System scan complete. Detected {len(self.detected_apps)} applications.")
    ...
    def _check_application(
        ...
        logger.info(f"Detected {app_type}: confidence={confidence:.2f}, reasons={reasons}")
