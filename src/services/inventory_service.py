...
from src.models.inventory_snapshot import (
    InventorySnapshot, SnapshotManager, SnapshotType, SnapshotDiff, EntityChange, ChangeType
)
...
class InventoryService:
    ...
    async def run_full_discovery(self) -> EnhancedFleetInventory:
        ...
        try:
            ...
            enhanced_inventory = self._convert_to_enhanced_inventory(discovered_inventory)
    ...
    async def _enhance_inventory_discovery(self, inventory: EnhancedFleetInventory) -> None:
    ...
    async def _enhance_targets_with_docker_info(self, inventory: EnhancedFleetInventory) -> None:
        """Enhance targets with Docker/container information."""
        try:
            ...
            if containers_result["success"]:
                for container in containers_result["data"]:
                    ...
                    for target in inventory.targets.values():
                        if (target.container_info and 
                            target.container_info.container_id == container_id) or \
                           target.name == container_name:
                            ...
                            target.container_info.image_name = container.get("image", "")
                            target.container_info.status = container.get("status", "")
                            
                            # Update target status
                            target.status = "running" if container.get("status") == "running" else "stopped"
                            
                            # Update resource usage
                            await self._update_container_resource_usage(target, container)
    ...
    async def _update_container_resource_usage(self, target: EnhancedTarget, container: Dict[str, Any]) -> None:
        """Update container resource usage metrics."""
        try:
            # Get container stats
            stats_result = await self.docker_manager.get_container_stats(container["id"])
            ...
            if stats_result["success"]:
                ...
                target.resource_usage.network_rx_bytes = stats.get("network_rx_bytes", 0)
                target.resource_usage.network_tx_bytes = stats.get("network_tx_bytes", 0)
    ...
    async def _update_security_posture(self, inventory: EnhancedFleetInventory) -> None:
        ...
        try:
            for target in inventory.targets.values():
                ...
                target.security_posture.tls_enabled = await self._check_tls_status(target)
                target.security_posture.open_ports = await self._scan_open_ports(target)
                target.security_posture.firewall_status = await self._check_firewall_status(target)
                
                # Update security status
                if target.security_posture.tls_enabled and len(target.security_posture.open_ports) <= 5:
                    target.security_posture.security_status = SecurityStatus.SECURE
                elif target.security_posture.tls_enabled or len(target.security_posture.open_ports) <= 10:
            ...
            self.logger.warning(f"Security posture update failed for {target.name}: {e}")
    ...
    async def _create_auto_snapshot(self, reason: str, inventory: EnhancedFleetInventory) -> None:
        """Create automatic snapshot."""
        try:
            snapshot_name = f"Auto-{reason}-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}"
    ...
    async def run_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Health check completed: {len(health_results['issues'])} issues found")
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "version": "2.0.0",
            "status": "running",
            "last_discovery": self.discovery_pipeline.last_discovery.isoformat() if self.discovery_pipeline.last_discovery else None,
