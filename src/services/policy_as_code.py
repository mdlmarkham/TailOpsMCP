...
class TargetConfig:
    ...
    credential_path: Optional[str] = (
        None  # Path to credentials file, never stored in repo
    )
...
class PolicyAsCodeConfig:
    ...
    def load_targets_config(
        self, file_path: str = "targets.yaml"
    ) -> List[TargetConfig]:
            ...
            try:
                target = TargetConfig(
                    ...
                    connection_method=ConnectionMethod(
                        target_data.get("connection_method", "ssh")
                    ),
    ...
    def load_credentials_references(
        self, file_path: str = "credentials.yaml"
    ) -> Dict[str, str]:
        ...
        for cred_data in config_data.get("credentials", []):
            try:
                ...
                if os.stat(credential_path).st_mode & 0o077:
                    logger.warning(
                        f"Credential path has insecure permissions: {credential_path}"
                    )
    ...
    def get_complete_config(self) -> PolicyConfig:
        ...
        return PolicyConfig(targets=targets, rules=rules)
...
class PolicyAsCodeManager:
    ...
    def validate_operation(
        self, target_id: str, operation: str, parameters: Dict[str, Any]
    ) -> List[str]:
        ...
        if not self.is_operation_allowed(target_id, operation):
            errors.append(
                f"Operation '{operation}' not allowed for target '{target_id}'"
            )
            return errors
        ...
        for rule in self.config.rules:
            if (
                re.match(rule.target_pattern, target_id)
                and operation in rule.allowed_operations
            ):
                matching_rule = rule
                break
        ...
        if matching_rule:
            ...
            errors.extend(
                self._validate_parameters(
                    parameters, matching_rule.parameter_constraints
                )
            )
        ...
        for param_name, constraint in constraints.items():
            ...
            if constraint.get("type") == "string":
                if not isinstance(value, str):
                    ...
                    "max_length" in constraint and len(value) > constraint["max_length"]
                ):
                    errors.append(
