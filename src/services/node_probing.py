...
class NodeProbing:
    ...
    def probe_node(self, node: Node) -> Dict[str, Any]:
        ...
        successful_connection = next(
            (
                conn
                for conn, result in connection_results.items()
    ...
    def _test_tailscale_ssh(self, node: Node) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "method": "tailscale_ssh"}
    ...
    def _test_regular_ssh(self, node: Node) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": "Connection timeout", "method": "ssh"}
        except Exception as e:
            return {"success": False, "error": str(e), "method": "ssh"}

    def _collect_system_info(
        self, node: Node, connection_method: str
    ) -> Dict[str, Any]:
    ...
    def _execute_remote_command(
        self, node: Node, connection_method: str, command: str
    ) -> Dict[str, Any]:
    ...
    def _parse_uptime(self, uptime: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(parts) >= 4:
                ...
                return {"uptime": uptime_str.rstrip(","), "load_average": load_average}
        except Exception:
            pass
        ...
        try:
            ...
            for line in lines:
                if line.strip() and not line.startswith("NAMES"):
                    ...
                    if len(parts) >= 2:
                        containers.append(
                            {"name": parts[0].strip(), "status": parts[1].strip()}
                        )
        except Exception:
            pass
        return containers
                    ...
                    if line.strip():
                        ...
                        if len(parts) >= 3:
                            ...
                            service = Service(
                                ...
                                tags=[
                                    "docker",
                                    image.split("/")[0] if "/" in image else "",
                                ],
                            )
                            services.append(service)
        except Exception as e:
            ...
            if result.get("success"):
                for compose_file in result["output"].split("\n"):
                    if compose_file.strip():
                        ...
                        check_result = self._execute_remote_command(
                            node, connection_method, check_command
                        )

                        if (
                            check_result.get("success")
                            and check_result["output"].strip().isdigit()
                        ):
    ...
    def create_probe_event(
        ...
        successful_connections = [
            conn
            for conn, result in probe_result["connection_tests"].items()
            if result.get("success")
        ]
