...
from src.models.workflow_models import (
    WorkflowBlueprint, WorkflowExecution, WorkflowStep, StepResult,
    ExecutionStatus, ApprovalStatus, ValidationResult, WorkflowMetrics,
    StepType, Parameter, Prerequisite, RollbackAction, ApprovalRecord
)
from src.models.execution import ExecutionResult, ExecutionSeverity
...
class WorkflowStepError(WorkflowExecutionError):
    ...
    def __init__(self, message: str, step_id: str, execution_id: str, original_error: Exception = None):
...
class WorkflowEngine:
    ...
    def __init__(self, capability_executor: CapabilityExecutor, 
                 event_collector: EventCollector, policy_engine: PolicyEngine):
    ...
    async def execute_workflow(self, blueprint: WorkflowBlueprint, 
                             parameters: Dict[str, Any],
                             created_by: str = "",
                             approvals: Dict[str, str] = None) -> WorkflowExecution:
        """Execute a workflow blueprint."""
        execution_id = str(uuid.uuid4())
        
        try:
            # Validate workflow
            validation_result = await self.validate_workflow_prerequisites(blueprint, parameters)
            ...
            policy_validation = await self._validate_workflow_policies(blueprint, created_by)
            ...
            await self._emit_workflow_event(
                execution, "workflow_started", 
                {"blueprint_name": blueprint.name, "created_by": created_by}
            )
            
            # Start execution task
            task = asyncio.create_task(self._execute_workflow_async(execution, blueprint))
    ...
    async def validate_workflow_prerequisites(self, blueprint: WorkflowBlueprint, 
    ...
    async def execute_workflow_step(self, execution: WorkflowExecution, 
        ...
        try:
            ...
            if step.requires_approval:
                ...
                if approval_status != ApprovalStatus.APPROVED:
                    ...
                    await self._emit_workflow_event(
                        execution, "step_waiting_approval", 
                        {"step_id": step_id, "step_name": step.name}
                    )
            ...
            if not result.success:
                ...
                await self._emit_workflow_event(
                    ...
                    {"step_id": step_id, "step_name": step.name, "error": result.message}
                )
                raise WorkflowStepError(f"Step {step_id} failed: {result.message}", step_id, execution.execution_id)
            ...
            await self._emit_workflow_event(
                ...
                {"step_id": step_id, "step_name": step.name, "execution_time": result.execution_time.total_seconds()}
            )
    ...
    async def handle_workflow_failure(self, execution: WorkflowExecution, 
                                    step: WorkflowStep, error: Exception) -> bool:
        """Handle workflow failure with rollback if needed."""
        try:
            # Check if rollback should be executed
            if not execution.blueprint_id or not execution.context.get("rollback_enabled", True):
                return False
            ...
            for rollback_action in reversed(blueprint.rollback_plan.actions):
                try:
                    ...
                    logger.error(f"Rollback action {rollback_action.action_id} failed: {rollback_error}")
    ...
    async def cancel_workflow(self, execution_id: str, reason: str = "") -> bool:
        ...
        if execution:
            ...
            execution.end_time = datetime.now(timezone.utc)
            await self._emit_workflow_event(execution, "workflow_cancelled", {"reason": reason})
    ...
    async def get_workflow_metrics(self, execution_id: str) -> Optional[WorkflowMetrics]:
        ...
        return WorkflowMetrics(
            ...
            total_steps=len(execution.blueprint_id),  # This would need to be loaded from blueprint
            completed_steps=len(execution.get_completed_steps()),
            failed_steps=len(execution.get_failed_steps()),
            total_execution_time=execution.get_total_execution_time() if execution.end_time else None,
            approval_count=len(execution.approvals),
            rollback_count=1 if execution.rollback_executed else 0
        )
    
    async def _execute_workflow_async(self, execution: WorkflowExecution, blueprint: WorkflowBlueprint):
        ...
        try:
            ...
            while len(completed_steps) < len(blueprint.steps):
                ...
                if not executable_steps:
                    ...
                    if failed_steps:
                        ...
                        execution.status = ExecutionStatus.FAILED
                        execution.end_time = datetime.now(timezone.utc)
                        await self._emit_workflow_event(execution, "workflow_failed", {"failed_steps": list(failed_steps)})
                        break
                    else:
                        # Deadlock - no steps can execute
                        raise WorkflowExecutionError("Workflow deadlock detected - no executable steps found")
                ...
                for step in executable_steps:
                    try:
                        ...
                        if result.success:
                            ...
                            await self.handle_workflow_failure(execution, step, Exception(result.message))
                            return
            ...
            execution.status = ExecutionStatus.FAILED
            execution.end_time = datetime.now(timezone.utc)
            await self._emit_workflow_event(execution, "workflow_error", {"error": str(e)})
        finally:
    ...
    async def _execute_step_with_retry(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
        ...
        while attempt < max_attempts:
            try:
                ...
                # Wait before retry
                delay = min(
                    retry_policy.initial_delay.total_seconds() * (retry_policy.backoff_factor ** (attempt - 1)),
    ...
    async def _execute_step_implementation(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
        ...
        try:
            ...
            if not handler:
                raise WorkflowExecutionError(f"No handler found for step type: {step.step_type}")
    ...
    async def _handle_validation_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_resource_allocation_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_container_operations_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_service_deployment_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_network_configuration_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_health_validation_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_backup_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_restore_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_snapshot_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_upgrade_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_testing_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_configuration_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_discovery_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_transfer_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _handle_maintenance_step(self, execution: WorkflowExecution, step: WorkflowStep) -> StepResult:
    ...
    async def _execute_rollback_action(self, execution: WorkflowExecution, rollback_action: RollbackAction):
        ...
        try:
            ...
            if not rollback_result.success:
                logger.warning(f"Rollback action {rollback_action.action_id} reported failure: {rollback_result.message}")
    ...
    def _validate_parameter(self, param_def: Parameter, value: Any) -> List[str]:
        ...
        if param_def.type == "string" and not isinstance(value, str):
            ...
            errors.append(f"Parameter must be a boolean")
            ...
            errors.append(f"Parameter must be a dictionary")
    ...
    async def _validate_step_preconditions(self, execution: WorkflowExecution, step: WorkflowStep):
        """Validate step preconditions."""
        for precondition in step.preconditions:
            # Check if precondition is satisfied
            if precondition not in execution.get_completed_steps():
                raise WorkflowExecutionError(f"Precondition '{precondition}' not satisfied")
    
    async def _check_step_approval(self, execution: WorkflowExecution, step: WorkflowStep) -> ApprovalStatus:
        """Check if step has required approval."""
        # Find approval record for this step
        for approval in execution.approvals:
            if approval.step_id == step.step_id and approval.status == ApprovalStatus.APPROVED:
                return ApprovalStatus.APPROVED
        
        return ApprovalStatus.PENDING
    
    async def _apply_approvals(self, execution: WorkflowExecution, approvals: Dict[str, str]):
    ...
    async def _validate_workflow_policies(self, blueprint: WorkflowBlueprint, user: str) -> bool:
    ...
    async def _emit_workflow_event(self, execution: Optional[WorkflowExecution], 
