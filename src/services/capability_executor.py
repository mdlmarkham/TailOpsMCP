...
from src.models.policy_models import (
    CapabilityOperation, OperationType, TargetRole, PolicyContext,
    PolicyEvaluation, PolicyDecision, ParameterConstraint, create_default_policy_config
)
...
class CapabilityValidator:
    ...
    def validate_parameters(self, operation_type: OperationType, parameters: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if operation_type not in self.parameter_schemas:
            errors.append(f"No parameter schema defined for operation: {operation_type}")
            return False, errors
        ...
        for param_name, param_schema in schema.items():
            ...
            if param_name in parameters:
                ...
                # Type validation
                if not self._validate_type(param_value, param_schema.get("type", "string")):
                    errors.append(f"Parameter {param_name}: invalid type, expected {param_schema.get('type')}")
                    continue
                
                # String validations
                if param_schema.get("type") == "string":
                    if "max_length" in param_schema and len(str(param_value)) > param_schema["max_length"]:
                        errors.append(f"Parameter {param_name}: exceeds maximum length of {param_schema['max_length']}")
                ...
                # Numeric validations
                    ...
                    if "min" in param_schema and param_value < param_schema["min"]:
                        errors.append(f"Parameter {param_name}: below minimum value of {param_schema['min']}")
                    if "max" in param_schema and param_value > param_schema["max"]:
                        errors.append(f"Parameter {param_name}: exceeds maximum value of {param_schema['max']}")
                
                # List validations
                elif param_schema.get("type") == "list":
                    if "allowed_values" in param_schema and param_value not in param_schema["allowed_values"]:
                        errors.append(f"Parameter {param_name}: value not in allowed list: {param_schema['allowed_values']}")
...
class CapabilityExecutor:
    ...
    async def execute_operation(self, operation: CapabilityOperation, 
        ...
        logger.info(f"Starting capability operation: {operation.name} (ID: {correlation_id})")
        ...
        try:
            ...
            if policy_result.decision == PolicyDecision.DENY:
                return self._create_error_result(
                    ...
                    start_time, operation,
                    structured_error_details={"policy_decision": policy_result.decision.value, "reason": policy_result.reason}
                )
            ...
            if dry_run:
                result = await self._execute_dry_run(operation, target_info, policy_result)
            else:
                result = await self._execute_capability(operation, target_info, policy_result)
            ...
            logger.error(f"Unexpected error in capability execution: {e}", exc_info=True)
            return self._create_error_result(
    ...
    async def _execute_capability(self, operation: CapabilityOperation, 
                                target_info: Dict[str, Any], 
                                policy_result: PolicyEvaluation) -> ExecutionResult:
    ...
    async def _execute_dry_run(self, operation: CapabilityOperation,
                             target_info: Dict[str, Any],
                             policy_result: PolicyEvaluation) -> ExecutionResult:
    ...
    def _create_policy_context(self, operation: CapabilityOperation, 
                             target_info: Dict[str, Any]) -> PolicyContext:
    ...
    async def _audit_operation(self, operation: CapabilityOperation, 
        ...
        await self.audit_logger.log_event(
            ...
            severity=ExecutionSeverity.INFO if result.success else ExecutionSeverity.ERROR
        )
    
    def _create_error_result(self, status: ExecutionStatus, error_message: str,
                           start_time: datetime, operation: CapabilityOperation,
                           structured_error_details: Optional[Dict[str, Any]] = None) -> ExecutionResult:
        ...
        if structured_error_details:
            ...
            structured_error = StructuredError(
                ...
                context={"operation_id": operation.id, "target_id": operation.target_id}
            )
...
async def create_service_restart_operation(service_name: str, target_id: str, 
                                         requested_by: str, timeout: int = 60) -> CapabilityOperation:
...
async def create_container_operation(operation_type: OperationType, 
                                   container_name: str, target_id: str,
                                   requested_by: str, **kwargs) -> CapabilityOperation:
...
async def create_backup_operation(operation_type: OperationType, 
                                backup_id: str, target_path: str,
                                target_id: str, requested_by: str, **kwargs) -> CapabilityOperation:
