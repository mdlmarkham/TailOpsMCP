...
from src.models.workflow_models import (
    WorkflowBlueprint, WorkflowExecution, ScheduledWorkflow,
    ExecutionStatus
)
...
class WorkflowScheduler:
    ...
    async def schedule_recurring_workflow(self, blueprint: WorkflowBlueprint,
        ...
        try:
            ...
            # Start schedule task
            task = asyncio.create_task(self._schedule_workflow_execution(scheduled_workflow))
            ...
            logger.info(f"Scheduled workflow {blueprint.name} with schedule ID {schedule_id}")
            return schedule_id
    ...
    async def update_scheduled_workflow(self, schedule_id: str, 
        ...
        try:
            ...
            if schedule_expression is not None:
                ...
                timezone_obj = pytz.timezone(timezone_str or scheduled_workflow.timezone)
                next_run = timezone_obj.localize(next_run)
    ...
    async def get_scheduled_workflows(self, blueprint_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get all scheduled workflows."""
        workflows = []
        
        for scheduled_workflow in self._scheduled_workflows.values():
            if blueprint_name is None or scheduled_workflow.blueprint_name == blueprint_name:
    ...
    async def get_upcoming_executions(self, hours_ahead: int = 24) -> List[Dict[str, Any]]:
        ...
        for schedule_id, scheduled_workflow in list(self._scheduled_workflows.items()):
            ...
            if now >= next_run_utc:
                try:
                    ...
                    logger.error(f"Failed to trigger scheduled execution {schedule_id}: {e}")
    ...
    async def _trigger_scheduled_execution(self, scheduled_workflow: ScheduledWorkflow):
        ...
        try:
            ...
            if not blueprint:
                logger.error(f"Blueprint not found for scheduled workflow: {scheduled_workflow.blueprint_id}")
                return
            ...
            logger.info(f"Triggered scheduled execution for {scheduled_workflow.blueprint_name}")
...
class ScheduleManager:
    ...
    async def create_daily_schedule(self, blueprint_name: str, hour: int = 2, minute: int = 0,
                                  timezone_str: str = "UTC") -> str:
        """Create daily schedule."""
        cron_expression = f"{minute} {hour} * * *"
        return await self._create_schedule(blueprint_name, cron_expression, timezone_str)
    
    async def create_weekly_schedule(self, blueprint_name: str, day_of_week: int, hour: int = 2, 
                                   minute: int = 0, timezone_str: str = "UTC") -> str:
        """Create weekly schedule."""
        cron_expression = f"{minute} {hour} * * {day_of_week}"
        return await self._create_schedule(blueprint_name, cron_expression, timezone_str)
    
    async def create_monthly_schedule(self, blueprint_name: str, day_of_month: int, hour: int = 2,
                                    minute: int = 0, timezone_str: str = "UTC") -> str:
        """Create monthly schedule."""
        cron_expression = f"{minute} {hour} {day_of_month} * *"
        return await self._create_schedule(blueprint_name, cron_expression, timezone_str)
    
    async def create_hourly_schedule(self, blueprint_name: str, minute: int = 0,
                                   timezone_str: str = "UTC") -> str:
        """Create hourly schedule."""
        cron_expression = f"{minute} * * * *"
        return await self._create_schedule(blueprint_name, cron_expression, timezone_str)
    
    async def _create_schedule(self, blueprint_name: str, cron_expression: str,
                             timezone_str: str = "UTC") -> str:
