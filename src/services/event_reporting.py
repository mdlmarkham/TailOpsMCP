...
from src.models.event_models import (
    SystemEvent, EventType, EventSeverity, EventSource, EventCategory, EventStatistics
)
from src.services.event_store import get_event_store, get_event_index
from src.services.event_analyzer import get_event_analyzer, analyze_events_comprehensive
...
class EventReporting:
    ...
    async def generate_health_report(self, time_range: TimeRange) -> HealthReport:
        ...
        self.logger.info(f"Generating health report for {time_range.start_time} to {time_range.end_time}")
        ...
        if health_events:
            ...
            health_scores = [e.health_score for e in health_events if e.health_score is not None]
            
            if health_scores:
                health_report.fleet_health_score = sum(health_scores) / len(health_scores)
            ...
            critical_events = [e for e in health_events if e.severity in [EventSeverity.CRITICAL, EventSeverity.ERROR]]
            health_report.critical_issues = critical_events[:10]  # Top 10
            
            # Identify recent failures
            recent_failures = [e for e in health_events if e.event_type == EventType.SERVICE_STATUS]
            ...
            health_report.health_trends = [trend.__dict__ for trend in trends if 'health' in trend.name.lower()]
            ...
            if health_report.unhealthy_systems > health_report.healthy_systems:
                health_report.recommendations.append("Consider system capacity planning")
    ...
    async def generate_security_report(self, time_range: TimeRange) -> SecurityReport:
        ...
        self.logger.info(f"Generating security report for {time_range.start_time} to {time_range.end_time}")
        ...
        if security_events:
            ...
            policy_events = [e for e in security_events if e.event_type == EventType.POLICY_VIOLATION]
            security_report.policy_violations = len(policy_events)
            
            auth_events = [e for e in security_events if e.event_type == EventType.AUTHENTICATION]
            security_report.authentication_events = len(auth_events)
            
            authz_events = [e for e in security_events if e.event_type == EventType.AUTHORIZATION]
            security_report.authorization_events = len(authz_events)
            
            # Identify critical security issues
            critical_security = [e for e in security_events if e.severity == EventSeverity.CRITICAL]
            ...
            if security_report.policy_violations > 0:
                security_report.security_recommendations.append("Review and update security policies")
            
            security_report.security_events = security_events
            
            # Calculate security score
            if security_events:
                security_report.security_score = max(0, 100 - (security_report.critical_security_events * 20))
    ...
    async def generate_operational_report(self, time_range: TimeRange) -> OperationalReport:
        """Generate comprehensive operational report."""
        self.logger.info(f"Generating operational report for {time_range.start_time} to {time_range.end_time}")
        ...
        if operational_events:
            ...
            operation_events = [e for e in operational_events if e.event_type in [
                EventType.OPERATION_COMPLETED, EventType.OPERATION_FAILED
            ]]
            
            operational_report.total_operations = len(operation_events)
            successful_ops = len([e for e in operation_events if e.event_type == EventType.OPERATION_COMPLETED])
            failed_ops = len([e for e in operation_events if e.event_type == EventType.OPERATION_FAILED])
            ...
            if durations:
                operational_report.average_operation_duration = sum(durations) / len(durations)
            
            # Identify critical operational issues
            critical_ops = [e for e in operational_events if e.severity == EventSeverity.CRITICAL]
            operational_report.critical_operational_issues = critical_ops[:10]
            
            # Identify performance issues
            perf_events = [e for e in operational_events if e.event_type == EventType.RESOURCE_THRESHOLD]
            ...
            if operational_report.average_operation_duration > 30:  # 30 seconds
                operational_report.operational_recommendations.append("Optimize operation performance")
    ...
    async def generate_compliance_report(self, time_range: TimeRange) -> ComplianceReport:
        """Generate comprehensive compliance report."""
        self.logger.info(f"Generating compliance report for {time_range.start_time} to {time_range.end_time}")
        ...
        if compliance_events:
            ...
            # Categorize by event type
            policy_compliance = [e for e in compliance_events if e.event_type == EventType.POLICY_VIOLATION]
            security_compliance = [e for e in compliance_events if e.category == EventCategory.SECURITY]
            ...
            compliance_report.passed_checks = len(compliance_events) - len(policy_compliance)
            ...
            if compliance_report.total_compliance_checks > 0:
                compliance_report.compliance_rate = (
                    compliance_report.passed_checks / compliance_report.total_compliance_checks * 100
                )
    ...
    async def create_event_dashboard(self, filters: Optional[Dict[str, Any]] = None) -> Dashboard:
            ...
            try:
                if widget_type == "event_count":
                    ...
                    stats = await self.event_store.get_statistics(
                        int((time_range.end_time - time_range.start_time).total_seconds() / 3600)
                    )
                    ...
                    time_range = self._get_time_range_from_config(widget_config)
                    from src.models.event_models import EventFilters
                    filters = EventFilters(start_time=time_range.start_time, end_time=time_range.end_time)
                    ...
                    data["widgets_data"][widget["title"]] = {"severity_distribution": severity_counts}
                    ...
                    data["widgets_data"][widget["title"]] = {
                        ...
                        "alerts_by_severity": {
                            alert.severity.value: len([a for a in alerts if a.severity == alert.severity])
                            for alert in alerts
                        }
                    }
                ...
                self.logger.error(f"Error generating data for widget {widget['title']}: {e}")
    ...
    async def export_report(self, report: Any, format: str = "json", output_path: Optional[str] = None) -> str:
