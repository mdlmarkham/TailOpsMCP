...
class EventStreamProcessor:
    ...
    def _event_matches_filters(self, event: SystemEvent, filters: EventFilters) -> bool:
        ...
        if filters.min_health_score and (event.health_score is None or event.health_score < filters.min_health_score):
            return False
        
        if filters.max_health_score and (event.health_score is None or event.health_score > filters.max_health_score):
            return False
        
        if filters.search_text and filters.search_text.lower() not in event.title.lower() and filters.search_text.lower() not in event.description.lower():
            return False
    ...
    async def _process_event_batch(self) -> None:
        ...
        for hook in self.batch_hooks:
            try:
                await hook(event_batch) if asyncio.iscoroutinefunction(hook) else hook(event_batch)
            except Exception as e:
    ...
    async def _handle_websocket_client(self, websocket: websockets.WebSocketServerProtocol, path: str) -> None:
        """Handle WebSocket client connection."""
        self.websocket_clients.add(websocket)
        self.stats['websocket_clients'] = len(self.websocket_clients)
        
        self.logger.info(f"WebSocket client connected. Total clients: {self.stats['websocket_clients']}")
        ...
        try:
            ...
            self.stats['websocket_clients'] = len(self.websocket_clients)
            self.logger.info(f"WebSocket client disconnected. Total clients: {self.stats['websocket_clients']}")
    
    async def _handle_websocket_message(self, websocket: websockets.WebSocketServerProtocol, data: Dict[str, Any]) -> None:
...
class EventWebSocket:
    ...
    async def subscribe_to_events(self, client_id: str, filters: Optional[EventFilters] = None) -> AsyncIterator[SystemEvent]:
...
def get_event_stream_processor(config: Optional[EventStreamConfig] = None) -> EventStreamProcessor:
...
async def start_realtime_event_processing(config: Optional[EventStreamConfig] = None) -> EventStreamProcessor:
    """Start real-time event processing."""
    processor = get_event_stream_processor(config)
    
    # Add default filter rules
    processor.add_filter_rule(EventFilterRule(
...
def create_default_filter_rules() -> List[EventFilterRule]:
    ...
    return [
        EventFilterRule(
            name="critical_only",
            condition=lambda e: e.severity in [EventSeverity.CRITICAL, EventSeverity.ERROR],
            priority=10,
