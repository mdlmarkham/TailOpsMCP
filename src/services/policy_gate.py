...
from src.auth.scopes import Scope, check_authorization, requires_approval, get_tool_risk_level
...
class PolicyGate:
    ...
    def __init__(self, target_registry: TargetRegistry, audit_logger: AuditLogger):
        ...
        self.enable_dry_run = os.getenv("SYSTEMMANAGER_ENABLE_DRY_RUN", "true").lower() == "true"
    ...
    def _register_discovery_tools(self):
        """Register discovery tools with allowlist manager."""
        self.allowlist_manager.register_discovery_tool("list_services", self.discovery_tools.list_services)
        self.allowlist_manager.register_discovery_tool("list_containers", self.discovery_tools.list_containers)
        self.allowlist_manager.register_discovery_tool("list_stacks", self.discovery_tools.list_stacks)
        self.allowlist_manager.register_discovery_tool("list_ports", self.discovery_tools.list_ports)
    ...
    def validate_capabilities(self, target: TargetMetadata, required_capabilities: List[str]) -> None:
    ...
    async def validate_parameters(self, operation: str, parameters: Dict[str, Any],
                          constraints: Dict[str, Any], target: Optional[str] = None) -> List[str]:
        ...
        for param_name, constraint in constraints.items():
            ...
            if param_name in param_type_mapping:
                ...
                errors.extend(self._basic_parameter_validation(param_name, value, constraint))
    ...
    def _get_parameter_type_mapping(self, operation: str) -> Dict[str, ParameterType]:
        ...
        mappings = {
            "start_container": {"container": ParameterType.CONTAINER_NAME, "timeout": ParameterType.TIMEOUT},
            "stop_container": {"container": ParameterType.CONTAINER_NAME, "timeout": ParameterType.TIMEOUT},
            "restart_container": {"container": ParameterType.CONTAINER_NAME, "timeout": ParameterType.TIMEOUT},
            "inspect_container": {"container": ParameterType.CONTAINER_NAME},
            "deploy_stack": {"stack": ParameterType.STACK_NAME, "timeout": ParameterType.TIMEOUT},
            "pull_stack": {"stack": ParameterType.STACK_NAME, "timeout": ParameterType.TIMEOUT},
            "restart_stack": {"stack": ParameterType.STACK_NAME, "timeout": ParameterType.TIMEOUT},
            "restart_service": {"service": ParameterType.SERVICE_NAME, "timeout": ParameterType.TIMEOUT},
            "read_file": {"path": ParameterType.FILE_PATH},
            "list_directory": {"path": ParameterType.FILE_PATH},
            "test_connectivity": {"host": ParameterType.HOSTNAME, "port": ParameterType.PORT_NUMBER},
            "scan_ports": {"host": ParameterType.HOSTNAME, "port_range": ParameterType.PORT_NUMBER}
        }
        
        return mappings.get(operation, {})
    
    def _basic_parameter_validation(self, param_name: str, value: Any, constraint: Dict[str, Any]) -> List[str]:
        ...
        if constraint.get("type") == "string":
            ...
            if not isinstance(value, list):
                ...
                errors.append(f"Parameter {param_name} exceeds max items {constraint['max_items']}")
        
        return errors
    
    def get_matching_policy_rule(self, tool_name: str, target_id: str, 
                               operation: str) -> Optional[PolicyRule]:
    ...
    async def enforce_policy(self, tool_name: str, target_id: str, operation: str,
        ...
        try:
            ...
            if not policy_rule:
                validation_errors.append(f"No policy rule found for operation {operation} on target {target_id}")
            ...
            if policy_rule.operation_tier == OperationTier.ADMIN and not dry_run:
                if policy_rule.requires_approval and not self._check_approval(tool_name, parameters):
            ...
            if validation_errors and self.validation_mode == ValidationMode.STRICT:
                ...
                logger.warning(f"Policy validation warnings for {tool_name}: {validation_errors}")
                return True, validation_errors
    ...
    def audit_policy_decision(self, tool_name: str, target_id: str, operation: str,
                            parameters: Dict[str, Any], claims: TokenClaims,
                            authorized: bool, validation_errors: List[str],
