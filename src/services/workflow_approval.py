...
from src.models.workflow_models import (
    ...
    ApprovalStatus, ApprovalRecord, ApprovalRequirement, ExecutionStatus
)
from src.models.policy_models import PolicyContext, PolicyEvaluation, PolicyDecision
...
class ApprovalSystem:
    ...
    async def request_approval(self, execution: WorkflowExecution, 
                             step: WorkflowStep) -> ApprovalRequest:
        """Request approval for workflow step."""
        approval_id = str(uuid.uuid4())
        
        # Check if approval is already pending
        existing_approval = self._find_pending_approval(execution.execution_id, step.step_id)
    ...
    async def reject_step(self, approval_id: str, approver: str, 
                        reason: str) -> bool:
    ...
    async def get_approval_history(self, execution_id: Optional[str] = None,
                                 approver: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def _find_pending_approval(self, execution_id: str, step_id: str) -> Optional[ApprovalRequest]:
        """Find pending approval for execution and step."""
        for approval_request in self._approval_requests.values():
            if approval_request.execution_id == execution_id and approval_request.step_id == step_id:
                return approval_request
        return None
...
class WorkflowGovernance:
    ...
    async def validate_workflow_compliance(self, blueprint: WorkflowBlueprint) -> ComplianceResult:
    ...
    async def enforce_sla_requirements(self, blueprint: WorkflowBlueprint) -> List[Dict[str, Any]]:
    ...
    async def _evaluate_governance_rule(self, rule: GovernanceRule, 
            ...
            if rule.policy_type == GovernancePolicy.SECURITY_VALIDATION:
                ...
                if rule.conditions.get("require_security_check", False):
                    if not blueprint.tags or "security-validated" not in blueprint.tags:
                        violations.append(f"Security validation required for {blueprint.name}")
                ...
                if cpu_cores > max_cpu:
                    violations.append(f"CPU usage exceeds limit: {cpu_cores} > {max_cpu}")
                ...
                if rule.conditions.get("require_weekend_approval", False):
                    if blueprint.category.value in ["production", "deployment"]:
                        recommendations.append("Weekend execution requires additional approval")
    ...
    async def _validate_compliance_standard(self, standard: ComplianceStandard,
                                          blueprint: WorkflowBlueprint) -> ComplianceResult:
        ...
        if standard == ComplianceStandard.SOX:
            ...
            if blueprint.category.value in ["production", "deployment"]:
                violations.append("SOX: Production changes require approval and documentation")
            ...
            if not blueprint.rollback_plan:
                warnings.append("ISO 27001: Rollback plan recommended for production workflows")
...
class WorkflowPolicyIntegration:
    ...
    def __init__(self, policy_engine, approval_system: ApprovalSystem,
    ...
    async def get_workflow_permissions(self, user: str, 
        ...
        try:
            ...
            return {
                ...
                "view": ["workflow:view"],
                "admin": ["workflow:admin"] if user in ["admin", "operations_manager"] else []
            }
    ...
    async def enforce_workflow_policies(self, execution: WorkflowExecution) -> bool:
        ...
        try:
            ...
            compliance_result = await self.governance.validate_workflow_compliance(blueprint)
