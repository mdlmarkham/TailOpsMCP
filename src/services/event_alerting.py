...
class AlertRule:
    ...
    def evaluate(self, events: List[SystemEvent]) -> bool:
        ...
        if len(filtered_events) >= self.threshold_count:
            ...
            if self.threshold_time_window > 0 and len(filtered_events) > 1:
                time_span = max(e.timestamp for e in filtered_events) - min(e.timestamp for e in filtered_events)
    ...
    def _evaluate_condition(self, event: SystemEvent) -> bool:
        ...
        try:
            ...
            get_logger("alert_rule").error(f"Error evaluating condition for {self.name}: {e}")
            return False
...
class Alert:
    ...
    def escalate(self, new_severity: EventSeverity, level: int, additional_info: Optional[Dict[str, Any]] = None) -> None:
        """Escalate the alert."""
    ...
    def needs_escalation(self) -> bool:
        ...
        return self.status in [AlertStatus.ACTIVE, AlertStatus.ACKNOWLEDGED] and self.escalation_level >= 0
...
class NotificationService:
    ...
    async def send_notification(self, channel: NotificationChannel, alert: Alert, recipients: List[str]) -> bool:
    ...
    async def _send_teams(self, alert: Alert, recipients: List[str]) -> bool:
        ...
        try:
            ...
            response = requests.post(self.config.teams_webhook_url, json=message, timeout=10)
            return response.status_code == 200
    ...
    def _format_alert_message(self, alert: Alert) -> str:
        ...
        if alert.triggering_events:
            ...
            lines.append(f"Latest Event Time: {alert.triggering_events[-1].timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
...
class EventAlerting:
    ...
    def __init__(self, notification_config: Optional[NotificationConfig] = None):
        self.notification_config = notification_config or self._load_notification_config()
    ...
    async def _create_alert_from_rule(self, rule: AlertRule, events: List[SystemEvent]) -> Alert:
    ...
    async def _send_alert_notifications(self, alert: Alert, rule: AlertRule) -> None:
        ...
        for channel in rule.channels:
            if await self.notification_service.send_notification(channel, alert, rule.recipients):
                success_count += 1
    ...
    async def _check_escalation(self, alert: Alert, rule: AlertRule) -> None:
        ...
        for escalation_rule in rule.escalation_rules:
            if alert.created_at + timedelta(minutes=escalation_rule.delay_minutes) <= datetime.utcnow():
                # Time to escalate
                alert.escalate(
                ...
                additional_recipients = rule.recipients + escalation_rule.additional_recipients
                additional_channels = rule.channels + escalation_rule.additional_channels
                
                for channel in additional_channels:
                    await self.notification_service.send_notification(channel, alert, additional_recipients)
    ...
    async def resolve_alert(self, alert_id: str, user_id: str, note: Optional[str] = None) -> bool:
        """Resolve an alert."""
        alert = self.active_alerts.get(alert_id)
        if alert:
    ...
    async def suppress_alert(self, alert_id: str, duration_minutes: int, reason: str) -> bool:
        ...
        if alert:
            ...
            self.logger.info(f"Suppressed alert {alert_id} for {duration_minutes} minutes")
            return True
        return False
    
    async def get_active_alerts(self, severity: Optional[EventSeverity] = None) -> List[Alert]:
    ...
    async def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        active_count = len([a for a in self.active_alerts.values() if not a.is_expired()])
...
async def initialize_alerting_system() -> EventAlerting:
    ...
    get_logger("alerting_system").info(f"Initialized alerting system with {len(default_rules)} default rules")
    return alerting