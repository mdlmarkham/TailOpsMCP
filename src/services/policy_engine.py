...
from src.models.policy_models import (
    PolicyConfig, PolicyRule, PolicyContext, PolicyEvaluation, PolicyDecision,
    OperationType, TargetRole, TimeConstraint, PolicyHistory, PolicyVersion,
    create_default_policy_config, validate_policy_config
)
...
class PolicyEngine:
    ...
    async def evaluate_operation(self, operation: Any, context: PolicyContext) -> PolicyEvaluation:
        ...
        try:
            ...
            evaluation_result = self._evaluate_rules(operation, context, applicable_rules)
    ...
    def _generate_cache_key(self, operation: Any, context: PolicyContext) -> str:
        ...
        key_parts = [
            str(operation.capability) if hasattr(operation, 'capability') else str(operation),
    ...
    def _get_applicable_rules(self, operation: Any, context: PolicyContext) -> List[PolicyRule]:
    ...
    def _rule_applies(self, rule: PolicyRule, operation: Any, context: PolicyContext) -> bool:
        ...
        if rule.target_patterns and context.target_metadata:
            ...
            if not any(pattern.match(target_id) for pattern in 
                      [self.compiled_patterns.get(p, re.compile(p)) for p in rule.target_patterns]):
                return False
    ...
    def _check_time_restrictions(self, time_constraints: List[TimeConstraint], 
    ...
    def _evaluate_rules(self, operation: Any, context: PolicyContext, 
                       applicable_rules: List[PolicyRule]) -> PolicyEvaluation:
        ...
        for param_name, param_value in parameters.items():
            if param_name in constraints:
                ...
                if not self._check_parameter_type(param_value, expected_type):
                    ...
                    validation_result["errors"].append(f"Invalid type: expected {expected_type}")
                
                # String validations
                if expected_type == "string":
                    if "max_length" in constraint and len(str(param_value)) > constraint["max_length"]:
                        validation_result["valid"] = False
                        validation_result["errors"].append(f"Exceeds maximum length: {constraint['max_length']}")
                    
                    if "pattern" in constraint:
                        pattern = constraint["pattern"]
                        if not re.match(pattern, str(param_value)):
                            validation_result["valid"] = False
                            validation_result["errors"].append(f"Does not match pattern: {pattern}")
                
                # Numeric validations
                elif expected_type in ["int", "float"]:
                    if "min" in constraint and param_value < constraint["min"]:
                        validation_result["valid"] = False
                        validation_result["errors"].append(f"Below minimum value: {constraint['min']}")
                    
                    if "max" in constraint and param_value > constraint["max"]:
                        validation_result["valid"] = False
                        validation_result["errors"].append(f"Above maximum value: {constraint['max']}")
                
                # List validations
                elif expected_type == "list":
                    if "allowed_values" in constraint and param_value not in constraint["allowed_values"]:
                        validation_result["valid"] = False
                        validation_result["errors"].append(f"Not in allowed values: {constraint['allowed_values']}")
    ...
    def _apply_global_settings(self, evaluation: PolicyEvaluation, 
                             context: PolicyContext) -> PolicyEvaluation:
        ...
        if (self.current_config.enable_dry_run and 
            evaluation.decision == PolicyDecision.ALLOW and
    ...
    async def _audit_evaluation(self, operation: Any, context: PolicyContext, 
        ...
        audit_data = {
            ...
            "operation_type": str(operation.capability) if hasattr(operation, 'capability') else str(operation),
    ...
    def update_policy_config(self, new_config: PolicyConfig, changed_by: str, 
        ...
        try:
            ...
            if not validation_result.is_valid:
                logger.error(f"Policy configuration validation failed: {validation_result.errors}")
                return False
    ...
    def rollback_policy(self, history_entry_id: str, rolled_back_by: str) -> bool:
        ...
        try:
            ...
            if not history_entry or history_entry.change_type != "update":
                logger.error(f"History entry not found or not an update: {history_entry_id}")
                return False
            
            if not history_entry.rollback_data:
                logger.error(f"No rollback data available for history entry: {history_entry_id}")
                return False
            ...
            rollback_entry = PolicyHistory(
                ...
                changes={
                    "rolled_back_from": self.current_config.dict() if self.current_config else {},
    ...
    def get_policy_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_updated": self.current_config.updated_at.isoformat() if self.current_config.updated_at else None
        }
    
    def list_allowed_operations(self, target_role: TargetRole, 
                              user_id: Optional[str] = None) -> List[OperationType]:
        ...
        for rule in self._get_all_rules():
            if (target_role in rule.target_roles and 
                rule.enabled and 
                rule.allowed):
    ...
    def simulate_policy_scenario(self, operation: Any, context: PolicyContext) -> Dict[str, Any]:
        ...
        for rule in applicable_rules:
            rule_analysis.append({
                ...
                "time_restrictions": len(rule.time_restrictions) if rule.time_restrictions else 0,
