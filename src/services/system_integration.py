...
from src.models.event_models import (
    ...
    EventBuilder, ResourceUsage, create_health_event, create_security_event, create_operation_event
)
...
class FleetInventoryIntegration:
    ...
    async def generate_health_events_from_inventory(self) -> List[SystemEvent]:
        ...
        try:
                ...
                for target_name, target_data in fleet_data['targets'].items():
                    ...
                    if 'resources' in target_data:
                        ...
                        if cpu_percent > 80:
                            ...
                            cpu_event.resource_usage = ResourceUsage(cpu_percent=cpu_percent)
                            cpu_event.details = {
                                'resource_type': 'cpu',
                        ...
                        if memory_percent > 80:
                            ...
                            memory_event.description = f"Memory usage at {memory_percent:.1f}%"
                            memory_event.resource_usage = ResourceUsage(memory_percent=memory_percent)
            ...
            if events:
                ...
                self.logger.info(f"Generated {len(events)} health events from fleet inventory")
    ...
    async def generate_fleet_update_events(self) -> List[SystemEvent]:
        ...
        try:
            ...
            event.description = "Fleet inventory has been updated with new target information"
            event.details = {
...
class PolicyEngineIntegration:
    ...
    async def generate_policy_audit_events(self) -> List[SystemEvent]:
        ...
        try:
            from src.models.policy_models import PolicyAuditResult
            ...
            for result in audit_results:
                if not result.compliant:
                    ...
                    event.description = f"System is not compliant with policy: {result.reason}"
                    event.details = {
    ...
    async def generate_security_events(self) -> List[SystemEvent]:
        ...
        try:
            ...
            for audit_event in audit_events:
                ...
                if audit_event.risk_level and audit_event.risk_level.lower() in ['high', 'critical']:
...
class RemoteAgentIntegration:
        ...
        try:
            ...
            for status in service_statuses:
                if status.state != "active":
                    severity = EventSeverity.ERROR if status.state == "failed" else EventSeverity.WARNING
    ...
    async def generate_operation_events(self, operation_results: List[Dict[str, Any]]) -> List[SystemEvent]:
        ...
        try:
            ...
            for container in containers:
                if container.get('status') != 'running':
                    ...
                    severity = EventSeverity.ERROR if container.get('status') == 'stopped' else EventSeverity.WARNING
            ...
            for node in nodes:
                if 'maxcpu' in node and 'cpu' in node:
                    ...
                    if cpu_usage > 80:
                        ...
                        event.description = f"CPU usage at {cpu_usage:.1f}% on node {node['node']}"
...
class SystemIntegrationManager:
        ...
        try:
            ...
            for name, test_func in integrations:
                try:
                    ...
                    self.logger.warning(f"Integration {name} initialization failed: {e}")
            
            active_integrations = sum(1 for status in self.integration_status.values() if status)
            self.logger.info(f"Initialized {active_integrations}/{len(integrations)} integrations")
    ...
    async def collect_all_events(self) -> List[SystemEvent]:
        ...
        try:
            ...
            if self.integration_status['fleet_inventory']:
                events = await self.fleet_inventory.generate_health_events_from_inventory()
            ...
            if self.integration_status['policy_engine']:
                policy_events = await self.policy_engine.generate_policy_violation_events()
            ...
            if self.integration_status['remote_agent']:
                service_events = await self.remote_agent.generate_service_status_events()
                all_events.extend(service_events)
            
            if self.integration_status['discovery_pipeline']:
                discovery_events = await self.discovery_pipeline.generate_discovery_events()
            ...
            self.logger.info(f"Collected {len(all_events)} events from all integrations")
            return all_events
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_count': sum(1 for status in self.integration_status.values() if status),
            'total_count': len(self.integration_status),
            'health_score': (sum(1 for status in self.integration_status.values() if status) / len(self.integration_status)) * 100
        }
