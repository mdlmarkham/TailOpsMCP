...
class TargetDiscovery:
    ...
    def discover_ssh_targets(
        self, username: str = "root", key_path: str = None
    ) -> List[TargetMetadata]:
    ...
    def discover_docker_targets(self) -> List[TargetMetadata]:
        ...
        for host, port in docker_endpoints:
            if self._check_port_open(host, port):
                ...
                constraints = TargetConstraints(
                    timeout=120, concurrency=3, sudo_policy=SudoPolicy.NONE
                )
    ...
    def discover_all_targets(
        self, ssh_username: str = "root", ssh_key_path: str = None
    ) -> List[TargetMetadata]:
        ...
        try:
            ...
            if target.connection.socket_path:
                client = docker.DockerClient(
                    base_url=f"unix://{target.connection.socket_path}"
                )
            else:
