...
class LogAnalyzer:
    ...
    async def analyze_container_logs(
        ...
        try:
            ...
            response = await mcp_context.sample(
                messages=prompt, temperature=0.3, max_tokens=2000
            )
    ...
    def _build_analysis_prompt(
        self, container_name: str, logs: str, context: Optional[str] = None
    ) -> str:
...
"""
    ...
    def _extract_stats(self, logs: str) -> Dict[str, int]:
        ...
        return {
            ...
            "warning_count": sum(
                1 for line in lines if re.search(r"(?i)warning", line)
            ),
            "critical_count": sum(
                1 for line in lines if re.search(r"(?i)(critical|fatal)", line)
            ),
        }
    ...
    def _generate_basic_recommendations(
        ...
        if len(errors) > 0:
            recommendations.append(
                f"Review detailed error logs ({len(errors)} errors found)"
            )

        return (
            recommendations
            if recommendations
            else ["Review logs for detailed diagnostics"]
        )
