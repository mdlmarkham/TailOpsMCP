...
class SessionToken:
    ...
    def __init__(
        self, token: str, identity: IdentityContext, expires_at: datetime.datetime
    ):
        self.token = token
...
class TailScaleOIDCIntegration:
    ...
    def __init__(self, db_path: str = "./logs/identity.db"):
        ...
        self.oidc_enabled = (
            os.getenv("TAILSCALE_OIDC_ENABLED", "false").lower() == "true"
        )
        self.oidc_issuer = os.getenv("TAILSCALE_OIDC_ISSUER", "")
        self.oidc_audience = os.getenv("TAILSCALE_OIDC_AUDIENCE", "")

        logger.info(
            f"Tailscale OIDC integration initialized: enabled={self.oidc_enabled}"
        )
        ...
        try:
            ...
            try:
                ...
                if self.oidc_issuer and payload.get("iss") != self.oidc_issuer:
                    logger.warning(
                        f"OIDC token issuer mismatch: {payload.get('iss')} != {self.oidc_issuer}"
                    )
                    return None

                if self.oidc_audience and self.oidc_audience not in payload.get(
                    "aud", []
                ):
                    logger.warning(
                        f"OIDC token audience mismatch: {payload.get('aud')} != {self.oidc_audience}"
                    )
                    return None
    ...
    async def map_tailscale_identity(
        self, tailscale_user: Dict[str, Any]
    ) -> IdentityContext:
        ...
        try:
            ...
            username = tailscale_user.get(
                "preferred_username", tailscale_user.get("name", "")
            )
    ...
    async def create_session_from_oidc(
        self, oidc_claims: Dict[str, Any]
    ) -> SessionToken:
    ...
    def _derive_permissions(self, groups: List[str], roles: List[str]) -> List[str]:
        ...
        permissions.extend(
            ["read:own_profile", "read:own_audit_logs", "read:own_sessions"]
        )

        # Admin permissions
        if "admin" in roles or "administrators" in groups:
            permissions.extend(
                ["admin:*", "security:*", "audit:*", "user:*", "configuration:*"]
            )
        ...
        if "user" in roles or "users" in groups:
            permissions.extend(
                ["targets:read", "targets:connect", "logs:read", "basic:operations"]
            )
...
class IdentityManager:
    ...
    def __init__(self, audit_logger: Optional[SecurityAuditLogger] = None):
        ...
        self.require_mfa_for_roles = os.getenv(
    ...
    async def authenticate_user(
        ...
        try:
            ...
            if credentials.oidc_token:
                ...
                result = await self._authenticate_password(
    ...
    async def _authenticate_oidc(self, oidc_token: str) -> AuthenticationResult:
        ...
        try:
            ...
            return AuthenticationResult(
                success=True, identity=session.identity, session_token=session.token
            )

        except Exception as e:
            logger.error(f"OIDC authentication failed: {e}")
            return AuthenticationResult(
                success=False, error_message=f"OIDC authentication failed: {str(e)}"
            )
    ...
    async def _authenticate_token(self, token: str) -> AuthenticationResult:
        ...
        try:
            ...
            return AuthenticationResult(
                success=True, identity=session.identity, session_token=token
            )

        except Exception as e:
            logger.error(f"Token authentication failed: {e}")
            return AuthenticationResult(
                success=False, error_message=f"Token authentication failed: {str(e)}"
            )
    ...
    async def validate_session(self, session_token: str) -> SessionValidationResult:
        ...
        try:
            ...
            with sqlite3.connect(self.oidc_integration.db_path) as conn:
                ...
                # Cache session
                session = SessionToken(session_token, identity, expires_at)
                session.last_accessed = datetime.datetime.fromisoformat(
                    row["last_accessed"]
                )
            ...
            with sqlite3.connect(self.oidc_integration.db_path) as conn:
                ...
                if not row:
                    return PermissionSet(
                        permissions=[], roles=[], effective_permissions=[]
                    )
    ...
    async def create_session(
        self, identity: IdentityContext, ttl: Optional[datetime.timedelta] = None
    ) -> str:
    ...
    def _calculate_effective_permissions(
        self, roles: List[str], direct_permissions: List[str]
    ) -> List[str]:
    ...
    async def _log_authentication_event(
        self, result: AuthenticationResult, duration_ms: int
    ) -> None:
        """Log authentication event."""
        try:
            event = IdentityEvent(
