...
class ProxmoxDiscovery:
    ...
    def _is_proxmox_environment(self) -> bool:
        ...
        try:
            ...
            checks = [
                ...
                subprocess.run(["pvesh", "get", "/version"], capture_output=True, text=True),
    ...
    def _get_local_proxmox_info(self) -> Optional[ProxmoxHost]:
        ...
        try:
            ...
            node_result = subprocess.run(["pvesh", "get", "/nodes/$(hostname)"], 
    ...
    def _get_container_ip(self, vmid: int) -> Optional[str]:
        ...
        try:
            result = subprocess.run(["pct", "config", str(vmid)], capture_output=True, text=True)
            if result.returncode == 0:
            ...
            if result.returncode == 0:
                for line in result.stdout.split('\n'):
                    if line.startswith('memory'):
                        return int(line.split('=')[1].strip()) // 1024  # Convert KB to MB
        except Exception:
            pass
        return 512
    ...
    def create_discovery_event(self, source: str, message: str, 
                             ...
                             target_id: str = None, target_type: str = None) -> Event:
