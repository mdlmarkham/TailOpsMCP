...
class DiscoveryPipeline:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.node_probing = NodeProbing(
            tailscale_config=self.config.get("tailscale")
        )
    ...
    async def run_discovery_cycle(self) -> FleetInventory:
        ...
        discovery_event = self._create_discovery_event("start", "Discovery cycle started")
        ...
        try:
            ...
            logger.info(f"Discovery cycle completed: {len(hosts)} hosts, {self.inventory.total_nodes} nodes")
            
        except Exception as e:
            # Create error event
            error_event = self._create_discovery_event(
                "error", 
                f"Discovery cycle failed: {e}",
                severity="error"
            )
        ...
        try:
            ...
            for node in nodes:
                ...
                if node.id in self.inventory.nodes:
                    ...
                    existing_node.ip_address = node.ip_address or existing_node.ip_address
                else:
                    # Add new node
    ...
    def _get_nodes_for_probing(self) -> List[Node]:
        ...
        for node in self.inventory.nodes.values():
            ...
            last_probe = datetime.fromisoformat(node.last_updated.replace('Z', '+00:00')) if node.last_updated else None
            
            if not last_probe or (now - last_probe) > timedelta(seconds=self.health_check_interval):
    ...
    def _update_node_from_probe(self, node: Node, probe_result: Dict[str, Any]) -> None:
        ...
        successful_connections = [conn for conn, result in connection_results.items() 
                                if result.get("success")]
        
        if successful_connections:
            # Update connection method if a better one is available
            best_connection = successful_connections[0]  # First successful (highest priority)
    ...
    def _update_services_from_probe(self, node: Node, probe_result: Dict[str, Any]) -> None:
        """Update services from probe results."""
        discovered_services = probe_result.get("services", [])
        
        # Remove old services for this node
        services_to_remove = [
            service_id for service_id, service in self.inventory.services.items()
    ...
    def _cleanup_stale_entries(self) -> None:
        ...
        for host_id, host in self.inventory.proxmox_hosts.items():
            last_seen = datetime.fromisoformat(host.last_seen.replace('Z', '+00:00')) if host.last_seen else None
        ...
        for node_id, node in self.inventory.nodes.items():
            last_updated = datetime.fromisoformat(node.last_updated.replace('Z', '+00:00')) if node.last_updated else None
    ...
    def _create_discovery_event(self, action: str, message: str, 
                              severity: str = "info", 
                              target_id: str = None, 
                              target_type: str = None) -> Event:
