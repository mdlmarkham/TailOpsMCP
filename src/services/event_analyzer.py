...
class EventAnalyzer:
    ...
    def _group_events_by_time_interval(self, events: List[SystemEvent], interval: str) -> Dict[str, List[SystemEvent]]:
        """Group events by time intervals."""
    ...
    async def _analyze_event_frequency_trends(self, time_grouped: Dict[str, List[SystemEvent]]) -> List[Trend]:
        ...
        if abs(change_ratio) > 0.2:  # 20% change threshold
            ...
            trend = Trend(
                ...
                impact='medium',
                data_points=[(datetime.fromisoformat(tp), c) for tp, c in zip(time_points, counts)]
            )
            trends.append(trend)
        
        return trends
    
    async def _analyze_severity_trends(self, time_grouped: Dict[str, List[SystemEvent]]) -> List[Trend]:
        ...
        for time_point in time_points:
            ...
            if events:
                avg_severity = sum(severity_mapping.get(e.severity, 0) for e in events) / len(events)
        ...
        if abs(change) > 0.5:  # Significant severity change
            ...
            trend = Trend(
                ...
                data_points=[(datetime.fromisoformat(tp), s) for tp, s in zip(time_points, severity_scores)]
            )
            trends.append(trend)
        
        return trends
    
    async def _analyze_source_trends(self, time_grouped: Dict[str, List[SystemEvent]]) -> List[Trend]:
    ...
    async def _analyze_health_score_trends(self, events: List[SystemEvent]) -> List[Trend]:
        ...
        for time_point in time_points:
            ...
            avg_score = statistics.mean(e.health_score for e in events_in_period if e.health_score is not None)
        ...
        if abs(change) > 5:  # 5-point change threshold
            ...
            trend = Trend(
                ...
                time_period=f'{len(time_points)} hours',
                impact='high',
                data_points=[(datetime.fromisoformat(tp), s) for tp, s in zip(time_points, health_scores)]
            )
            trends.append(trend)
        
        return trends
    
    async def _analyze_resource_usage_trends(self, events: List[SystemEvent]) -> List[Trend]:
        ...
        for resource_type in ['cpu', 'memory', 'disk']:
            ...
            for time_point in time_points:
                ...
                for event in events_in_period:
                    if event.resource_usage:
                        if resource_type == 'cpu' and event.resource_usage.cpu_percent is not None:
                            usage_values_period.append(event.resource_usage.cpu_percent)
                        elif resource_type == 'memory' and event.resource_usage.memory_percent is not None:
                            usage_values_period.append(event.resource_usage.memory_percent)
                        elif resource_type == 'disk' and event.resource_usage.disk_percent is not None:
                            usage_values_period.append(event.resource_usage.disk_percent)
            ...
            valid_points = [(tp, uv) for tp, uv in zip(time_points, usage_values) if uv is not None]
    ...
    async def _detect_recurring_patterns(self, events: List[SystemEvent]) -> List[Pattern]:
        ...
        for (event_type, source), group_events in event_groups.items():
            if len(group_events) >= 3:
                ...
                if intervals:
                    ...
                    if avg_interval > 0 and std_interval / avg_interval < 0.3:  # CV < 30%
                        pattern = Pattern(
    ...
    async def _detect_sequential_patterns(self, events: List[SystemEvent]) -> List[Pattern]:
        ...
        for i in range(len(sorted_events) - sequence_length + 1):
            ...
            sequence_signature = [(e.event_type.value, e.source.value) for e in sequence]
            ...
            for j in range(i + 1, len(sorted_events) - sequence_length + 1):
                ...
                comparison_signature = [(e.event_type.value, e.source.value) for e in comparison_sequence]
    ...
    async def _detect_correlation_patterns(self, events: List[SystemEvent]) -> List[Pattern]:
            ...
            for event_type, count in event_type_counts.items():
                ...
                if ratio > 0.3:  # Event type represents > 30% of events for this target
                    pattern = Pattern(
                        ...
                        frequency=count,
                        confidence=ratio,
                        events_involved=[e.event_id for e in target_events if e.event_type.value == event_type],
                        time_range=(min(e.timestamp for e in target_events), max(e.timestamp for e in target_events)),
                        characteristics={
                            'target': target,
    ...
    async def _detect_anomaly_patterns(self, events: List[SystemEvent]) -> List[Pattern]:
        ...
        if len(time_grouped) >= 3:
            counts = [len(events_in_period) for events_in_period in time_grouped.values()]
            ...
            for time_period, period_events in time_grouped.items():
                ...
                if abs(z_score) > 2:  # More than 2 standard deviations
                    ...
                    pattern = Pattern(
                        ...
                        time_range=(min(e.timestamp for e in period_events), max(e.timestamp for e in period_events)),
                        characteristics={
    ...
    async def _generate_operational_insights(self, events: List[SystemEvent]) -> List[Insight]:
        """Generate operational insights."""
        insights = []
        
        # Analyze operation completion rates
        operation_events = [e for e in events if e.event_type in [EventType.OPERATION_COMPLETED, EventType.OPERATION_FAILED]]
        
        if operation_events:
            completed = len([e for e in operation_events if e.event_type == EventType.OPERATION_COMPLETED])
            failed = len([e for e in operation_events if e.event_type == EventType.OPERATION_FAILED])
            ...
            if success_rate < 0.8:  # Less than 80% success rate
                insight = Insight(
                    ...
                    supporting_evidence=[f'{failed} failed operations out of {total} total'],
    ...
    async def _generate_security_insights(self, events: List[SystemEvent]) -> List[Insight]:
        """Generate security insights."""
        insights = []
        ...
        if security_events:
            critical_security = len([e for e in security_events if e.severity == EventSeverity.CRITICAL])
            ...
            if critical_security > 0:
                insight = Insight(
                    ...
                    supporting_evidence=[f'{len(security_events)} total security events'],
                    timestamp=datetime.utcnow(),
    ...
    async def _generate_performance_insights(self, events: List[SystemEvent]) -> List[Insight]:
        """Generate performance insights."""
        insights = []
        
        # Analyze resource usage events
        resource_events = [e for e in events if e.event_type == EventType.RESOURCE_THRESHOLD]
        ...
        if resource_events:
            ...
            for event in high_resource_events:
                if event.resource_usage:
                    if event.resource_usage.cpu_percent and event.resource_usage.cpu_percent > 90:
                        insight = Insight(
                            ...
                            supporting_evidence=[f'CPU usage: {event.resource_usage.cpu_percent}%'],
                            timestamp=datetime.utcnow(),
                            related_events=[event.event_id]
                        )
                        insights.append(insight)
                    
                    elif event.resource_usage.memory_percent and event.resource_usage.memory_percent > 90:
                        insight = Insight(
                            ...
                            supporting_evidence=[f'Memory usage: {event.resource_usage.memory_percent}%'],
                            timestamp=datetime.utcnow(),
                            related_events=[event.event_id]
                        )
                        insights.append(insight)
    ...
    async def _generate_health_insights(self, events: List[SystemEvent]) -> List[Insight]:
        """Generate health insights."""
        insights = []
        ...
        if health_events:
            ...
            if low_health_events:
                avg_low_health = statistics.mean(e.health_score for e in low_health_events)
                insight = Insight(
                    ...
                    supporting_evidence=[f'Systems with health score < 50: {len(low_health_events)}'],
                    timestamp=datetime.utcnow(),
    ...
    async def _predict_capacity_issues(self, events: List[SystemEvent]) -> List[Prediction]:
            ...
            for event in resource_events:
                if event.resource_usage and event.target:
                    if event.resource_usage.cpu_percent:
                        resource_trends[(event.target, 'cpu')].append(event.resource_usage.cpu_percent)
                    if event.resource_usage.memory_percent:
                        resource_trends[(event.target, 'memory')].append(event.resource_usage.memory_percent)
            
            for (target, resource_type), usage_values in resource_trends.items():
                if len(usage_values) >= 2:
                    # Check if usage is increasing
                    usage_values.sort()
                    recent_usage = statistics.mean(usage_values[-2:])  # Average of last 2 values
                    
                    if recent_usage > 85:  # High usage threshold
                        trend_direction = 'increasing' if len(usage_values) > 2 and usage_values[-1] > usage_values[-2] else 'stable'
    ...
    async def _predict_failure_risks(self, events: List[SystemEvent]) -> List[Prediction]:
        """Predict system failure risks."""
        predictions = []
        
        # Analyze error patterns
        error_events = [e for e in events if e.severity in [EventSeverity.ERROR, EventSeverity.CRITICAL]]
        ...
        if len(error_events) >= 5:
            ...
            for target, target_error_events in target_errors.items():
                if len(target_error_events) >= 3:
                    ...
                    time_span = (recent_errors[-1].timestamp - recent_errors[0].timestamp).total_seconds() / 3600  # hours
    ...
    async def _predict_security_risks(self, events: List[SystemEvent]) -> List[Prediction]:
        ...
        if len(security_events) >= 3:
            ...
            if len(time_grouped) >= 2:
                daily_counts = [len(events_in_day) for events_in_day in time_grouped.values()]
                daily_counts.sort()
                
                if len(daily_counts) >= 2 and daily_counts[-1] > daily_counts[0] * 2:  # Doubled in frequency
                    prediction = Prediction(
    ...
    async def _predict_health_risks(self, events: List[SystemEvent]) -> List[Prediction]:
        ...
        if len(health_events) >= 5:
            ...
            for target, target_health_events in target_health.items():
                if len(target_health_events) >= 3:
                    ...
                    if len(health_scores) >= 3:
                        recent_avg = statistics.mean(health_scores[-2:])  # Average of last 2 scores
                        
                        if recent_avg < 60:  # Poor health threshold
                            # Check if trend is declining
                            declining_trend = len(health_scores) > 2 and health_scores[-1] < health_scores[-2]
