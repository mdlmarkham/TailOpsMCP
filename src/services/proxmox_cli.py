...
class ProxmoxCLI:
    ...
    def __init__(self, credentials: Optional[ProxmoxAPICredentials] = None):
        ...
        if not self._proxmox_environment and not self._is_local:
            logger.warning(
                "Not running in Proxmox environment and no remote credentials provided"
            )
        ...
        try:
            if not self._proxmox_environment:
                return OperationResult.failure(
                    "Proxmox CLI not available", "Not running in Proxmox environment"
                )
            ...
            if version_result.returncode != 0:
                return OperationResult.failure(
                    "Proxmox version command failed", version_result.stderr
                )
    ...
    async def _run_command(
        self, command: List[str], timeout: int = 300, check: bool = True
    ...
    async def create_container_cli(
    ...
    async def delete_container_cli(self, vmid: int) -> DeleteResult:
        ...
        try:
            ...
            if result.returncode == 0:
                return DeleteResult(
                    status="deleted", message="Container deleted successfully via CLI"
                )
            else:
                return DeleteResult(
                    status="failed",
    ...
    async def start_container_cli(self, vmid: int) -> StartResult:
        ...
        try:
            ...
            if result.returncode == 0:
                return StartResult(
                    status="started", message="Container started successfully via CLI"
                )
            else:
                return StartResult(
                    status="failed", message=f"Container start failed: {result.stderr}"
                )
    ...
    async def stop_container_cli(self, vmid: int, force: bool = False) -> StopResult:
        ...
        try:
            ...
            if result.returncode == 0:
                return StopResult(
                    status="stopped", message="Container stopped successfully via CLI"
                )
            else:
                return StopResult(
                    status="failed", message=f"Container stop failed: {result.stderr}"
                )
    ...
    async def reboot_container_cli(self, vmid: int) -> RebootResult:
        ...
        try:
            ...
            if result.returncode == 0:
                return RebootResult(
                    status="rebooted", message="Container rebooted successfully via CLI"
                )
            else:
                return RebootResult(
                    status="failed", message=f"Container reboot failed: {result.stderr}"
                )
    ...
    async def create_snapshot_cli(
        ...
        try:
            ...
            logger.error(
                f"Failed to create snapshot {snapshot_name} for {vmid} via CLI: {e}"
            )
    ...
    async def delete_snapshot_cli(self, vmid: int, snapshot_name: str) -> DeleteResult:
        ...
        try:
            ...
            result = await self._run_command(
                ["pct", "delsnapshot", str(vmid), snapshot_name]
            )

            # If container command fails, try VM
            if result.returncode != 0:
                result = await self._run_command(
                    ["qm", "delsnapshot", str(vmid), snapshot_name]
                )

            if result.returncode == 0:
                return DeleteResult(
                    status="deleted", message="Snapshot deleted successfully via CLI"
                )
            else:
                return DeleteResult(
                    status="failed",
            ...
            logger.error(
                f"Failed to delete snapshot {snapshot_name} for {vmid} via CLI: {e}"
            )
            return DeleteResult(status="failed", message=str(e))

    async def restore_snapshot_cli(
        self, vmid: int, snapshot_name: str
    ) -> RestoreResult:
        ...
        try:
            # Try container first
            result = await self._run_command(
                ["pct", "rollback", str(vmid), snapshot_name]
            )

            # If container command fails, try VM
            if result.returncode != 0:
                result = await self._run_command(
                    ["qm", "rollback", str(vmid), snapshot_name]
                )

            if result.returncode == 0:
                return RestoreResult(
                    status="restored", message="Snapshot restored successfully via CLI"
                )
            else:
                return RestoreResult(
                    status="failed", message=f"Snapshot restore failed: {result.stderr}"
                )

        except Exception as e:
            logger.error(
                f"Failed to restore snapshot {snapshot_name} for {vmid} via CLI: {e}"
            )
            return RestoreResult(status="failed", message=str(e))

    # Backup Management via CLI

    async def create_backup_cli(
    ...
    async def get_system_info_cli(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get resources
            resources_result = await self._run_command(
    ...
    def _parse_snapshot_output(
        self, snapshot_output: str, vmid: int
    ) -> List[ProxmoxSnapshot]:
            ...
            if line.strip() and not line.startswith("Name"):
                ...
                if len(parts) >= 2:
                    ...
                    snapshot = ProxmoxSnapshot(
                        name=name, vmid=vmid, node="localhost", description=description
                    )
