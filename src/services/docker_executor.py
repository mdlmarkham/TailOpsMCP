...
class DockerExecutor(Executor):
    ...
    def __init__(self, socket_path: Optional[str] = None, host: Optional[str] = None,
                 timeout: int = 30, tls_verify: bool = False, cert_path: Optional[str] = None,
    ...
    def connect(self) -> bool:
        ...
        for attempt in range(self.retry_attempts):
            try:
                if self.socket_path:
                    ...
                    if self.tls_verify and self.cert_path:
                        tls_config = docker.tls.TLSConfig(
                            ...
                            client_cert=(f"{self.cert_path}/cert.pem", f"{self.cert_path}/key.pem"),
                            verify=True
                        )
                ...
                logger.warning(f"Docker connection attempt {attempt + 1} failed: {str(e)}")
                self.client = None
                
                if attempt < self.retry_attempts - 1:
                    time.sleep(self.retry_delay)
                else:
                    logger.error(f"Docker connection failed after {self.retry_attempts} attempts")
                    return False
    ...
    def execute_container_command(self, container_id: str, command: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "output": exec_result.output.decode('utf-8').strip() if exec_result.output else "",
                "command": command
            }
