...
from src.services.capability_executor import CapabilityExecutor, CapabilityOperation
...
class RemoteAgentCapabilities:
    ...
    @staticmethod
    async def get_journald_logs(target: TargetConnection, params: dict) -> OperationResult:
    ...
    @staticmethod
    async def docker_operations(target: TargetConnection, params: dict) -> OperationResult:
        ...
        try:
            ...
            if operation == 'list':
                ...
                result = await remote_agent_tools.get_container_logs_remote(
                    target=target_id, container_id=container_id, lines=lines, since=since
                )
                ...
                result = {"success": False, "error": f"Unknown Docker operation: {operation}"}
    ...
    @staticmethod
    async def file_operations(target: TargetConnection, params: dict) -> OperationResult:
        ...
        try:
            ...
            if operation == 'read':
                ...
                result = await remote_agent_tools.write_remote_file(
                    target=target_id, path=path, content=content, create_backup=create_backup
                )
            elif operation == 'list':
                ...
                result = {"success": False, "error": f"Unknown file operation: {operation}"}
    ...
    @staticmethod
    async def fleet_operations(targets: List[TargetConnection], params: dict) -> OperationResult:
        ...
        try:
            ...
            if operation == 'analyze_logs':
                ...
                result = {"success": False, "error": f"Unknown fleet operation: {operation}"}
...
class RemoteAgentCapabilityRegistry:
    ...
    def _setup_capabilities(self):
        ...
        self.capabilities.update({
            "docker_container_list": {
                ...
                "parameters": {
                    "all_containers": {"type": "bool", "required": False, "default": False}
                },
        ...
        self.capabilities.update({
            ...
            "file_list": {
                ...
                "parameters": {
                    "path": {"type": "string", "required": True},
                    "include_hidden": {"type": "bool", "required": False, "default": False}
                },
                "executor": RemoteAgentCapabilities.file_operations,
                "tier": "observe"
            }
        ...
        self.capabilities.update({
            "fleet_analyze_logs": {
                ...
                "parameters": {
                    "service": {"type": "string", "required": True},
                    "time_range": {"type": "string", "required": False, "default": "1 hour"}
                },
    ...
    def get_capabilities_by_tier(self, tier: str) -> List[str]:
        ...
        """
        return [name for name, cap in self.capabilities.items() if cap.get('tier') == tier]
...
def register_remote_agent_capabilities():
    ...
    try:
        ...
        logger.info(f"Registered {len(remote_agent_capability_registry.list_capabilities())} remote agent capabilities")
...
def validate_remote_agent_capability_params(capability_name: str, params: dict) -> Tuple[bool, Optional[str]]:
