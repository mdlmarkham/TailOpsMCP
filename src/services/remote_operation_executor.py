...
from src.connectors.remote_agent_connector import OperationResult, RemoteAgentError, ConnectionError
...
class ResilientRemoteOperation:
    ...
    async def execute_with_retry(self, 
                                ...
                                operation_name: str = "unnamed_operation") -> OperationResult:
        ...
        try:
            ...
            self.logger.error(f"Operation {operation_name} failed after {metrics.retries_attempted} retries: {str(e)}")
    ...
    async def execute_with_timeout(self,
                                  operation: Callable,
                                  timeout: float,
                                  operation_name: str = "timeout_operation") -> OperationResult:
    ...
    async def handle_connection_errors(self,
                                      operation: Callable,
                                      operation_name: str = "connection_operation") -> OperationResult:
    ...
    def get_operation_metrics(self, operation_name: Optional[str] = None) -> List[OperationMetrics]:
    ...
    async def _execute_with_retry_internal(self,
                                         operation: Callable,
                                         config: ResilientOperationConfig,
                                         metrics: OperationMetrics) -> OperationResult:
        ...
        while attempt <= config.max_retries:
            try:
                ...
                self.logger.warning(f"Operation {metrics.operation_name} failed (attempt {attempt}), "
...
def resilient_operation(operation_type: OperationType = OperationType.COMMAND_EXECUTION,
                       retry_strategy: RetryStrategy = RetryStrategy.EXPONENTIAL_BACKOFF,
                       max_retries: int = 3,
                       timeout: float = 30.0,
                       operation_name: Optional[str] = None):
