...
from src.models.event_models import (
    SystemEvent, EventType, EventSeverity, EventSource, EventCategory, EventStatus,
    EventFilters, EventStatistics, ResourceUsage
)
...
class EventStore:
    ...
    def __init__(self, database_path: Optional[str] = None):
        self.database_path = database_path or os.getenv("SYSTEMMANAGER_EVENT_DB", "./data/events.db")
    ...
    async def get_events(self, filters: Optional[EventFilters] = None, limit: Optional[int] = None) -> List[SystemEvent]:
    ...
    def _build_query(self, filters: EventFilters, limit: Optional[int]) -> Tuple[str, List]:
        ...
        if filters.search_text:
            base_query += " AND id IN (SELECT rowid FROM events_fts WHERE events_fts MATCH ?)"
    ...
    def _row_to_event(self, row: sqlite3.Row) -> Optional[SystemEvent]:
        ...
        try:
            ...
            resource_usage_data = json.loads(row['resource_usage']) if row['resource_usage'] else None
    ...
    async def get_events_by_time_range(self, start: datetime, end: datetime, limit: Optional[int] = None) -> List[SystemEvent]:
        """Get events within a time range."""
        filters = EventFilters(start_time=start, end_time=end, limit=limit)
        return await self.get_events(filters)
    
    async def get_events_by_source(self, source: str, hours: int = 24, limit: Optional[int] = None) -> List[SystemEvent]:
    ...
    async def get_events_by_target(self, target: str, hours: int = 24, limit: Optional[int] = None) -> List[SystemEvent]:
    ...
    async def search_events(self, query: str, hours: int = 24, limit: Optional[int] = None) -> List[SystemEvent]:
    ...
    async def delete_old_events(self, before_date: datetime) -> int:
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("DELETE FROM events WHERE timestamp < ?", (before_date.isoformat(),))
                deleted_count = cursor.rowcount
                
                # Delete from FTS index (SQLite handles this automatically for content table)
                conn.commit()
                
                self.logger.info(f"Deleted {deleted_count} events older than {before_date}")
...
class EventIndex:
    ...
    async def create_index(self, field: str) -> None:
        ...
        if field not in ['timestamp', 'event_type', 'severity', 'source', 'target', 'category', 'status']:
        ...
        try:
            with self.event_store.get_connection() as conn:
                ...
                cursor.execute(f"CREATE INDEX IF NOT EXISTS {index_name} ON events({field})")
                conn.commit()
