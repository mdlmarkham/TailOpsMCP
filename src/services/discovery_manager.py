    ...
    def _load_config_from_env(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "max_concurrent_probes": int(
                os.getenv("SYSTEMMANAGER_MAX_CONCURRENT_PROBES", "5")
            ),
...
def integrate_with_target_registry(
    discovery_manager: DiscoveryManager, target_registry
) -> None:
    ...
    def _create_target_metadata_from_node(node):
        ...
        if node.connection_method == "tailscale_ssh":
            ...
            executor = ExecutorType.SSH
            connection = TargetConnection(
                executor=executor, host=node.ip_address, username="root", timeout=30
            )
        else:
