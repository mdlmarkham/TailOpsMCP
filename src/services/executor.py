...
class ConnectionInfo:
    ...
    connection_id: str = field(
        default_factory=lambda: f"conn_{int(time.time() * 1000)}"
    )
    ...
    def execute_with_retry(self, command: str, **kwargs) -> ExecutionResult:
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                if result.success:
                    logger.debug(
                        f"Command executed successfully on attempt {attempt + 1}"
                    )
                    return result
                ...
                if result.status in [
                    ExecutionStatus.AUTHENTICATION_ERROR,
...
class ExecutorFactory:
    ...
    def create_executor(self, config: ExecutorConfig) -> Optional[Executor]:
        ...
        if cache_key in self._executor_cache:
            ...
            if cached_executor.is_available():
                logger.debug(
                    f"Returning cached executor for {config.executor_type.value}"
                )
        ...
        if not executor_class:
            logger.error(
                f"No executor class registered for type: {config.executor_type.value}"
            )
            return None
    ...
    def register_executor_type(
        self, executor_type: ExecutorType, executor_class: type
    ) -> None:
