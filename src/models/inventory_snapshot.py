...
class SnapshotDiff(BaseModel):
    ...
    def calculate_metrics(self) -> None:
        ...
        self.total_changes = len(self.target_changes) + len(self.service_changes) + len(self.stack_changes)
        
        for change_list in [self.target_changes, self.service_changes, self.stack_changes]:
...
class SnapshotManager:
    ...
    def compare_snapshots(
        self,
        snapshot_a: InventorySnapshot,
        snapshot_b: InventorySnapshot
    ) -> SnapshotDiff:
        ...
        diff.service_changes = self._compare_entities(
            inventory_a.services,
            inventory_b.services,
            "service"
        )
        ...
        for entity_id, entity_b in entities_b.items():
            if entity_id not in entities_a:
                ...
                changes.append(EntityChange(
                    ...
                    new_value=entity_b.to_dict() if hasattr(entity_b, 'to_dict') else entity_b
                ))
            else:
                # Entity might have been modified
                entity_a = entities_a[entity_id]
                entity_a_dict = entity_a.to_dict() if hasattr(entity_a, 'to_dict') else entity_a
                entity_b_dict = entity_b.to_dict() if hasattr(entity_b, 'to_dict') else entity_b
        ...
        for entity_id, entity_a in entities_a.items():
            if entity_id not in entities_b:
                changes.append(EntityChange(
                    ...
                    old_value=entity_a.to_dict() if hasattr(entity_a, 'to_dict') else entity_a
                ))
        
        return changes
    
    def _find_field_changes(self, old_dict: Dict[str, Any], new_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_health_impact(
        ...
        """
        impact = {
            "health_score_change": inventory_b.average_health_score - inventory_a.average_health_score,
            "healthy_targets_change": inventory_b.healthy_targets - inventory_a.healthy_targets,
            "unhealthy_targets_change": inventory_b.unhealthy_targets - inventory_a.unhealthy_targets,
            "critical_changes": [],
            "potential_issues": []
        }
        ...
        for change in diff.target_changes + diff.service_changes + diff.stack_changes:
            if change.change_type == ChangeType.DELETED:
                ...
                if "status" in change.field_changes:
                    ...
                    if old_status in ["running", "healthy"] and new_status in ["stopped", "failed"]:
                ...
                if "health_score" in change.field_changes:
                    ...
                    if old_score - new_score > 0.2:  # Health score dropped by more than 20%
                        impact["potential_issues"].append({
