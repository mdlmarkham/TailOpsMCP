...
class ParameterConstraint:
    ...
    allowed_values: Optional[List[str]] = Field(None, description="Allowed values for the parameter")
    allowlist_source: Optional[str] = Field(None, description="Source for dynamic allowlist")
...
class PolicyRule(BaseModel):
    ...
    operations: List[OperationType] = Field(..., description="Operations this rule applies to")
    target_roles: List[TargetRole] = Field(..., description="Target roles this rule applies to")
    target_patterns: Optional[List[str]] = Field(None, description="Regex patterns for target matching")
    
    # Access control
    allowed: bool = Field(default=True, description="Whether operations are allowed")
    requires_approval: bool = Field(default=False, description="Whether operations require approval")
    approval_timeout: Optional[int] = Field(None, description="Approval timeout in minutes")
    
    # Time-based restrictions
    time_restrictions: Optional[List[TimeConstraint]] = Field(None, description="Time-based constraints")
    maintenance_windows_only: bool = Field(default=False, description="Only during maintenance windows")
    
    # Parameter constraints
    parameter_constraints: Dict[str, ParameterConstraint] = Field(default_factory=dict, description="Parameter validation rules")
    
    # Resource limits
    max_concurrent_operations: Optional[int] = Field(None, description="Maximum concurrent operations")
    operation_timeout: Optional[int] = Field(default=300, description="Default operation timeout in seconds")
...
class PolicyConfig(BaseModel):
    ...
    # Policy metadata
    id: str = Field(default_factory=lambda: str(uuid4()), description="Policy configuration identifier")
    name: str = Field(..., description="Policy configuration name")
    description: str = Field(..., description="Policy configuration description")
    version: PolicyVersion = Field(default=PolicyVersion.V2, description="Policy configuration version")
    enabled: bool = Field(default=True, description="Whether policy is enabled")
    
    # Global settings
    deny_by_default: bool = Field(default=True, description="Deny operations not explicitly allowed")
    enable_dry_run: bool = Field(default=True, description="Enable dry-run mode")
    require_approval_for_admin: bool = Field(default=True, description="Require approval for admin operations")
    audit_all_operations: bool = Field(default=True, description="Audit all operations")
    
    # Inheritance settings
    enable_policy_inheritance: bool = Field(default=True, description="Enable policy inheritance")
    parent_policies: List[str] = Field(default_factory=list, description="Parent policy IDs")
    
    # Role-based policies
    role_policies: Dict[TargetRole, List[PolicyRule]] = Field(default_factory=dict, description="Policies by target role")
    
    # Global policies (apply to all targets)
    global_policies: List[PolicyRule] = Field(default_factory=list, description="Global policy rules")
    
    # Emergency policies
    emergency_policies: List[PolicyRule] = Field(default_factory=list, description="Emergency override policies")
    
    # Maintenance windows
    maintenance_windows: List[Dict[str, Any]] = Field(default_factory=list, description="Scheduled maintenance windows")
    ...
    effective_from: Optional[datetime] = Field(None, description="Policy effective date")
...
class PolicyEvaluation(BaseModel):
    ...
    matched_rules: List[str] = Field(default_factory=list, description="IDs of matched policy rules")
    required_approvals: List[str] = Field(default_factory=list, description="Required approval sources")
    time_constraints: List[TimeConstraint] = Field(default_factory=list, description="Applicable time constraints")
    parameter_validations: Dict[str, Any] = Field(default_factory=dict, description="Parameter validation results")
    audit_context: Dict[str, Any] = Field(default_factory=dict, description="Audit context")
...
class PolicyHistory(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()), description="History entry identifier")
    ...
    changes: Dict[str, Any] = Field(..., description="Changed fields and their old/new values")
    changed_by: str = Field(..., description="User who made the change")
    changed_at: datetime = Field(default_factory=datetime.utcnow, description="Change timestamp")
...
class CapabilityOperation(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()), description="Operation identifier")
    ...
    correlation_id: str = Field(default_factory=lambda: str(uuid4()), description="Correlation ID for tracking")
...
class PolicyValidationResult(BaseModel):
    ...
    rule_validations: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Individual rule validation results")
    dependency_checks: Dict[str, Any] = Field(default_factory=dict, description="Dependency validation results")
    conflict_checks: List[Dict[str, Any]] = Field(default_factory=list, description="Policy conflict results")
...
class PolicyContext(BaseModel):
    ...
    user_roles: List[str] = Field(default_factory=list, description="User roles and permissions")
    ...
    compliance_mode: str = Field(default="standard", description="Compliance mode level")
    
    # Time context
    current_time: datetime = Field(default_factory=datetime.utcnow, description="Current time for evaluation")
...
def create_default_policy_config() -> PolicyConfig:
    ...
    global_allow_rules = [
        PolicyRule(
            ...
            target_roles=[TargetRole.GATEWAY, TargetRole.PRODUCTION, TargetRole.DEVELOPMENT],
            allowed=True,
        ...
        PolicyRule(
            ...
            operations=[OperationType.CONTAINER_START, OperationType.CONTAINER_STOP, OperationType.CONTAINER_INSPECT],
    ...
    production_rules = [
        PolicyRule(
            ...
            operations=[OperationType.SERVICE_RESTART, OperationType.SERVICE_START, OperationType.SERVICE_STOP],
...
def validate_policy_config(config: PolicyConfig) -> PolicyValidationResult:
    ...
    if config.deny_by_default:
        suggestions.append("Consider documenting the rationale for deny-by-default policy")
    
    if not config.audit_all_operations:
        suggestions.append("Consider enabling audit_all_operations for better compliance tracking")
        ...
        if constraint.type in ["int", "float"]:
            if constraint.min_value is not None and constraint.max_value is not None:
                if constraint.min_value >= constraint.max_value:
                    errors.append(f"Parameter {param_name} has invalid min/max value range")
    
    # Time constraint validation
    for time_constraint in rule.time_restrictions or []:
        if time_constraint.start_time >= time_constraint.end_time:
            errors.append(f"Time constraint has invalid start/end time range")
...
def check_policy_conflicts(config: PolicyConfig) -> List[str]:
    ...
    for i, rule1 in enumerate(all_rules):
        for j, rule2 in enumerate(all_rules[i+1:], i+1):
            ...
            if (set(rule1.operations) & set(rule2.operations) and
                set(rule1.target_roles) & set(rule2.target_roles) and
                rule1.allowed != rule2.allowed):
                conflicts.append(f"Rules '{rule1.name}' and '{rule2.name}' have conflicting allow settings")
