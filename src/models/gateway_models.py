...
class GatewayMetadata:
    ...
    def validate(self) -> List[str]:
        ...
        if len(self.managed_targets) > self.max_fleet_size:
            errors.append(f"Managed targets exceed max fleet size of {self.max_fleet_size}")
    ...
    def add_managed_target(self, target_id: str) -> bool:
        ...
        if target_id not in self.managed_targets and len(self.managed_targets) < self.max_fleet_size:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> GatewayMetadata:
        ...
        return cls(
            ...
            discovery_method=DiscoveryMethod(data.get("discovery_method", DiscoveryMethod.MANUAL.value)),
...
class FleetState:
    ...
    def _recalculate_metrics(self) -> None:
        ...
        if self.total_memory_mb > 0:
            self.memory_utilization_percent = min(100.0, (self.total_memory_mb / (self.total_memory_mb * 1.1)) * 100)
        
        if self.total_cpu_cores > 0:
            self.cpu_utilization_percent = min(100.0, (self.total_cpu_cores / (self.total_cpu_cores * 1.1)) * 100)
