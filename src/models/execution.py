...
class ExecutionResult(BaseModel):
    ...
    severity: ExecutionSeverity = Field(ExecutionSeverity.INFO, description="Execution severity level")
    ...
    structured_error: Optional[StructuredError] = Field(None, description="Structured error details")
    duration: float = Field(..., description="Execution duration in seconds")
    
    # Timing and identification
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Execution timestamp")
    correlation_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Correlation ID for traceability")
    ...
    audit_trail: List[Dict[str, Any]] = Field(default_factory=list, description="Audit trail entries")
    metrics: Dict[str, Union[int, float, str]] = Field(default_factory=dict, description="Execution metrics")
    ...
    def set_structured_error(self, code: str, message: str, details: Optional[Dict[str, Any]] = None,
...
class ExecutionRequest(BaseModel):
    ...
    scopes: List[str] = Field(default_factory=list, description="Required authorization scopes")
    risk_level: Optional[str] = Field(None, description="Risk level of the operation")
    
    # Metrics and monitoring
    collect_metrics: bool = Field(True, description="Whether to collect execution metrics")
...
class ExecutionBatchResult(BaseModel):
    ...
    results: Dict[str, ExecutionResult] = Field(..., description="Individual execution results")
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Batch execution timestamp")
    
    # Batch-level metrics
    correlation_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Batch correlation ID")
    metrics: Dict[str, Union[int, float, str]] = Field(default_factory=dict, description="Batch-level metrics")
...
class AuditLogEntry(BaseModel):
    ...
    scopes: List[str] = Field(default_factory=list, description="Authorization scopes used")
