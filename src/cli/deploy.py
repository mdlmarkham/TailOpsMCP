...
def create_parser():
    ...
    parser = argparse.ArgumentParser(description="TailOpsMCP Deployment Tool")
    ...
    server_parser.add_argument(
        "--host", default="localhost", help="Host for HTTP server"
    )
    server_parser.add_argument(
    ...
    container_parser.add_argument(
        "--all", action="store_true", help="Include stopped containers"
    )
...
async def handle_system_status(args):
    ...
    if status["success"]:
        ...
        print(
            f"  Load Average: {status['data']['load_average']['1min']:.2f}, {status['data']['load_average']['5min']:.2f}, {status['data']['load_average']['15min']:.2f}"
        )
...
async def handle_list_containers(args):
    ...
    if containers["success"]:
        ...
        for container in containers["data"]:
            print(
                f"  {container['id']} {container['name']} ({container['status']}) - {container['image']}"
            )
    else:
...
async def handle_list_directory(args):
    ...
    if entries["success"]:
        ...
        for entry in entries["data"]:
            print(
                f"  {entry['type'][0]} {entry['permissions']} {entry['size']:10} {entry['name']}"
            )
    else:
