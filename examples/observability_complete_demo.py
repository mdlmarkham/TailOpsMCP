...
from src.services.event_processor import get_event_stream_processor, EventStreamConfig
...
from src.services.system_integration import get_system_integration_manager
...
class ObservabilityDemo:
    ...
    async def setup(self):
        ...
        self.logger.info(
            f"Configuration loaded: {config.system_name} v{config.version}"
        )
        ...
        self.logger.info(
            f"Initialized {active_integrations}/{len(integration_status)} integrations"
        )
        ...
        processor_config = EventStreamConfig(
            buffer_size=100, batch_size=10, websocket_enabled=True, websocket_port=8765
        )
    ...
    async def generate_sample_events(self, count: int = 50):
        ...
        for i in range(count):
            ...
            if event_type == EventType.HEALTH_CHECK:
                ...
                event.severity = (
                    EventSeverity.WARNING if i % 3 != 0 else EventSeverity.ERROR
                )
                event.source = source
                ...
                event.resource_usage = ResourceUsage(
                    cpu_percent=85 + i % 15, memory_percent=70 + i % 25
                )
                event.details = {
                    "resource_type": "cpu",
                ...
                event = EventBuilder.security_alert(
                    ...
                    severity
                    if severity in [EventSeverity.ERROR, EventSeverity.WARNING]
            ...
            if i < count * 0.8:  # 80% in the past 24 hours
                event.timestamp = current_time - timedelta(hours=i % 24, minutes=i % 60)
    ...
    async def demonstrate_event_analysis(self, events):
        ...
        for trend in trends[:3]:  # Show first 3
            self.logger.info(
                f"  - {trend.name}: {trend.direction} (confidence: {trend.confidence:.2f})"
            )
        ...
        for pattern in patterns[:3]:  # Show first 3
            self.logger.info(
                f"  - {pattern.name}: {pattern.pattern_type} (frequency: {pattern.frequency})"
            )
        ...
        for prediction in predictions[:3]:  # Show first 3
            self.logger.info(
                f"  - {prediction.description} (probability: {prediction.probability:.2f})"
            )
    ...
    async def demonstrate_alerting(self, events):
        ...
        for alert in alerts[:5]:  # Show first 5
            self.logger.info(
                f"  - Alert {alert.id}: {alert.title} ({alert.severity.value})"
            )
    ...
    async def demonstrate_reporting(self):
        ...
        self.logger.info(
            f"  - Fleet health score: {health_report.fleet_health_score:.1f}%"
        )
        self.logger.info(f"  - Health status: {health_report.system_health_status}")
        self.logger.info(
            f"  - Systems: {health_report.healthy_systems}/{health_report.total_systems} healthy"
        )
        ...
        self.logger.info("Security Report:")
        ...
        self.logger.info(
            f"  - Security events: {security_report.total_security_events}"
        )
        ...
        self.logger.info("Operational Report:")
        self.logger.info(
            f"  - Operational score: {operational_report.operational_score:.1f}%"
        )
        self.logger.info(
            f"  - Operations: {operational_report.successful_operations}/{operational_report.total_operations} successful"
        )
        self.logger.info(
            f"  - Success rate: {operational_report.operation_success_rate:.1f}%"
        )
        ...
        self.logger.info(
            f"  - Health report: {'health_report' in comprehensive_report}"
        )
        self.logger.info(
            f"  - Security report: {'security_report' in comprehensive_report}"
        )
        self.logger.info(
            f"  - Operational report: {'operational_report' in comprehensive_report}"
        )
        self.logger.info(
            f"  - Dashboard data: {'dashboard_data' in comprehensive_report}"
        )
    ...
    async def demonstrate_mcp_tools(self):
        ...
        if health_result["success"]:
            ...
            self.logger.info(
                f"  - Health score: {health_summary['fleet_health_score']:.1f}%"
            )
        ...
        if security_result["success"]:
            ...
            self.logger.info(
                f"  - Security status: {security_summary['security_status']}"
            )
            self.logger.info(
                f"  - Security events: {security_summary['total_security_events']}"
            )
        ...
        if alerts_result["success"]:
            self.logger.info(
                f"  - Active alerts: {alerts_result['total_active_alerts']}"
            )
    ...
    async def demonstrate_system_integration(self):
        ...
        self.logger.info(
            f"  - Active integrations: {integration_status['active_count']}/{integration_status['total_count']}"
        )
        ...
        if cycle_result["success"]:
            self.logger.info("Integration cycle completed:")
            self.logger.info(
                f"  - Events collected: {cycle_result['events_collected']}"
            )
            self.logger.info(
                f"  - Active integrations: {cycle_result['active_integrations']}"
            )
        else:
            self.logger.warning(
                f"Integration cycle failed: {cycle_result.get('error', 'Unknown error')}"
            )
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            if final_status["success"]:
                ...
                self.logger.info(
                    f"Final System Status: {system_status['overall_status']}"
                )
                self.logger.info(f"Health Score: {system_status['health_score']:.1f}%")
                self.logger.info(f"Active Alerts: {system_status['active_alerts']}")
                self.logger.info(
                    f"Total Events (24h): {system_status['total_events_24h']}"
                )
            self.logger.info("=" * 60)
