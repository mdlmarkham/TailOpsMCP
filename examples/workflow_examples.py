...
from src.models.workflow_models import (
    WorkflowBlueprint, WorkflowExecution, ExecutionStatus, StepResult,
    WorkflowCategory, ApprovalRequirement, RollbackAction, RollbackPlan
)
...
from src.services.workflow_blueprints import (
    ...
    SafeUpgradeWorkflow, DisasterRecoveryWorkflow,
    create_production_environment_workflow, create_daily_backup_workflow
)
from src.services.workflow_integration import (
    WorkflowPolicyIntegration, WorkflowInventoryIntegration,
    WorkflowEventIntegration, WorkflowCapabilityIntegration, WorkflowOrchestrator
)
...
class WorkflowExamples:
    ...
    async def example_provision_environment():
        ...
        print(f"Required approvals: {[step.step_id for step in workflow.steps if step.requires_approval]}")
...
class WorkflowTestScenarios:
    ...
    async def test_workflow_with_approvals():
        ...
        try:
            ...
            print(f"✅ Workflow with approvals created: {len(approval_steps)} approval steps")
            return True
    ...
    async def test_workflow_rollback():
        ...
        try:
            ...
            print(f"✅ Workflow with rollback created: {len(rollback_plan.actions)} rollback actions")
            return True
...
class WorkflowPerformanceTests:
    ...
    async def test_workflow_validation_performance():
        ...
        try:
            ...
            for workflow in workflows:
                validation_result = workflow.validate()
                if validation_result:
                    print(f"❌ Validation failed for workflow {workflow.name}: {validation_result}")
                    return False
            
            end_time = datetime.now()
            validation_time = (end_time - start_time).total_seconds()
            
            print(f"✅ Validated {len(workflows)} workflows in {validation_time:.2f} seconds")
            
            if validation_time > 5.0:  # Should complete within 5 seconds
                print(f"⚠️  Validation took longer than expected: {validation_time:.2f}s")
    ...
    async def test_workflow_serialization_performance():
        ...
        try:
            ...
            print(f"✅ Serialized workflow 100 times in {serialization_time:.2f} seconds")
            
            if serialization_time > 2.0:  # Should complete within 2 seconds
                print(f"⚠️  Serialization took longer than expected: {serialization_time:.2f}s")
